...
For the sake of simplicity and consistency, public custom DNS is also sent inside the tunnel when
using a plain old static `resolv.conf`, even though it is technically possible to exclude public
custom DNS in that case.
