...
block all connections to avoid leaking any data.

The objective is to ensure no data leaks from the tunnel while the user has requested for a secure
connection. This means that if a secure connection can't be established, no connection should be
available at all. Out of tunnel connections are only allowed after the user explicitly requests for
a disconnection, or in other words, for the connections to be unsecured. This implements the
functionality that is sometimes described as a "kill-switch".
