...
lost upon a reboot (except on Windows, cf. below). In other words, if the "Always require VPN"
...
wants to continue to enforce a blocking policy. These block any traffic occurring before the
service has started back up again during boot, including before the BFE service has started.

As with "Always require VPN", enabling "Auto-connect" in the service will also cause it to
