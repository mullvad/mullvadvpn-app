...
- Obfuscation - a service that obfuscates WireGuard traffic, often running on the same WireGuard
...
- If the tunnel protocol is specified as WireGuard
  and obfuscation mode is set to _auto_, then the transport protocol is not applicable as only UDP
  endpoints exist and any port will be matched. If obfuscation is set to auto:
