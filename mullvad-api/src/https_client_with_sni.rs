...
impl InnerConnectionMode {
    ...
    ///
    /// The `make_proxy_stream` closure recieves a [`TcpStream`] and produces a
    /// stream which can be used to send and recieve data from some proxy
    /// protocol.
    ///
    /// If no proxy protocl is used this closure may return the [`TcpStream`]
    /// itself. See for example how a connection is established from connection
    /// mode [`InnerConnectionMode::Direct`].
