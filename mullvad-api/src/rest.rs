...
/// An active TCP stream where HTTP requests can be sent.
struct Connection {
    /// Handle for sending HTTP requests.
    send_request: http1::SendRequest<Full<Bytes>>,
    /// Time until an unused [`Connection`] should be discarded.
    idle_timeout: Duration,
    /// Abort handle for the tokio task driving the underlying I/O.
...
/// An actor message for `RequestService` to send `request`. Reply is sent on `response_tx`.
#[derive(Debug)]
...
impl RequestService {
        ...
        loop {
            let evict_connection = async {
                match &self.connection {
                    None => pending().await,
                    Some(c) => {
                        // To keep things simple, we does not take into account in-flight `Body`s
    ...
    async fn get_connection_inner(&mut self) -> Result<Connection> {
    ...
    /// Get [`Self::connection`], or (if `None`), establish a new connection.
    ///
    /// Will retry up to `3` times on connection error.
    /// Returns [`Error::TimeoutError`] after [`CONNECT_TIMEOUT`].
...
impl IntoFuture for Request<Full<Bytes>> {
    ...
    // TODO: When ATPIT is stabilized, switch to `impl Future`
...
mod test {
    ...
    // Use macros (instead of fns) for mock helpers to make insta snapshots work.
    macro_rules! mock_auth_token {
        ($server:expr, $expiry:expr) => {{
            let server: &mut mockito::ServerGuard = $server;
            let expiry: &str = $expiry;
            server
