...
impl DevicesProxy {
    ...
    > + use<> {
        let request = self.create_reponse(account, pubkey);
    ...
    pub fn get(
        ...
        let request = self.get_device(account, id);
        async move {
    ...
    pub fn list(
        ...
        let request = self.get_devices(account);
        async move {
            let data = request.await?.deserialize().await?;
            Ok(data)
        }
    ...
    {
        let request = self.replace_wg_key_reponse(account, id, pubkey);
        async move {
            let DeviceResponse {
                ipv4_address,
                ipv6_address,
