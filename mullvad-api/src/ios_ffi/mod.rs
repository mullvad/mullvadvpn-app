use std::{net::SocketAddr, ptr, sync::Arc};
...
pub extern "C" fn mullvad_api_list_devices(
    ...
    let result: Result<_, rest::Error> = runtime.block_on(devices_proxy.list(account));

    match result {
        Ok(devices) => {
            let iter = device::MullvadApiDeviceIterator::new(devices);
...
pub extern "C" fn mullvad_api_delete_account(
    ...
    let result: Result<_, rest::Error> = runtime.block_on(accounts_proxy.delete_account(account));
