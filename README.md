...
```
keytool -genkey -v -keystore /home/user/app-keys.jks -alias release -keyalg RSA -keysize 2048 -validity 10000
```
...
Now a signed release APK can be build with:
```
./build.sh
```
...
Running the `android/build.sh` script will build the necessary Rust daemon for all supported ABIs
and build the final APK. You may pass a `--dev-build` to build the Rust daemon and the UI in debug
mode.
```bash
cd android
./build.sh
