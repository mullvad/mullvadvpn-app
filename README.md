...
The log directory can be change by setting the `MULLVAD_LOG_DIR` environment variable.
...
The cache directory can be change by setting the `MULLVAD_CACHE_DIR` environment variable.
...
The full path to the RPC address file can be change by setting the `MULLVAD_RPC_ADDRESS_PATH`
