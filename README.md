...
1. Install all JavaScript dependencies (unless you already have) and package the application with:
    ```bash
    yarn install
    yarn workspace desktop pack:<OS>  # <OS> can be linux, mac or win
    ```
...
- `$ yarn lint` - lint code
- `$ yarn workspace desktop pack:<OS>` - prepare app for distribution for your platform. Where `<OS>` can be
  `linux`, `mac` or `win`
