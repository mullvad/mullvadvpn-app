...
impl SystemdResolved {
    ...
    pub fn set_dns(&mut self, interface: impl Into<String>, servers: Vec<IpAddr>) -> Result<()> {
        let interface_name = interface.into();
        ...
        self.interface_links
            .push((interface_name, link_object_path));

        Ok(())
    }

    fn fetch_link(&mut self, interface_name: &str) -> Result<dbus::Path<'static>> {
        let interface_index =
