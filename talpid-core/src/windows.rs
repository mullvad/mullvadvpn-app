...
use winapi::shared::{
    ...
    ws2def::{AF_INET, AF_INET6, AF_UNSPEC, SOCKADDR_IN as sockaddr_in},
...
pub fn inet_sockaddr_from_socketaddr(addr: SocketAddr) -> SOCKADDR_INET {
    ...
    let mut sockaddr_inet: SOCKADDR_INET = unsafe { mem::zeroed() };

    match addr {
        SocketAddr::V4(_) => unsafe {
            *sockaddr_inet.si_family_mut() = AF_INET as u16;
            *sockaddr_inet.Ipv4_mut() = *(sockaddr.as_ptr() as *const _);
        },
        SocketAddr::V6(_) => unsafe {
            *sockaddr_inet.si_family_mut() = AF_INET6 as u16;
            *sockaddr_inet.Ipv6_mut() = *(sockaddr.as_ptr() as *const _);
        },
    }
    sockaddr_inet
}
...
pub fn try_socketaddr_from_inet_sockaddr(addr: SOCKADDR_INET) -> Result<SocketAddr> {
    unsafe {
        match *addr.si_family() as i32 {
            AF_INET => SockAddr::new(
                *(addr.Ipv4() as *const sockaddr_in as *const _),
                mem::size_of_val(addr.Ipv4()) as i32,
            )
            .as_socket()
            .ok_or(Error::UnknownAddressFamily(0)),
            AF_INET6 => SockAddr::new(
                *(addr.Ipv6() as *const sockaddr_in6 as *const _),
                mem::size_of_val(addr.Ipv6()) as i32,
            )
            .as_socket()
            .ok_or(Error::UnknownAddressFamily(0)),
            family => Err(Error::UnknownAddressFamily(family)),
