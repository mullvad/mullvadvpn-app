...
impl Firewall {
    ...
    fn get_allow_dns_rules(
        ...
        if is_local {
            ...
            let allow_nontunnel_tcp_in = self
                .create_rule_builder(FilterRuleAction::Pass)
                ...
                .keep_state(pfctl::StatePolicy::Keep)
                .tcp_flags(Self::get_tcp_flags())
                .from(pfctl::Endpoint::new(server, 53))
                .build()?;
            rules.push(allow_nontunnel_tcp_in);
            let allow_nontunnel_udp_in = self
                .create_rule_builder(FilterRuleAction::Pass)
                .direction(pfctl::Direction::In)
                .quick(true)
                .proto(pfctl::Proto::Udp)
                .from(pfctl::Endpoint::new(server, 53))
                .build()?;
            rules.push(allow_nontunnel_udp_in);
        } else {
