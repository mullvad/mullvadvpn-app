...
impl FirewallT for Firewall {
    ...
    fn new(args: FirewallArguments) -> Result<Self, Self::Error> {
        ...
        if args.initialize_blocked {
            ...
            let winfw_allowed_endpoint = if let Some(allow_endpoint) = args.allow_endpoint {
                let allow_endpoint_ip = Self::widestring_ip(allow_endpoint.address.ip());
                Some(WinFwEndpoint {
                    ip: allow_endpoint_ip.as_ptr(),
                    port: allow_endpoint.address.port(),
                    protocol: WinFwProt::from(allow_endpoint.protocol),
                })
            } else {
                None
            };
    ...
    fn apply_policy(&mut self, policy: FirewallPolicy) -> Result<(), Self::Error> {
        match policy {
            ...
            FirewallPolicy::Blocked {
                ...
                let cfg = &WinFwSettings::new(allow_lan);
                self.set_blocked_state(&cfg, &allow_endpoint)
            }
        }
    }
...
mod winfw {
    ...
    extern "system" {
        ...
        pub fn WinFw_InitializeBlocked(
            ...
            allow_endpoint: *const WinFwEndpoint,
            sink: Option<LogSink>,
            sink_context: *const u8,
        ) -> InitializationResult;
