...
error_chain!{
    errors{
        ...
        WfpctlFailure(desc: &'static str){
            description("Opque Wfpctl failure")
            display("Wfpctl failed when {}", desc)
        }
    }
    foreign_links {
        WideString(self::widestring::NulError) #[doc = "Invalid strings"];
    }
}

const WFPCTL_INIT_TIMEOUT: u32 = 30;
...
impl Firewall for WindowsFirewall {
    ...
    fn new() -> Result<Self> {
        let ok =
            unsafe { Wfpctl_Initialize(WFPCTL_INIT_TIMEOUT, Some(error_sink), ptr::null_mut()) };
        Self::handle_result(ok, "initialise wfpctl").map(|_| {
    ...
    fn reset_policy(&mut self) -> Result<()> {
        ...
        let result = unsafe { Wfpctl_Reset() };
        Self::handle_result(result, "resetting firewall")
    }
}

impl Drop for WindowsFirewall {
    fn drop(&mut self) {
        if unsafe { Wfpctl_Deinitialize() } {
...
impl WindowsFirewall {
    fn set_connecting_state(
        ...
        Self::handle_result(ok, "applying 'connecting' policy")
    }

    fn widestring_ip(ip: &IpAddr) -> Result<WideCString> {
        let buf = ip.to_string().encode_utf16().collect::<Vec<_>>();
        WideCString::new(buf).map_err(|e| e.into())
    }
    ...
    fn set_connected_state(
        ...
        let tunnel_alias =
            WideCString::new(tunnel_metadata.interface.encode_utf16().collect::<Vec<_>>())?;
        ...
        Self::handle_result(ok, "applying 'connected' policy")
    }

    // handles returns from FFI functions
    fn handle_result(ok: bool, description: &'static str) -> Result<()> {
        match ok {
            true => Ok(()),
...
mod ffi {
    ...
    pub extern "system" fn error_sink(msg: *const c_char, _ctx: *mut libc::c_void) {
        // an assumption that on x86 and arm, a null pointer is represented as a 0;
        if msg == ptr::null() {
            error!("log message is NULL");
            return;
        }
        unsafe {
    ...
    extern "system" {
        ...
        pub fn Wfpctl_Deinitialize() -> bool;

        pub fn Wfpctl_ApplyPolicyConnecting(settings: &WfpCtlSettings, relay: &WfpCtlRelay)
            -> bool;
