...
impl FirewallT for Firewall {
    ...
    fn new(args: FirewallArguments) -> Result<Self, Self::Error> {
        ...
        let app_strings = args.approved_applications
            .iter()
            .map(|app| Self::to_widestring(&app.to_string_lossy().into_owned()))
            .collect::<Vec<_>>();
...
impl Firewall {
    ...
    fn to_widestring(string: &String) -> WideCString {
        let buf = string.encode_utf16().collect::<Vec<_>>();
        WideCString::new(buf).unwrap()
    }
