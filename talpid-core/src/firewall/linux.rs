...
impl<'a> PolicyBatch<'a> {
    ...
    fn add_allow_tunnel_endpoint_rules(&mut self, endpoint: &Endpoint, use_fwmark: bool) {
        let mut in_pre_chain = Rule::new(&self.allow_tunnel_chain);
        check_endpoint(&mut in_pre_chain, End::Src, endpoint);
        in_pre_chain.add_expr(&nft_expr!(immediate data crate::linux::TUNNEL_FW_MARK));
        in_pre_chain.add_expr(&nft_expr!(meta mark set));

        self.batch.add(&in_pre_chain, nftnl::MsgType::Add);
