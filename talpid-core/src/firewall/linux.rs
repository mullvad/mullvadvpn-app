...
lazy_static! {
    ...
    static ref DONT_SET_KERNEL_CONFIG: bool = env::var("TALPID_FIREWALL_DONT_SET_SRC_VALID_MARK")
        .map(|v| v != "0")
        .unwrap_or(false);
}
...
impl Firewall {
    fn apply_kernel_config(policy: &FirewallPolicy) {
        if *DONT_SET_KERNEL_CONFIG {
            log::debug!("Not setting src_valid_mark");
            return;
        }

        if let FirewallPolicy::Connecting { .. } = policy {
            if let Err(v4_err) = crate::linux::set_src_valid_mark_sysctl_ipv4() {
                log::error!("Failed to apply src_valid_mark for IPv4: {}", v4_err);
            }

            if let Err(v6_err) = crate::linux::set_src_valid_mark_sysctl_ipv6() {
                log::error!("Failed to apply src_valid_mark for IPv6: {}", v6_err);
            }
        }
    }
