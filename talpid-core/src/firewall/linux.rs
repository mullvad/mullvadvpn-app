...
impl<'a> PolicyBatch<'a> {
    ...
    fn add_dns_rule(
        ...
        check_l3proto(&mut allow_rule, tunnel.v4_gateway.into())?;

        allow_rule.add_expr(&nft_expr!(payload ipv4 daddr))?;
        allow_rule.add_expr(&nft_expr!(cmp == tunnel.v4_gateway))?;
        ...
        if let Some(v6_gateway) = tunnel.v6_gateway {
            ...
            check_iface(&mut allow_rule, Direction::Out, &tunnel.interface[..])?;
            check_port(&mut allow_rule, protocol, End::Dst, 53)?;
            check_l3proto(&mut allow_rule, v6_gateway.into())?;

            allow_rule.add_expr(&nft_expr!(payload ipv6 daddr))?;
            allow_rule.add_expr(&nft_expr!(cmp == v6_gateway))?;
            self.batch.add(&allow_rule, nftnl::MsgType::Add)?;
        }
