...
impl Netfilter {
    fn policy_batch(
        ...
        let mut policy_batch = PolicyBatch::new(&self.table, delete_table_first)?;

        policy_batch.add_loopback_rules()?;
        policy_batch.add_dhcp_rules()?;
        policy_batch.add_policy_specific_rules(policy)?;

        Ok(policy_batch.batch.finalize()?)
    }
