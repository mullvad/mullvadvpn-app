...
pub fn string_from_guid(guid: &GUID) -> String {
    ...
    let length = unsafe { StringFromGUID2(guid, &mut buffer[0] as *mut _, buffer.len() as i32 - 1) } as usize;
    let length = std::cmp::max(0, length - 1);

    OsString::from_wide(&buffer[0..length]).to_string_lossy().to_string()
}
