    ...
    pub fn wait(mut self) -> Result<()> {
        if let Some(mut proxy_monitor) = self.proxy_monitor.take() {
            ...
            handle.spawn(async move {
                let result = tokio::task::spawn_blocking(|| Stopped::Tunnel(self.wait_tunnel()))
                    .await
                    .expect("tunnel task unexpectedly stopped");
                tx_tunnel.send(result).unwrap();
                let _ = proxy_close_handle.close();
            });

            handle.spawn(async move {
                let result = proxy_monitor.wait().await;
                tx_proxy.send(Stopped::Proxy(result)).unwrap();
            ...
            match result {
                ...
                Stopped::Proxy(proxy_result) => {
                    ...
                    match proxy_result {
                        Ok(()) => Err(Error::ProxyExited("No details".to_owned())),
                        Err(proxy::Error::UnexpectedExit(details)) => {
                            Err(Error::ProxyExited(details))
                        }
                        Err(proxy::Error::Io(error)) => Err(Error::MonitorProxyError(error)),
                    }
                }
            }
        } else {
