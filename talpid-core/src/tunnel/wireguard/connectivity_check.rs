...
impl ConnectivityMonitor {
    ...
    pub fn establish_connectivity(&mut self) -> Result<bool, Error> {
        ...
        while start.elapsed() < PING_TIMEOUT {
            iters += 1;
            log::debug!("Checking connectivity for the {}th time", iters);
...
mod test {
    ...
    fn test_conn_state_connects() {
        ...
        conn_state.update(Instant::now(), Stats{rx_bytes: 1, tx_bytes: 0});
    ...
    fn test_conn_state_traffic_times_out_after_connecting() {
        ...
        let start = Instant::now().checked_sub(TRAFFIC_TIMEOUT + Duration::from_secs(1)).unwrap();
        let mut conn_state = ConnState::new(start, Default::default());

        let connect_time = Instant::now().checked_sub(TRAFFIC_TIMEOUT).unwrap();
        conn_state.update(connect_time, Stats{rx_bytes: 1, tx_bytes: 0});
    ...
    fn test_conn_state_rx_times_out_after_connecting() {
        let start = Instant::now().checked_sub(BYTES_RX_TIMEOUT + Duration::from_secs(1)).unwrap();
        let mut conn_state = ConnState::new(start, Default::default());

        conn_state.update(start, Stats{rx_bytes: 1, tx_bytes: 0});

        let update_time = Instant::now().checked_sub(BYTES_RX_TIMEOUT).unwrap();
        conn_state.update(update_time, Stats{rx_bytes: 1, tx_bytes: 1});
