        ...
        match self {
            ...
            Endpoint(endpoint) => match &endpoint {
                InetAddr::V4(sockaddr_in) => {
                    ...
                    buffer
                        .write(unsafe { struct_serializer::as_byte_slice(sockaddr_in) })
                        .expect("Buffer too small for sockaddr_in");
                }
                InetAddr::V6(sockaddr_in6) => {
                    // SAFETY: sockaddr_in6 has no padding
                    buffer
                        .write(unsafe { struct_serializer::as_byte_slice(sockaddr_in6) })
            ...
            LastHandshakeTime(last_handshake) => {
                ...
                buffer
                    .write(unsafe { struct_serializer::as_byte_slice(timespec) })
