...
fn serialize_config(config: &Config) -> Result<Vec<MaybeUninit<u8>>> {
    ...
    buffer.extend(crate::slice::as_uninit_byte_slice(&header));
    ...
    for peer in &config.peers {
        ...
        buffer.extend(crate::slice::as_uninit_byte_slice(&wg_peer));
        ...
        for allowed_ip in &peer.allowed_ips {
            ...
            buffer.extend(crate::slice::as_uninit_byte_slice(&wg_allowed_ip));
        }
    }
...
mod tests {
    ...
    fn test_config_deserialization() {
        let config_buffer = crate::slice::as_uninit_byte_slice(&*WG_STRUCT_CONFIG);
