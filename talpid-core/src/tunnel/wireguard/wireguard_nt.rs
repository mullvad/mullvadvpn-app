...
use crate::{struct_serializer, windows};
...
fn serialize_config(config: &Config) -> Result<Vec<MaybeUninit<u8>>> {
    ...
    buffer.extend(struct_serializer::as_uninit_byte_slice(&header));
    ...
    for peer in &config.peers {
        ...
        buffer.extend(struct_serializer::as_uninit_byte_slice(&wg_peer));
        ...
        for allowed_ip in &peer.allowed_ips {
            ...
            buffer.extend(struct_serializer::as_uninit_byte_slice(&wg_allowed_ip));
        }
    }
...
mod tests {
    ...
    fn test_config_deserialization() {
        let config_buffer = struct_serializer::as_uninit_byte_slice(&*WG_STRUCT_CONFIG);
