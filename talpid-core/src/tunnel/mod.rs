...
impl TunnelMonitor {
    ...
    fn ensure_ipv6_can_be_used_if_enabled(tunnel_options: &TunnelOptions) -> Result<()> {
        let ipv6_enabled_in_app = tunnel_options.openvpn.enable_ipv6;
        let ipv6_enabled_in_os = is_ipv6_enabled();

        if ipv6_enabled_in_app && !ipv6_enabled_in_os {
            Err(Error::from(ErrorKind::EnableIpv6Error))
        } else {
            Ok(())
        }
    }
