...
pub struct OpenVpnMonitor<C: OpenVpnBuilder = OpenVpnCommand> {
    ...
    event_server_quit_rx: Option<tokio02::sync::oneshot::Receiver<()>>,
...
impl<C: OpenVpnBuilder + 'static> OpenVpnMonitor<C> {
        ...
        L: Fn(openvpn_plugin::EventType, HashMap<String, String>) + Send + Sync + 'static,
    {
        ...
        let (quit_tx, event_server_quit_rx) = tokio02::sync::oneshot::channel();
        ...
        runtime.spawn(async {
            event_server::start(ipc_path_copy, on_event, event_server_abort_rx).await;
            let _ = quit_tx.send(());
        });

        let (start_tx, start_rx) = std::sync::mpsc::channel();
        runtime.block_on(async {
            // FIXME: I'm assuming that this will necessarily block until the server is running.
            // verify
            start_tx.send(());
        });
        ...
        Ok(OpenVpnMonitor {
            ...
            runtime,
            event_server_quit_rx: Some(event_server_quit_rx),
    ...
    fn inner_wait_tunnel(&mut self) -> WaitResult {
        ...
        let event_quit_rx = self
            .event_server_quit_rx
            .take()
            .expect("No event_dispatcher");
        let runtime_handle = self.runtime.handle().clone();
        thread::spawn(move || {
            runtime_handle.block_on(event_quit_rx);
            dispatcher_tx.send(WaitResult::EventDispatcher).unwrap();

            log::info!("WaitResult::EventDispatcher");
...
use proto::{
    open_vpn_event_proxy_server::{OpenVpnEventProxy, OpenVpnEventProxyServer},
    Empty, EventType,
};
    ...
    L: Fn(openvpn_plugin::EventType, HashMap<String, String>) + Send + Sync + 'static,
{
    async fn openvpn_event(
        ...
        (self.on_event)(event, request.env);
...
mod event_server {
    ...
    use super::{OpenVpnEventProxyImpl, OpenVpnEventProxyServer};
