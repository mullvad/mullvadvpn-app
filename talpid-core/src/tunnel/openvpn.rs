...
fn parse_openvpn_dict_routes(
    ...
    // TODO: lazy_static?
    let expression = Regex::new(r"route_(ipv6_)?(\w+)_(\d+)").unwrap();

    for (key, value) in env.iter() {
        if let Some(captures) = expression.captures(key) {
            let route_index: u32 = captures.get(3).unwrap().as_str().parse().unwrap();
            let property = captures.get(2).unwrap().as_str();

            let is_ipv6 = captures.get(1).map(|_| true).unwrap_or(false);
    ...
    let parse_dict = |routes: HashMap<u32, HashMap<&str, &str>>,
     ...
     -> std::result::Result<Vec<OpenVpnRoute>, RouteParseError> {
        ...
        routes.sort_by(|(idx0, _), (idx1, _)| idx0.partial_cmp(idx1).unwrap());
        ...
        for (_, route) in routes {
            ...
            let network = if is_ipv4 {
                ...
                let prefix = if let Some(mask) = route.get("netmask") {
                    let mask = mask.parse().map_err(RouteParseError::ParseMaskAddress)?;
                    ipnetwork::ipv4_mask_to_prefix(mask).map_err(RouteParseError::ParseMask)?
                } else {
                    32
                };

                IpNetwork::new(network, prefix).map_err(RouteParseError::ParseNetwork)?
            } else {
