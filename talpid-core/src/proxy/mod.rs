...
use std::{fmt, sync::mpsc, path::PathBuf};
...
impl fmt::Debug for ProxyRuntimeData {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "ProxyRuntimeData {{ monitor: present, port: {} }}", self.port)
    }
}
...
pub fn start_proxy(settings: &openvpn::ProxySettings, resource_data: &ProxyResourceData) -> Result<ProxyRuntimeData> {
    match settings {
