...
impl fmt::Display for ShadowsocksCommand {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        fmt.write_str(&shell_escape::escape(self.shadowsocks_bin.to_string_lossy()))?;
...
#[cfg(unix)]
const SHADOWSOCKS_BIN_FILENAME: &str = "shadowsocks";
#[cfg(windows)]
const SHADOWSOCKS_BIN_FILENAME: &str = "shadowsocks.exe";

impl ShadowsocksProxyMonitor {
    pub fn start(settings: &ShadowsocksProxySettings, resource_data: &ProxyResourceData) -> Result<(Self, u16)>
    {
        let binary = resource_data.binaries_dir.join(SHADOWSOCKS_BIN_FILENAME).into_os_string();

        let mut cmd = ShadowsocksCommand::new(binary)
            .local(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), settings.port))
            .peer(settings.peer)
            .peer_password(settings.password.clone())
            .cipher(settings.cipher.clone())
            .build();

        let logfile = resource_data.logs_dir.join(SHADOWSOCKS_LOG_FILENAME);
        ...
        let extract_bound_port = || {
            Self::get_bound_port(Self::get_readable_fd(&logfile)?, &subproc)
        };

        match extract_bound_port() {
            Ok(port) => {
                Ok((Self {
    ...
    fn get_readable_fd(file: &PathBuf) -> Result<File> {
    ...
    fn get_bound_port(logfile: File, subproc: &duct::Handle) -> Result<u16> {
        ...
        return Err(Error::new(ErrorKind::Other, "Could not determine which port Shadowsocks has bound to"));
    }

    fn parse_port(logline: &str) -> Result<u16> {
        // TODO: Compile once and reuse.
        let re = Regex::new(r"(?:TCP Listening on \d+\.\d+\.\d+\.\d+:)(\d+$)")
            .map_err(|_| Error::new(ErrorKind::Other, "Failed to compile regex"))?;

        if let Some(captures) = re.captures(logline) {
            return Ok(captures[1].parse().map_err(|_| Error::new(ErrorKind::Other, "Failed to parse port number string"))?);
        }
