...
impl RouteManager {
    ...
    pub fn set_routes(&mut self, required_routes: RequiredRoutes) -> Result<(), imp::Error> {
        self.inner.set_routes(required_routes)
    }

    /// Remove previously set routes from the routign table
    pub fn reset_routes(&mut self) -> Result<(), imp::Error> {
...
impl Drop for RouteManager {
    fn drop(&mut self) {
        if let Err(e) = self.reset_routes() {
...
pub trait RoutingT: Sized {
    ...
    /// Error type of the implementation
    fn set_routes(&mut self, required_routes: RequiredRoutes) -> Result<(), Self::Error>;
    /// Error type of the implementation
    fn reset_routes(&mut self) -> Result<(), Self::Error>;
    /// Retrieves the gateway for the default route
