...
impl RouteManagerImpl {
    ...
    fn parse_route_message_inner(&self, msg: RouteMessage) -> Result<Option<Route>> {
        ...
        if prefix.is_none() && gateway.is_some() {
            prefix = if is_ipv4 {
                Some("0.0.0.0/0".parse().expect("failed to parse ipnetwork"))
            } else {
                Some("::/0".parse().expect("failed to parse ipnetwork"))
            };
        }

        if device.is_none() && (node_addr.is_none() && gateway.is_none()) || prefix.is_none() {
