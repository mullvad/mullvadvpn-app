...
impl DirContext {
    fn new<P: AsRef<Path>>(
        ...
        let handle = unsafe {
            CreateIoCompletionPort(
                dir_handle.as_raw_handle() as isize,
                io_completion_port.as_raw_handle() as isize,
                completion_key,
    ...
    fn read_directory_changes(&mut self) -> io::Result<()> {
        ...
        if unsafe {
            ReadDirectoryChangesW(
                self.dir_handle.as_raw_handle() as isize,
    ...
    fn cancel_io(&mut self) -> io::Result<bool> {
        if unsafe { CancelIoEx(self.dir_handle.as_raw_handle() as isize, ptr::null_mut()) } == 0 {
