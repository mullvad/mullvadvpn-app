...
impl VolumeMonitor {
    pub fn spawn(
        ...
        let internal_monitor_task = tokio::spawn(start_frontend_monitor(
            known_state.clone(),
            path_monitor.clone(),
            update_tx.clone(),
            paths.clone(),
...
async fn start_frontend_monitor(
    ...
    while let Some(()) = volume_update_rx.next().await {
        // May have to keep track of current state to do things properly. Call `GetLogicalDrives`
...
fn start_internal_monitor(
    ...
    create_hidden_window(move |window, message, w_param, l_param| {
        if !is_broadcast_message(message, w_param) {
...
/// Each bit refers to a volume letter. The first bit refers to 'A', the
/// second to 'B', the third to 'C', etc.
        ...
        if let Some(path::Component::Prefix(prefix)) = path.components().next() {
            match prefix.kind() {
                path::Prefix::VerbatimDisk(disk) | path::Prefix::Disk(disk) => {
                    let disk = disk.saturating_sub('A' as u8);
                    if (volumes.wrapping_shr(u32::from(disk)) & 1) != 0 {
                        return true;
                    }
                }
                _ => (),
...
fn is_broadcast_message(message: u32, w_param: usize) -> bool {
...
unsafe fn parse_broadcast(broadcast: &DEV_BROADCAST_HDR) -> u32 {
