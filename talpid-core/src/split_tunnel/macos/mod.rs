...
pub enum Error {
    ...
    #[error(display = "Process monitor error")]
    Process(#[error(source)] process::Error),
    /// Failed to initialize split tunnel
    #[error(display = "Failed to initialize split tunnel")]
    InitializeTunnel(#[error(source)] tun::Error),
    /// Split tunnel is unavailable
    #[error(display = "Split tunnel is unavailable")]
    Unavailable,
}
