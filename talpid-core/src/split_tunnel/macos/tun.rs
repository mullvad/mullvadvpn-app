...
pub enum Error {
    ...
    #[error(display = "Failed to create split tunnel interface")]
    CreateSplitTunnelInterface(#[error(source)] tun::Error),
    /// Failed to begin capture on split tunnel utun
    #[error(display = "Failed to begin capture on split tunnel utun")]
    CaptureSplitTunnelDevice(#[error(source)] pcap::Error),
    /// Failed to set direction on capture
    #[error(display = "Failed to set direction on pcap")]
    SetDirection(#[error(source)] pcap::Error),
    /// Failed to enable nonblocking I/O
    #[error(display = "Failed to enable nonblocking I/O")]
    EnableNonblock(#[error(source)] pcap::Error),
    /// pcap_create failed
    #[error(display = "pcap_create failed: {}", _0)]
    CreatePcap(String),
    /// Failed to create packet stream
    #[error(display = "Failed to create packet stream")]
    CreateStream(#[error(source)] pcap::Error),
    /// Failed to get next packet
    #[error(display = "Failed to get next packet")]
    GetNextPacket(#[error(source)] pcap::Error),
    /// Failed to get default routes
    #[error(display = "Failed to get default routes")]
    GetDefaultRoutes(#[error(source)] talpid_routing::Error),
    /// Failed to get default gateways
    #[error(display = "Failed to get default gateways")]
    GetDefaultGateways(#[error(source)] talpid_routing::Error),
    /// Failed to get interface addresses
    #[error(display = "Failed to get interface addresses")]
    GetInterfaceAddresses(#[error(source)] nix::Error),
    /// Using different interfaces for IPv4 and IPv6 is not supported
    #[error(display = "Using different interfaces for IPv4 and IPv6 is not supported")]
    DefaultInterfaceMismatch,
    /// Found no suitable default interface
    #[error(display = "Found no suitable default interface")]
    NoDefaultInterface,
    /// Failed to create BPF device for default interface
    #[error(display = "Failed to create BPF device for default interface")]
    CreateDefaultBpf(#[error(source)] bpf::Error),
    /// Failed to configure BPF device for default interface
    #[error(display = "Failed to configure BPF device for default interface")]
    ConfigDefaultBpf(#[error(source)] bpf::Error),
    /// Failed to retrieve BPF buffer size
    #[error(display = "Failed to retrieve BPF buffer size")]
    GetBpfBufferSize(#[error(source)] bpf::Error),
    /// Failed to create BPF device for VPN tunnel
    #[error(display = "Failed to create BPF device for VPN tunnel")]
    CreateVpnBpf(#[error(source)] bpf::Error),
    /// Failed to configure BPF device for VPN
    #[error(display = "Failed to configure BPF device for VPN tunnel")]
    ConfigVpnBpf(#[error(source)] bpf::Error),
    /// Failed to stop tunnel redirection
    #[error(display = "Failed to stop tunnel redirection")]
    StopRedirect,
    /// Failed to receive next pktap packet
    #[error(display = "Failed to receive next pktap packet")]
    PktapStreamStopped,
}
