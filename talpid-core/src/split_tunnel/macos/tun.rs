...
async fn create_utun() -> Result<tun::AsyncDevice, Error> {
    ...
    let tun_name = tun_device.tun_name().expect("tun has name").to_owned();
...
fn redirect_packets(
    ...
    let tun_name = st_tun_device.tun_name().expect("tun has name");
    let pktap_stream = capture_outbound_packets(&tun_name)?;
    redirect_packets_for_pktap_stream(
        st_tun_device,
...
fn redirect_packets_for_pktap_stream(
    ...
    let st_utun_name = st_tun_device.tun_name().expect("tun has name");
