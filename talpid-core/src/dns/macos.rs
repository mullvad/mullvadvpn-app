...
impl State {
    ...
    fn reset(&mut self, store: &SCDynamicStore) -> Result<()> {
        ...
        let old_backup = std::mem::replace(&mut self.backup, HashMap::new());
...
const MAX_CHANGES_PER_5_SECONDS: usize = 25;
...
impl ChangeCounter {
    fn new() -> Self {
        Self {
            changes: Vec::with_capacity(MAX_CHANGES_PER_5_SECONDS),
        }
    }
    ...
    fn increment(&mut self) -> bool {
        ...
        self.changes.len() >= MAX_CHANGES_PER_5_SECONDS
    }
}
