...
pub enum Error {
    #[error(display = "Failed to initialize a connection to dbus")]
    ...
    #[error(display = "system-resolved DNS 127.0.0.53, is not currently configured")]
    NoDnsPointsToResolved,

    #[error(display = "Systemd resolved not detected")]
    NoSystemdResolved(#[error(cause)] dbus::Error),

    #[error(display = "Failed to read system-resolved's resolv.conf")]
    ...
    #[error(display = "Failed to perform RPC call on DBus")]
    DBusRpcError(#[error(cause)] dbus::Error),

    #[error(display = "Failed to match the returned DBus object with expected type")]
