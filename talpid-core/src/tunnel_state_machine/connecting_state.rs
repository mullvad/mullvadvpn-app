...
impl ConnectingState {
    ...
    fn spawn_tunnel_monitor_wait_thread(
        ...
        thread::spawn(move || {
            ...
            let event = Self::wait_for_tunnel_monitor(tunnel_monitor);
            ...
            if tunnel_close_event_tx.send(event).is_err() {
    ...
    fn wait_for_tunnel_monitor(tunnel_monitor: TunnelMonitor) -> Option<BlockReason> {
        match tunnel_monitor.wait() {
            ...
            | error @ Err(tunnel::Error(
                ...
                warn!("{}", chained_error.unwrap_err().display_chain());
