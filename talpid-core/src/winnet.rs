use self::api::*;
pub use self::api::ErrorSink as ErrorSink;
use std::{
    ffi::OsString,
    ptr,
};
...
pub fn ensure_top_metric_for_interface(interface_alias: &str) -> Result<bool, Error> {
    ...
    match metric_result {
        ...
        i => {
            log::error!(
                "Unexpected return code from WinNet_EnsureTopMetric: {}",
                i
            );
...
pub fn get_tap_interface_ipv6_status() -> Result<bool, Error> {
    let tap_ipv6_status = unsafe { WinNet_GetTapInterfaceIpv6Status(Some(error_sink), ptr::null_mut()) };
...
pub fn get_tap_interface_alias() -> Result<OsString, Error> {
    ...
    let status = unsafe { WinNet_GetTapInterfaceAlias(&mut alias_ptr as *mut _, Some(error_sink), ptr::null_mut()) };
