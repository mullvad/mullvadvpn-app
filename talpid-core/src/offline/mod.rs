...
pub fn spawn_monitor(
    ...
    let damper = graceful_stream::GracefulStream::new(rx, ::std::time::Duration::from_secs(GRACE_PERIOD));
    // damper.forward(sender.map_err(|e| log::error!("failed to send command: {}", e)));
    handle.spawn(
        sender
