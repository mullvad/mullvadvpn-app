...
impl MonitorHandle {
    pub async fn is_offline(&mut self) -> bool {
        match public_ip_not_reachable(&self.handle).await {
...
pub async fn spawn_monitor(sender: Weak<UnboundedSender<TunnelCommand>>) -> Result<MonitorHandle> {
    ...
    let mut is_offline = public_ip_not_reachable(&handle).await?;
    ...
    tokio::spawn(async move {
        while let Some(_new_message) = messages.next().await {
            match sender.upgrade() {
                Some(sender) => {
                    let new_offline_state = public_ip_not_reachable(&handle).await.unwrap_or(false);
...
mod test {
    ...
    fn test_route_table_query() {
        ...
        runtime
            .block_on(public_ip_not_reachable(&handle))
