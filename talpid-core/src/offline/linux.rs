...
error_chain! {
    errors {
        ...
        NetlinkError {
            description("Error while listening on netlink socket")
        }
        NetlinkDisconnected {
...
pub fn spawn_monitor(sender: UnboundedSender<TunnelCommand>) -> Result<MonitorHandle> {
    thread::spawn(|| {
        if let Err(error) = monitor_event_loop(sender) {
            let chained_error = error.chain_err(|| "Failed to run link monitor event loop");
...
fn list_links() -> Result<Vec<Link>> {
    let (connection, connection_handle) = iproute2::new_connection()?;
    ...
    match connection.select2(links_request).wait() {
        ...
        Err(Either::A((error, _))) => bail!(error),
...
fn monitor_event_loop(sender: UnboundedSender<TunnelCommand>) -> Result<()> {
    let mut socket = TokioSocket::new(Protocol::Route)?;
    socket.bind(&SocketAddr::new(1, RTMGRP_NOTIFY | RTMGRP_LINK))?;
    let channel = NetlinkFramed::new(socket, NetlinkCodec::<NetlinkMessage>::new());

    let mut link_monitor = LinkMonitor::new(sender)?;

    channel
        .for_each(move |(message, _address)| {
