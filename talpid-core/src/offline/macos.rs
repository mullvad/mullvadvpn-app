...
async fn handle_route_changes(
    mut route_listener: impl Stream<Item = DefaultRouteEvent> + Unpin,
    state: Arc<Mutex<ConnectivityInner>>,
    notify_tx: Arc<UnboundedSender<Connectivity>>,
) {
...
/// When the local DNS resolver is disabled, the offline monitor synthesizes an offline state between
/// network switches and before coming online from an offline state. This is done to work around
/// issues with DNS being blocked due to macOS's connectivity check. In the offline state, a DNS
/// server on localhost prevents the connectivity check from being blocked.
async fn handle_route_changes_with_synthetic_state(
    route_listener: impl Stream<Item = DefaultRouteEvent> + Unpin,
    state: Arc<Mutex<ConnectivityInner>>,
    notify_tx: Arc<UnboundedSender<Connectivity>>,
) {
    ...
    let mut real_state = { state.lock().unwrap().clone() };
