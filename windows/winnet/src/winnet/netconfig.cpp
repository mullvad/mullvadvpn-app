...
void SetIpv6BindingForBindName(INetCfg *netCfg, const std::wstring &bindName, bool enable)
{
	...
	for (; S_OK == result; result = pathsEnum->Next(1, &bindingPath, nullptr))
	{
		...
		while (S_OK == enumInterface->Next(1, &iface, nullptr))
		{
			...
			wchar_t *componentBindNameTemp = 0;

			status = cfgComponent->GetBindName(&componentBindNameTemp);
			...
			std::wstring componentBindName(componentBindNameTemp);
			CoTaskMemFree(componentBindNameTemp);

			if (0 == _wcsicmp(bindName.c_str(), componentBindName.c_str()))
			{
				//
...
void EnableIpv6ForAdapter(const std::wstring &adapterGuid)
{
	...
	{
		std::stringstream ss;
		ss << "Failed to instantiate COM: " << result;
		THROW_ERROR(ss.str().c_str());
