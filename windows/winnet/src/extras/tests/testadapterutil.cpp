...
MIB_IF_TABLE2 MibIfTable::table()
{
	MibTable tableAdapter{
		static_cast<ULONG>(m_table.size()),
		m_table.data()
	};
	return *reinterpret_cast<MIB_IF_TABLE2*>(&tableAdapter);
}
...
{
	// C-level
	*tableOut = static_cast<MIB_IF_TABLE2*>(malloc(
		sizeof(MIB_IF_TABLE2)
		+ sizeof(MIB_IF_ROW2) * m_adapterTable.entries().size()
	));

	(*tableOut)->NumEntries = static_cast<ULONG>(m_adapterTable.entries().size());

	std::copy(
		m_adapterTable.entries().begin(),
		m_adapterTable.entries().end(),
		static_cast<MIB_IF_ROW2*>((*tableOut)->Table)
	);
	return NO_ERROR;
}
...
DWORD TestDataProvider::getIpInterfaceEntry(PMIB_IPINTERFACE_ROW Row)
{
	...
	if (it != m_adapterTable.entries().end())
	{
		return ERROR_NOT_FOUND;
	}
