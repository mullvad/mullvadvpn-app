...
std::vector<char> CreateRawStringArray(const std::vector<std::string> &arr)
{
	//
	...
	if (arr.empty())
	{
		return std::vector<char>(sizeof(char *), '\x0');
	}
	...
	std::vector<char> buffer(bufferSize, '\x0');

	char **pointerTable = reinterpret_cast<char**>(&buffer[0]);
	char *data = (&buffer[0] + (sizeof(char*) * arr.size()));
