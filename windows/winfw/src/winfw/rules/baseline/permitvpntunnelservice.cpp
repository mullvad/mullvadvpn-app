...
bool PermitVpnTunnelService::apply(IObjectInstaller &objectInstaller)
{
    ...
    if (m_tunnelEndpoints.empty()) {
        filterBuilder
            .key(MullvadGuids::Filter_Baseline_PermitVpnTunnelService_Ipv4_Entry())
		...
		filterBuilder
			.key(MullvadGuids::Filter_Baseline_PermitVpnTunnelService_Ipv6_Entry())
        ...
        for (int i = 0; i < min; i++) {
            if (m_tunnelEndpoints[i].ip.type() == wfp::IpAddress::Ipv4)
            {
                GUID guid;
                if (i == 0) {
                    guid = MullvadGuids::Filter_Baseline_PermitVpnTunnelService_Ipv4_Entry();
                } else if (i == 1) {
                    guid = MullvadGuids::Filter_Baseline_PermitVpnTunnelService_Ipv4_Exit();
                } else {
                    return false;
                }
            ...
            if (m_tunnelEndpoints[i].ip.type() == wfp::IpAddress::Ipv6)
            {
                GUID guid;
                if (i == 0) {
                    guid = MullvadGuids::Filter_Baseline_PermitVpnTunnelService_Ipv6_Entry();
                } else if (i == 1) {
                    guid = MullvadGuids::Filter_Baseline_PermitVpnTunnelService_Ipv6_Exit();
                } else {
                    return false;
                }
