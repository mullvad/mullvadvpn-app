	...
	, m_activePolicy(Policy::None)
{
	...
	if (false == applyBlockedBaseConfiguration(settings, allowEndpoint, checkpoint))
	{
		THROW_ERROR("Failed to apply base configuration in BFE");
	}
...
)
{
	...
	if (allowEndpoint.has_value())
	{
		AppendAllowEndpointRules(ruleset, allowEndpoint.value());
	}
...
bool FwContext::applyPolicyBlocked(const WinFwSettings &settings, const std::optional<WinFwEndpoint> &allowEndpoint)
{
	const auto status = applyRuleset(composePolicyBlocked(settings, allowEndpoint));
...
FwContext::Ruleset FwContext::composePolicyBlocked(const WinFwSettings &settings, const std::optional<WinFwEndpoint> &allowEndpoint)
{
	Ruleset ruleset;
...
bool FwContext::applyBlockedBaseConfiguration(const WinFwSettings &settings, const std::optional<WinFwEndpoint> &allowEndpoint, uint32_t &checkpoint)
{
	return m_sessionController->executeTransaction([&](SessionController &controller, wfp::FilterEngine &engine)
	{
		...
		return applyRulesetDirectly(composePolicyBlocked(settings, allowEndpoint), controller);
	});
}
