...
void DeleteDevice(HDEVINFO devInfo, PSP_DEVINFO_DATA devInfoData)
{
...
void ForEachTapDevice(const std::wstring &tapHardwareId, std::function<void(HDEVINFO, PSP_DEVINFO_DATA)> func)
{
...
std::set<NetworkAdapter> GetTapAdapters(const std::wstring &tapHardwareId)
{
	...
	ForEachTapDevice(tapHardwareId, [&](HDEVINFO devInfo, PSP_DEVINFO_DATA devInfoData) {
		try
		{
			//
...
DeletionResult DeleteVanillaMullvadAdapter()
{
	...
	ForEachTapDevice(DEPRECATED_TAP_HARDWARE_ID, [&](HDEVINFO devInfo, PSP_DEVINFO_DATA devInfoData) {
		try
		{
