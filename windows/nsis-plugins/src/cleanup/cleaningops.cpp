...
void RemoveLogsCacheOtherUsers()
{
	...
	std::optional<std::pair<StringVectorConstIter, StringVectorConstIter>> relativeAppData;
	...
	{
		relativeAppData = std::make_optional(std::make_pair(std::next(roamingAppDataTokens.cbegin(), equalTokensCount), roamingAppDataTokens.cend()));
	}
	...
	while (files.next(file))
	{
		...
		{
			const auto userRoamingAppData = ConstructUserPath(files.getDirectory(), file.cFileName, relativeAppData.value());
