...
void UpdateRegistry(common::registry::RegistryKey &regkey, const std::wstring &valueName, const TrayJuggler &juggler)
{
	...
	auto windir = common::fs::GetKnownFolderPath(FOLDERID_Windows, 0, nullptr);
	auto explorer = std::experimental::filesystem::path(windir).append(L"explorer.exe");
	...
	auto pids = common::process::GetAllProcessIdsFromName(explorer);
