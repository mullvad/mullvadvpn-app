...
pub fn get_default_settings_dir() -> Result<PathBuf> {
    #[cfg(not(target_os = "android"))]
    {
        ...
        #[cfg(windows)]
        {
            #[cfg(not(feature = "deduce-system-service"))]
            {
                dir = dirs::data_local_dir().ok_or_else(|| crate::Error::FindDirError);
            }
            #[cfg(feature = "deduce-system-service")]
            {
                dir = system_service::get_system_service_local_app_data()
                    .map_err(crate::Error::FailedToFindSystemServiceDir);
            }
        }
...
#[cfg(all(feature = "deduce-system-service", windows))]
mod system_service {
    use std::{
        ffi::OsString,
        io, mem,
    ...
    use winapi::{
        ...
        um::{
            ...
            winnt::{
                SecurityImpersonation, HANDLE, LUID_AND_ATTRIBUTES, PROCESS_QUERY_INFORMATION,
                PWSTR, SE_PRIVILEGE_ENABLED, TOKEN_ADJUST_PRIVILEGES, TOKEN_DUPLICATE,
                TOKEN_IMPERSONATE, TOKEN_PRIVILEGES, TOKEN_QUERY, TOKEN_READ,
            },
        },
    };
    ...
    pub fn get_system_service_local_app_data() -> std::io::Result<PathBuf> {
        ...
        let local_app_data_dir: io::Result<PathBuf> = (|| {
            let mut lsass_path =
            ...
            let folder = get_known_folder_path(FOLDERID_LocalAppData, KF_FLAG_DEFAULT, lsass_token);
        ...
        if let Err(err) = adjust_current_thread_token_privilege(&system_debug_priv, false) {
            log::error!("Failed to drop system privileges: {}", err);
        }
    ...
    fn adjust_current_thread_token_privilege(
        ...
        } == FALSE
        {
            ...
            if let Some(1008) = thread_token_error.raw_os_error() {
        ...
        adjust_token_privilege(token_handle, privilege, enable)
    }
    ...
    fn get_running_process_id_from_name(target_name: &Path) -> io::Result<DWORD> {
        ...
        if unsafe { K32EnumProcesses(pid_buffer.as_mut_ptr(), bytes_available, &mut num_procs) }
            == FALSE
        {
            return Err(io::Error::last_os_error());
        }
        ...
        for process in pid_buffer {
            ...
            if process_handle.is_null() {
                log::error!("Failed to open process {}", process);
                continue;
            };


            let mut process_name = vec![0u16; 512];
            let mut process_name_size = (process_name.len() * mem::size_of::<DWORD>()) as u32;
