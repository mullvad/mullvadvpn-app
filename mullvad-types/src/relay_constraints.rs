...
pub mod allowed_ip {
    ...
    impl AllowedIps {
        ...
        /// Constructs a `Constraint<AllowedIps>` from an iterator of string representations of IP networks.
        ///
        /// Each string should be a valid CIDR notation (e.g., "192.168.1.0/24").
        /// Ignores empty strings. Returns `Constraint::Any` if there are no non-empty elements.
        /// Returns an error if any string is not a valid network or if it contains non-zero host bits.
        ///
        /// # Errors
        ///
        ...
        pub fn from_strings(
            allowed_ips: &[String],
        ) -> Result<Constraint<AllowedIps>, AllowedIpParseError> {
            ...
            if result.is_empty() {
                ...
                Ok(Constraint::Only(AllowedIps(result)))
            }
        }
