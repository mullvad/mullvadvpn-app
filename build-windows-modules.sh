...
function usage {
    echo "usage: $0 [clean|build] [solution...]"
    exit 1
}
...
function build_solution_config {
    ...
    local build_command="msbuild.exe /p:MultiProcessorCompilation=true"
    build_command+=" $(to_win_path "$sln") /p:Configuration=$config /p:Platform=$platform"
    ...
    build_command+=" /p:AdditionalOptions=/Zm400"
    # msbuild picks the host architecture of the compiler from its own
    # process, so an x86 msbuild selects the 32-bit x86 compiler, which runs
    # emulated on an ARM64 host. Ask for the native compiler explicitly.
    build_command+=" /p:PreferredToolArchitecture=${HOST_TARGET,,}"

    set -x
    cmd.exe "/c $build_command"
    set +x
}
