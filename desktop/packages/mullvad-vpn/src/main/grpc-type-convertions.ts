...
export function convertFromAppUpgradeEvent(data: grpcTypes.AppUpgradeEvent): DaemonAppUpgradeEvent {
  ...
  if (downloadStartingData !== undefined) {
    return { type: 'APP_UPGRADE_EVENT_STATUS_DOWNLOAD_STARTED' };
  }
  ...
  if (downloadProgressData !== undefined) {
    ...
    return { type: 'APP_UPGRADE_EVENT_STATUS_DOWNLOAD_PROGRESS', server, progress, timeLeft };
  }

  if (data.hasUpgradeAborted()) {
    return { type: 'APP_UPGRADE_EVENT_STATUS_ABORTED' };
  }

  if (data.hasVerifyingInstaller()) {
    return { type: 'APP_UPGRADE_EVENT_STATUS_VERIFYING_INSTALLER' };
  }

  if (data.hasVerifiedInstaller()) {
    return { type: 'APP_UPGRADE_EVENT_STATUS_VERIFIED_INSTALLER' };
  }
