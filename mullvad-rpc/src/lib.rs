...
impl MullvadRpcRuntime {
    ...
    fn new_connection_on_event_loop(
        &mut self,
    ...
    pub fn mullvad_rest_handle(&mut self) -> rest::MullvadRestHandle {
        let service = self.new_connection_on_event_loop(Some(API_HOST.to_owned()));
    ...
    pub fn rest_handle(&mut self) -> rest::RequestServiceHandle {
        self.new_connection_on_event_loop(None)
    }
}

impl Drop for MullvadRpcRuntime {
    fn drop(&mut self) {
        let old_runtime = std::mem::replace(&mut self.runtime, event_loop::create_runtime());
...
impl AccountsProxy {
    ...
    pub fn get_expiry(
        ...
        self.handle.service.compat_spawn(async move {
            let account: AccountResponse = rest::desereialize_body(response.await?).await?;
            Ok(account.expires)
        })
    }
    ...
    pub fn submit_voucher(
        ...
        self.handle
            .service
            .compat_spawn(async move { rest::desereialize_body(response.await?).await })
    }
    ...
    pub fn get_www_auth_token(
        ...
        let future = async move {
            let response: AuthTokenResponse = rest::desereialize_body(response.await?).await?;
...
impl AppVersionProxy {
    ...
    ) -> impl futures01::future::Future<Item = mullvad_types::version::AppVersionInfo, Error = rest::Error>
    {
        ...
        self.handle
            .service
            .compat_spawn(async move { rest::desereialize_body(request.await?).await })
    }
}
