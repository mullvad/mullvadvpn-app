...
impl AddressCache {
    ...
    pub fn with_cache(cache_path: Box<Path>) -> Self {
        let cache = AddressCacheInner::from_cache_file(&cache_path).unwrap_or_default();
...
impl AddressCacheInner {
    fn from_cache_file(path: &Path) -> io::Result<Self> {
