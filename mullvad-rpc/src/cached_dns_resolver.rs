...
impl CachedDnsResolver<SystemDnsResolver> {
    pub fn new(hostname: String, cache_dir: &Path, filename: &str) -> Self {
        Self::with_dns_resolver(SystemDnsResolver, hostname, cache_dir, filename)
    }
}
...
impl<R: DnsResolver> CachedDnsResolver<R> {
    ...
    fn load_from_file(file: &Path) -> io::Result<IpAddr> {
...
mod tests {
    ...
    fn resolves_even_if_impossible_to_store_in_cache() {
        ...
        assert_eq!(cache.resolve().unwrap(), mock_resolver.address());
    }
    ...
    fn create_cached_dns_resolver<'a>(
        ...
        CachedDnsResolver::with_dns_resolver(mock_resolver, hostname, cache_dir, filename)
    }
