...
>(
    ...
    // Move event_handler on heap turning it into trait object which is 128-bit long.
    let heap_event_handler = Box::new(event_handler) as Box<HandlerFn>;

    // Box trait object to turn it into regular boxed pointer (i.e 64-bit long)
...
extern "system" fn service_control_handler(
    ...
    // Important: cast context to Box<Box<HandlerFn>>> but do not take ownership.
