...
use std::path::Path;
use std::{fmt::{self, Display, Formatter}, fs, ops::{Deref, DerefMut}};
...
impl TryFrom<&Path> for StringResources {
    ...
    fn try_from(value: &Path) -> Result<Self, Self::Error> {
        ...
        let strings = fs::read_to_string(value).map_err(|e| format!("Failed to read string resources file: {e}"))?;
...
fn pre_process_strings(original: &str) -> String {
    ...
    let result = re.replace_all(original, |caps: &Captures<'_>| {
        ...
        if text.contains("<![CDATA") {
            ...
            entry.replace(text, &escaped).to_string()
        }
    });
...
#[cfg(test)]
mod tests {
    use super::{pre_process_strings, StringResource, StringResources, StringValue};
