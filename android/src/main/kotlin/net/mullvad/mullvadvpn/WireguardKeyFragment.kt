...
class WireguardKeyFragment : Fragment() {
    ...
    private var updateConnectionStatusJob: Job? = null;
    private var updateKeyStatusJob: Job? = null;
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_wireguard_key, container, false)

        view.findViewById<ImageButton>(R.id.close_wg_keys).setOnClickListener {
            parentActivity.onBackPressed()
        }


        statusMessage = view.findViewById<TextView>(R.id.wireguard_key_status_tv)
        ...
        connectionProxy.onUiStateChange = { _ ->
            updateConnectionStatusJob?.cancel()
            updateConnectionStatusJob = updateViewJob()
        }

        keyStatusListener.onKeyStatusChange = { _ ->
            updateKeyStatusJob?.cancel()
            updateKeyStatusJob = updateViewJob()
        }
    ...
    private fun onGenerateKeyPress() {
        ...
        currentJob = GlobalScope.launch(Dispatchers.Main) {
            currentJob = keyStatusListener.generateKey()
            currentJob?.join()
    ...
    private fun onValidateKeyPress() {
        ...
        generatingKey = false;
        currentJob = GlobalScope.launch(Dispatchers.Main) {
            currentJob = keyStatusListener.verifyKey()
            currentJob?.join()
            validatingKey = false;
