...
val ANTICIPATED_STATE_TIMEOUT = 1500L
...
class ConnectionProxy(val context: Context, val daemon: Deferred<MullvadDaemon>) {
    ...
    private fun anticipateConnectingState(): Boolean {
        synchronized(this) {
            ...
            if (currentState is TunnelState.Connecting || currentState is TunnelState.Connected) {
                ...
                resetAnticipatedStateJob = resetAnticipatedState()
    ...
    private fun resetAnticipatedState() = GlobalScope.launch(Dispatchers.Main) {
        delay(ANTICIPATED_STATE_TIMEOUT)
        uiState = state
    }
