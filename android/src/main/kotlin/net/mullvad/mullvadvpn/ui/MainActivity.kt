...
class MainActivity : FragmentActivity() {
    ...
    private val serviceConnectionManager = object : android.content.ServiceConnection {
        override fun onServiceConnected(className: ComponentName, binder: IBinder) {
            ...
            serviceConnectionSubscription = localBinder.connectionNotifier.subscribe { connection ->
                serviceConnection?.onDestroy()

                serviceConnection = connection?.let { connection ->
                    ServiceConnection(connection, this@MainActivity)
                }
            }
        ...
        override fun onServiceDisconnected(className: ComponentName) {
            ...
            serviceConnectionSubscription?.let { subscription ->
                runBlocking {
                    service.await().connectionNotifier.unsubscribe(subscription)
                }
