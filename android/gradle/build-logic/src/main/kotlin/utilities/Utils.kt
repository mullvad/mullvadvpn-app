...
fun printBuildHeader(versionName: String) {
    ...
    println("Building Mullvad VPN $versionName on $hostname")
}
...
fun Project.registerReleaseTask(
    ...
    skipClean: Boolean = getBooleanPropertyOrNull("mullvad.app.release.skipClean") ?: false,
    skipDirtyCheck: Boolean =
        getBooleanPropertyOrNull("mullvad.app.release.skipDirtyCheck") ?: false,
) {
    val releaseVersionName = appVersion.name
    val releaseDistDirPath = rootDir.parentFile.resolve("dist").absolutePath
    val releaseSkipDirtyCheck = skipDirtyCheck
    ...
    val postBuild =
        tasks.register<PostBuildTask>("${releaseName}PostBuild") {
            this.skipDirtyCheck.set(releaseSkipDirtyCheck)
