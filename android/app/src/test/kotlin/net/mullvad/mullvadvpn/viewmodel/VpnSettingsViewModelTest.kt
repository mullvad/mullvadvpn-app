...
class VpnSettingsViewModelTest {
    ...
    fun `onWireguardPortSelected should invoke updateSelectedWireguardConstraint with Constraint Only with same port`() =
        runTest {
            ...
            coEvery { mockWireguardConstraintsRepository.updateWireguardPort(any()) } returns
                Unit.right()
            ...
            // Assert
            coVerify(exactly = 1) {
                mockWireguardConstraintsRepository.updateWireguardPort(wireguardConstraints.port)
            }
        }
