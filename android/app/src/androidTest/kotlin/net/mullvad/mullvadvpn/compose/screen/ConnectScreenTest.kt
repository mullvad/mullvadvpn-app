...
class ConnectScreenTest {
    ...
    fun showLocationInfo() {
        ...
        val mockTunnelEndpoint: TunnelEndpoint = mockk(relaxed = true)
        val mockEndpoint: Endpoint = mockk(relaxed = true)
        val mockInetSocketAddress: InetSocketAddress = mockk(relaxed = true)
        val mockHostName = "Host-Name"
        val mockPort = 99
        val mockHost = "Host"
        val mockProtocol = TransportProtocol.Udp
        val mockHostAddressV4 = "HostAddressV4"
        val mockHostAddressV6 = "HostAddressV6"
        every { mockTunnelEndpoint.obfuscation } returns null
        every { mockLocation.hostname } returns mockHostName
        every { mockTunnelEndpoint.endpoint } returns mockEndpoint
        every { mockEndpoint.address } returns mockInetSocketAddress
        every { mockInetSocketAddress.port } returns mockPort
        every { mockInetSocketAddress.address.hostAddress } returns mockHost
        every { mockEndpoint.protocol } returns mockProtocol
        every { mockLocation.ipv4?.hostAddress } returns mockHostAddressV4
        every { mockLocation.ipv6?.hostAddress } returns mockHostAddressV6
        composeTestRule.setContent {
            ConnectScreen(
                uiState =
                    ConnectUiState(
        ...
        composeTestRule.apply {
            ...
            onNodeWithText("Out $mockHostAddressV4 / $mockHostAddressV6").assertExists()
        }
    }
}
