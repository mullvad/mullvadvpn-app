...
class ConnectScreenTest {
    ...
    fun testOnShowAccountClick() {
        ...
        composeTestRule.setContent {
            ConnectScreen(
                onShowAccountClick = mockedClickHandler,
                uiState =
                    ConnectUiState(
                        location = null,
                        relayLocation = null,
    ...
    fun testOpenAccountView() {
        ...
        composeTestRule.setContent {
            ConnectScreen(
                uiState = ConnectUiState.INITIAL,
                viewActions = MutableStateFlow(ConnectViewModel.ViewAction.OpenAccountView("222"))
            )
        }

        // Assert
        composeTestRule.apply {
            onNodeWithText("UNSECURED CONNECTION").assertDoesNotExist()
            onNodeWithText("Secure my connection").assertDoesNotExist()
        }
    }
}
