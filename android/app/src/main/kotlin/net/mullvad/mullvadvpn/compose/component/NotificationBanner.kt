...
fun PreviewNotificationBanner() {
    AppTheme {
        SpacedColumn(Modifier.background(color = MaterialTheme.colorScheme.background)) {
            ...
            NotificationBanner(
                data = accountExpiryNotification(expiry = DateTime.now(), onClickShowAccount = {})
            )
            NotificationBanner(data = genericBlockingMessage())
        }
    }
}
...
private fun ShowNotification(
    ...
    val data: NotificationBannerData? =
    ...
    data?.let { NotificationBanner(data = data) }
}

@Composable
private fun NotificationBanner(data: NotificationBannerData) {
    ConstraintLayout(
        modifier =
            Modifier.fillMaxWidth()
                ...
                .then(data.onClick?.let { Modifier.clickable(onClick = data.onClick) } ?: Modifier)
                .animateContentSize()
    ) {
        ...
        Box(
            modifier =
                Modifier.background(
                        color =
                            if (data.statusLevel == StatusLevel.Warning) {
