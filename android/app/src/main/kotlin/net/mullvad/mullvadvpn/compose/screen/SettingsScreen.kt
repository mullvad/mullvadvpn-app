...
private fun PreviewSettings() {
    SettingsScreen(uiState = SettingsUiState(true, DateTime.now(), "2023.03", true))
}
                        ...
                        bodyView =
                            @Composable {
                                NavigationCellBody(
                                    title = stringResource(id = R.string.settings_account),
                                    content =
                                        TimeLeftFormatter(LocalContext.current.resources)
                                            .format(uiState.accountExpiry ?: DateTime.now())
                                )
                            },
