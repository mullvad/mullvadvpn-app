...
open class SendEmail : ActivityResultContract<EmailData, Void?>() {
    ...
    override fun createIntent(context: Context, input: EmailData): Intent {
        val intent = if(input.attachment != null) {
            emailIntentWithoutAttachment().also {
    ...
    /**
     * Create email intent with attachment
     * Due to a limitation with attachments we can only use ACTION_SEND i.e. a generic share intent
     * This causes the list of apps to choose from to be larger than just email apps
     */
    private fun emailIntentWithAttachment(
    ) : Intent {
    ...
    /**
     * Create email intent without attachment
     * This uses the standard email intent ACTION_SENDTO with a mailto: Uri
     * This limits the list of apps to choose from to email apps only
     */
    private fun emailIntentWithoutAttachment(
    ) : Intent {
