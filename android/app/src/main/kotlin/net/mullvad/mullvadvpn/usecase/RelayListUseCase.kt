...
class RelayListUseCase(
    ...
    fun relayListWithSelection(): Flow<Pair<List<RelayCountry>, RelayItem?>> =
        combine(relayListListener.relayListEvents, settingsRepository.settingsUpdates) {
            ...
            Pair(relayCountries, selectedItem)
        }
    ...
    private fun findSelectedRelayItem(
        ...
        return location?.let {
            relayCountries?.findItemForLocation(location.toGeographicLocationConstraint())
        }
    }
