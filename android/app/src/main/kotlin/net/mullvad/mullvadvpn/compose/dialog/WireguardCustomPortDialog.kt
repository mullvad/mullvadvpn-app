...
fun WireguardCustomPortDialog(
    ...
    val port = remember { mutableStateOf(navArg.customPort?.toString() ?: "") }
    ...
    AlertDialog(
        ...
        confirmButton = {
            Column(verticalArrangement = Arrangement.spacedBy(Dimens.buttonSpacing)) {
                PrimaryButton(
                    ...
                    onClick = { backNavigator.navigateBack(port.value.toInt()) },
                    isEnabled =
                        port.value.isNotEmpty() &&
                            navArg.allowedPortRanges.isPortInValidRanges(
                                port.value.toIntOrNull() ?: 0
                            )
                )
                if (navArg.customPort != null) {
                    NegativeButton(
                        text = stringResource(R.string.custom_port_dialog_remove),
                        onClick = { backNavigator.navigateBack(null) }
                    )
                }
                PrimaryButton(
                    text = stringResource(id = R.string.cancel),
                    onClick = backNavigator::navigateBack
                )
            }
        },
        text = {
                ...
                CustomPortTextField(
                    ...
                    onSubmit = { input ->
                        if (
                            ...
                            backNavigator.navigateBack(result = input.toIntOrNull())
                        }
                    },
                ...
                Text(
                    text =
                        stringResource(
                            ...
                            navArg.allowedPortRanges.asString()
                        ),
        ...
        onDismissRequest = backNavigator::navigateBack
    )
}
