...
class LoginFragment : BaseFragment(), NavigationBarPainter {
    ...
    override fun onStart() {
        ...
        input.onTextChanged.subscribe(this) { ـ ->
            val isInputEmpty = input.input.text.toString().trim().isEmpty()
            createAccountButton.isEnabled = isInputEmpty
            createAccountButton.alpha = if (isInputEmpty) 1f else 0.5f
        }
    }
