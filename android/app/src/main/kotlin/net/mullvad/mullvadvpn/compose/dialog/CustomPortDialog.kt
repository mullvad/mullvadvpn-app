...
import net.mullvad.mullvadvpn.util.portValid
...
fun PreviewCustomPortDialog() {
    AppTheme {
        CustomPortDialog(
            ...
            onReset = {},
            customPort = "",
            allowedRange = listOf(PortRange(10, 10), PortRange(40, 50)),
            showReset = true,
            onDismissRequest = {}
        )
    }
        ...
        confirmButton = {
            Column {
                ActionButton(
                    ...
                    isEnabled =
                        port.value.isNotEmpty() && allowedRange.portValid(port.value.toInt())
                )
                if (showReset) {
                    ActionButton(
