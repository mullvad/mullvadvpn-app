...
The Mullvad Android app is by default reproducible when built using the Docker container, as the container ensures a consistent build environment with fixed versions of all tools and dependencies.
...
- Use the exact same versions of all build dependencies as specified in the [root Dockerfile](../building/Dockerfile) and [Android Dockerfile](docker/Dockerfile).
This includes for example the Go version and Android SDK and NDK versions.
- Make sure that the `local.properties` file does not change keys that affect the reproducibility of the build such as `CARGO_TARGETS` and `ENABLE_IN_APP_VERSION_NOTIFICATIONS`.

### Verifying that a build is reproducible

A simple way to check that a build is reproducible is to build the `fdroid` version of the app with and without the container and comparing the checksums of the produced APKs.
...
If the checksums are not equal this means the build is not reproducible. This could be because of either:
