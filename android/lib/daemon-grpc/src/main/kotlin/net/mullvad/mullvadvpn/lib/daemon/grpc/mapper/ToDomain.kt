...
private fun TunnelState.Disconnected.toDomain(): ModelTunnelState.Disconnected =
    ModelTunnelState.Disconnected(
        location = disconnected_location?.toDomain()
    )
...
internal fun ObfuscationEndpoint.ObfuscationType.toDomain(): ModelObfuscationType =
    when (this) {
        ...
        ObfuscationEndpoint.ObfuscationType.SHADOWSOCKS ->
            ModelObfuscationType.Shadowsocks
        ...
        cause =
            when (cause) {
                ...
                ErrorState.Cause.IPV6_UNAVAILABLE ->
                    ModelErrorStateCause.Ipv6Unavailable
                ...
                ErrorState.Cause.START_TUNNEL_ERROR ->
                    ModelErrorStateCause.StartTunnelError
                ...
                ErrorState.Cause.SPLIT_TUNNEL_ERROR ->
                    ModelErrorStateCause.StartTunnelError
...
private fun ErrorState.AuthFailedError.toDomain(): ModelAuthFailedError =
    when (this) {
        ...
        ErrorState.AuthFailedError.INVALID_ACCOUNT ->
            ModelAuthFailedError.InvalidAccount
        ErrorState.AuthFailedError.EXPIRED_ACCOUNT ->
            ModelAuthFailedError.ExpiredAccount
        ErrorState.AuthFailedError.TOO_MANY_CONNECTIONS ->
            ModelAuthFailedError.TooManyConnections
    }

internal fun ErrorState.FirewallPolicyError.toDomain():
    ModelErrorStateCause.FirewallPolicyError =
    when (type) {
...
internal fun LocationConstraint.toDomain(): ModelConstraint<ModelRelayItemId> =
    when {
        custom_list != null ->
            ModelConstraint.Only(ModelCustomListId(custom_list))
        location != null ->
            ModelConstraint.Only(location.toDomain())
...
internal fun List<String>.toDomain(): ModelConstraint<ModelProviders> =
    if (isEmpty()) ModelConstraint.Any else ModelConstraint.Only(map { ModelProviderId(it) }.toSet())

internal fun WireguardConstraints.toDomain(): ModelWireguardConstraints =
    ModelWireguardConstraints(
        port =
            if (port != null) {
                ModelConstraint.Only(ModelPort(port))
            } else {
                ModelConstraint.Any
            },
...
internal fun ObfuscationSettings.SelectedObfuscation.toDomain():
    ModelObfuscationMode =
    when (this) {
        ...
        ObfuscationSettings.SelectedObfuscation.UDP2TCP ->
            ModelObfuscationMode.Udp2Tcp
        ObfuscationSettings.SelectedObfuscation.SHADOWSOCKS ->
            ModelObfuscationMode.Shadowsocks
        ...
        ObfuscationSettings.SelectedObfuscation.PORT ->
            throw IllegalArgumentException("PORT is not a support obfuscation")
    }

internal fun Udp2TcpObfuscationSettings.toDomain(): ModelUdp2TcpObfuscationSettings =
    if (port != null) {
        ModelUdp2TcpObfuscationSettings(ModelConstraint.Only(ModelPort(port)))
    } else {
        ModelUdp2TcpObfuscationSettings(ModelConstraint.Any)
    }

internal fun ShadowsocksSettings.toDomain(): ModelShadowsocksSettings =
    if (port != null) {
        ModelShadowsocksSettings(ModelConstraint.Only(ModelPort(port)))
    } else {
        ModelShadowsocksSettings(ModelConstraint.Any)
    }
...
internal fun AppVersionInfo.toDomain(): ModelAppVersionInfo =
    ModelAppVersionInfo(
        supported = supported,
        suggestedUpgrade = suggested_upgrade?.version,
    )
...
internal fun RelayList.toDomain(): ModelRelayList =
    ModelRelayList(countries.toDomain(), wireguard!!.toDomain())
...
/**
 * Convert from a list of RelayListCountry to a model.RelayList. Non-wireguard
 ...
 */
internal fun List<RelayListCountry>.toDomain():
    List<ModelRelayItem.Location.Country> =
    map(RelayListCountry::toDomain)
...
internal fun RelayListCity.toDomain(
    ...
    return ModelRelayItem.Location.City(
        ...
        relays =
            relays
                .filter { it.endpoint_data?.wireguard != null }
...
internal fun Relay.toDomain(
    cityCode: ModelGeoLocationId.City
): ModelRelayItem.Location.Relay =
    ModelRelayItem.Location.Relay(
        id = ModelGeoLocationId.Hostname(cityCode, hostname),
        active = active,
        provider = ModelProviderId(provider),
        ownership = if (owned) ModelOwnership.MullvadOwned else ModelOwnership.Rented,
        daita = endpoint_data?.wireguard?.daita == true,
        quic =
            if (endpoint_data?.wireguard?.quic != null) {
                endpoint_data.wireguard.quic.toDomain()
            } else {
                null
            },
        lwo = endpoint_data?.wireguard?.lwo == true,
    )

private fun Relay.RelayData.Wireguard.Quic.toDomain(): ModelQuic =
...
internal fun DeviceState.toDomain(): ModelDeviceState =
    when (state) {
        DeviceState.State.LOGGED_IN if device != null && device.device != null ->
            ModelDeviceState.LoggedIn(ModelAccountNumber(device.account_number), device.device.toDomain())
...
internal fun ApiAccessMethodSettings.toDomain(): List<ModelApiAccessMethodSetting> =
    buildList {
        if(direct != null) {
            add(direct.toDomain())
        }
...
internal fun FeatureIndicator.toDomain(): ModelFeatureIndicator =
    when (this) {
        FeatureIndicator.QUANTUM_RESISTANCE ->
            ModelFeatureIndicator.QUANTUM_RESISTANCE
        ...
        FeatureIndicator.LOCKDOWN_MODE,
