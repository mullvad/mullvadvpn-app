...
class RelayListFilterRepositoryTest {
    ...
    fun `when successfully updating selected ownership should return successful`() = runTest {
        ...
        coEvery { mockManagementService.setOwnership(ownership, RelayHopType.EXIT) } returns
            Unit.right()

        // Act
        val result = relayListFilterRepository.updateSelectedOwnership(ownership, RelayHopType.EXIT)

        // Assert
        coVerify { mockManagementService.setOwnership(ownership, RelayHopType.EXIT) }
        assertEquals(Unit.right(), result)
    }
    ...
    fun `when failing to update selected ownership should return SetWireguardConstraintsError`() =
        runTest {
            ...
            // Act
            val result =
                relayListFilterRepository.updateSelectedOwnership(ownership, RelayHopType.EXIT)
    ...
    fun `when successfully updating selected providers should return successful`() = runTest {
        ...
        coEvery { mockManagementService.setProviders(providers, RelayHopType.EXIT) } returns
            Unit.right()

        // Act
        val result = relayListFilterRepository.updateSelectedProviders(providers, RelayHopType.EXIT)

        // Assert
        coVerify { mockManagementService.setProviders(providers, RelayHopType.EXIT) }
        assertEquals(Unit.right(), result)
    }
    ...
    fun `when failing to update selected providers should return SetWireguardConstraintsError`() =
        runTest {
            ...
            // Act
            val result =
                relayListFilterRepository.updateSelectedProviders(providers, RelayHopType.EXIT)
