...
internal fun WireguardConstraints.toDomain(): ModelWireguardConstraints =
    ModelWireguardConstraints(
        isMultihopEnabled = multihop == WireguardConstraints.Multihop.Always,
        entryLocation = entry_location?.toDomain() ?: Constraint.Any,
        ipVersion =
            if (ip_version != null) {
...
internal fun ObfuscationSettings.Lwo.toDomain(): LwoObfuscationSettings =
    if (port != null) {
...
internal fun DaitaSettings.toDomain(): ModelDaitaSettings =
    ModelDaitaSettings(enabled = enabled, directOnly = false)
...
private fun Location.toDomain(): LatLong =
...
internal fun FeatureIndicator.toDomain() =
    when (this) {
        ...
        FeatureIndicator.MULTIHOP_AUTO -> ModelFeatureIndicator.MULTIHOP
...
internal fun RelayPartitions.toDomain() =
    ModelRelayPartitions(
        matches = matches.map { it.relay!!.hostname },
