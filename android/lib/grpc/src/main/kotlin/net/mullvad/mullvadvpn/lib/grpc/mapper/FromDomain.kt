...
internal fun ModelWireguardConstraints.fromDomain(): WireguardConstraints =
    WireguardConstraints(
        multihop =
            if (isMultihopEnabled) WireguardConstraints.Multihop.Always
            else WireguardConstraints.Multihop.Never,
...
internal fun ApiAccessMethodId.fromDomain(): UUID = UUID(value_ = value.toString())
...
internal fun ModelExitConstraints.fromDomain(): ExitConstraints =
    ExitConstraints(
        ...
        ownership = ownership.getOrNull()?.fromDomain()!!,
    )
