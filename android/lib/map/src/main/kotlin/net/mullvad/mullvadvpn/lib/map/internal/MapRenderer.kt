...
import net.mullvad.mullvadvpn.lib.map.data.rotateAroundX
...
internal class MapRenderer(private val resources: Resources) : GLSurfaceView.Renderer {
    ...
    // Hack around needing lateinit while also being have custom setter.
    private lateinit var _viewState: GlobeViewState
    internal var viewState: GlobeViewState
        get() = _viewState
        set(value) {
            _viewState = value
            markerVector = value.markers.associateBy { it.latLong.toVector3() }
        }
    ...
    fun calculateIntersection(
        ...
        if (!tapOffset.isSpecified || viewPortWidth <= 0 || viewPortHeight <= 0) return null
        ...
        return if (discriminant < 0f) {
            ...
            // Position in the sphere where intersection is
            point2
