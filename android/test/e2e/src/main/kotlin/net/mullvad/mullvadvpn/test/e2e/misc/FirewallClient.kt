...
class FirewallClient(private val httpClient: HttpClient = defaultHttpClient()) {
    suspend fun createRule(rule: FirewallDropRule) {
        Logger.v(
            "Sending create rule request with body: ${Json.encodeToString(FirewallDropRule.serializer(), rule)}"
        )
        Logger.v(
        ...
        httpClient.post("rule") {
            ...
            setBody(Json.encodeToString(FirewallDropRule.serializer(), rule))
        }
    }
