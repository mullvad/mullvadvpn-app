...
class LeakTest : EndToEndTest(BuildConfig.FLAVOR_infrastructure) {
    ...
    fun testNegativeLeak() =
        runBlocking<Unit> {
            ...
            val streamList = splitStreamList(capturedStreams, Interval(connectTime, disconnectTime))
            val leakRules = listOf(NoTrafficToHostRule(targetIpAddress))
            LeakCheck.assertNoLeaks(streamList, leakRules)
        }
    ...
    fun testShouldHaveNegativeLeak() =
        runBlocking<Unit> {
            ...
            val streamList = splitStreamList(capturedStreams, Interval(connectTime, disconnectTime))
            val leakRules = listOf(NoTrafficToHostRule(targetIpAddress))
            LeakCheck.assertLeaks(streamList, leakRules)
        }
