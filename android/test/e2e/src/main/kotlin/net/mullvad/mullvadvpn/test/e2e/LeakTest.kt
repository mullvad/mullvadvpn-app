...
import net.mullvad.mullvadvpn.compose.test.SECURE_CONNECTION_TEST_TAG
...
class LeakTest : EndToEndTest(BuildConfig.FLAVOR_infrastructure) {
    ...
    fun testNegativeLeak() =
        runBlocking<Unit> {
            Logger.v("Running test testNegativeLeak")
            ...
            device.findObjectWithTimeout(By.text(SECURE_CONNECTION_TEST_TAG), CONNECTION_TIMEOUT)
            ...
            // Attachment.saveAttachment("capture.pcap", "should-leak", capturedPcap)
    ...
    fun testShouldHaveNegativeLeak() =
        runBlocking<Unit> {
            Logger.v("Running test testShouldHaveNegativeLeak")
            ...
            // Attachment.saveAttachment("capture.pcap", "should-leak", pcapContent.toByteArray())
