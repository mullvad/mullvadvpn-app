...
import net.mullvad.mullvadvpn.test.e2e.misc.FirewallClient
...
class ConnectionTest : EndToEndTest(BuildConfig.FLAVOR_infrastructure) {
    ...
    fun testWireGuardObfuscationOff() =
        runBlocking {
            ...
            scrollView.scrollUntil(Direction.DOWN, Until.hasObject(By.res(WIREGUARD_OBFUSCATION_OFF_CELL_TEST_TAG)))
            device.findObjectWithTimeout(By.res(WIREGUARD_OBFUSCATION_OFF_CELL_TEST_TAG)).click()
            device.pressBack()
            device.pressBack()

            // Block UDP traffic to the relay
            val firewallRule = FirewallClient.FirewallRule.blockUDPTrafficRule(relayIpAddress)
            val firewallClient = FirewallClient()
            firewallClient.createRule(firewallRule)

            // Ensure it is not possible to connect to relay
            device.findObjectWithTimeout(By.text("Connect")).click()
            delay(UNSUCCESSFUL_CONNECTION_TIMEOUT.milliseconds) // Give it some time and then verify still unable to connect. This duration must be long enough to ensure all retry attempts have been made.
    ...
    fun testUDPOverTCP() =
        runBlocking<Unit> {
            ...
            scrollView2.scrollUntil(Direction.DOWN, Until.hasObject(By.res(WIREGUARD_OBFUSCATION_UDP_OVER_TCP_CELL_TEST_TAG)))
            device
