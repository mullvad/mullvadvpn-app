...
class ConnectionTest : EndToEndTest(BuildConfig.FLAVOR_infrastructure) {
    ...
    fun testUDPOverTCP() =
        runBlocking<Unit> {
            ...
            device.findObjectWithTimeout(By.text("Connect")).click()
            delay(5000.milliseconds) // Give it some time and then verify still unable to connect
            ...
            scrollView2.swipe(Direction.UP, 0.1f)
            device
            ...
            // Ensure it is now possible to connect by using UDP-over-TCP
            device.findObjectWithTimeout(By.text("Connect")).click()
            device.findObjectWithTimeout(By.text("CONNECTED"), VERY_LONG_TIMEOUT)
            device.findObjectWithTimeout(By.text("Disconnect")).click()
        }
    ...
    companion object {
        const val SETTINGS_SCROLL_VIEW_TEST_TAG = "lazy_list_test_tag"
