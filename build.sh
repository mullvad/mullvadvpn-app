...
function build() {
    ...
    local forTargetString
    forTargetString=$([[ -n $current_target ]] && echo " for $current_target")

    echo "Building Mullvad VPN $PRODUCT_VERSION$forTargetString"
    ...
    echo "Building Rust code in release mode using $RUSTC_VERSION$forTargetString..."
    ...
    for binary in ${BINARIES[*]}; do
        ...
        if [[ -n ${TARGET:-""} && "${#TARGET[*]}" -gt 1 ]]; then
            DST_DIR="$SCRIPT_DIR/dist-assets/$current_target"
            DST="$DST_DIR/$binary"
            mkdir -p "$DST_DIR"
        else
...
################################################################################
# Sort binaries by target for Electron builder
################################################################################

if [[ "${MACOS_UNIVERSAL:-""}" == 1 ]]; then
    rm -rf "$SCRIPT_DIR/dist-assets/x64"
    rm -rf "$SCRIPT_DIR/dist-assets/arm64"
    mv "$SCRIPT_DIR/dist-assets/x86_64-apple-darwin" "$SCRIPT_DIR/dist-assets/x64"
    mv "$SCRIPT_DIR/dist-assets/aarch64-apple-darwin" "$SCRIPT_DIR/dist-assets/arm64"
    cp "$SCRIPT_DIR/dist-assets/binaries/x86_64-apple-darwin/openvpn" "$SCRIPT_DIR/dist-assets/x64"
    cp "$SCRIPT_DIR/dist-assets/binaries/x86_64-apple-darwin/sslocal" "$SCRIPT_DIR/dist-assets/x64"
    cp "$SCRIPT_DIR/dist-assets/binaries/aarch64-apple-darwin/openvpn" "$SCRIPT_DIR/dist-assets/arm64"
    cp "$SCRIPT_DIR/dist-assets/binaries/aarch64-apple-darwin/sslocal" "$SCRIPT_DIR/dist-assets/arm64"
fi
