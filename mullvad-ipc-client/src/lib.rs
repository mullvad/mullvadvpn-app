...
error_chain! {
    errors {
        ...
        OpenRpcFileError(file_path: String) {
            description("Failed to open RPC information file")
            display("Failed to open RPC information file {}", file_path)
        }
...
impl DaemonRpcClient {
    ...
    fn read_rpc_file() -> Result<String> {
        ...
        let rpc_file = File::open(&file_path)
            .chain_err(|| ErrorKind::OpenRpcFileError(file_path.display().to_string()))?;
        let reader = BufReader::new(rpc_file);
        let mut lines = reader.lines();

        lines
            .next()
            .ok_or_else(|| ErrorKind::EmptyRpcFile(file_path.display().to_string()))?
            .chain_err(|| ErrorKind::ReadRpcFileError(file_path.display().to_string()))
    }
...
pub fn rpc_file_path() -> Result<PathBuf> {
    let windows_directory =
        ::std::env::var_os("WINDIR").ok_or_else(|| ErrorKind::UnknownRpcFilePath.into());
