...
impl DaemonRpcClient {
    ...
    {
        let rpc_client = if let Some(ref mut rpc_client) = self.rpc_client {
            rpc_client
        } else {
            self.connect_to_rpc_server()?
        };
    ...
    fn connect_to_rpc_server(&mut self) -> Result<&mut WsIpcClient> {
