...
>(
    ...
    thread::spawn(move || match spawn_transport(rpc_path, transport_func) {
        ...
        Ok((client, server_handle, client_handle)) => {
            ...
            // rt.handle().spawn(client.map_err(|e| {
    ...
    rx.wait().chain_err(|| ErrorKind::TransportError)?.map(
        |(rpc_client, server_handle, executor)| DaemonRpcClient {
            rpc_client: rpc_client.clone(),
            subscriber: jsonrpc_client_pubsub::Subscriber::new(executor, rpc_client, server_handle),
        },
    )
}
...
impl DaemonRpcClient {
    ...
    > {
        self.subscriber.subscribe(
            "new_state_subscribe".into(),
            "new_state_unsubscribe".into(),
            "new_state".into(),
            1,
            &NO_ARGS,
        )
