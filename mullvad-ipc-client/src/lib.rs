...
impl DaemonRpcClient {
    ...
    pub fn generate_wg_key(&mut self) -> Result<()> {
        self.call("generate_wg_key", &NO_ARGS)
    }

    pub fn get_wg_key(&mut self) -> Result<Option<wireguard::PublicKey>> {
        self.call("get_wg_key", &NO_ARGS)
    }

    pub fn verify_wg_key(&mut self) -> Result<bool> {
        self.call("verify_wg_key", &NO_ARGS)
    }
