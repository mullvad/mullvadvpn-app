...
unsigned_changed_paths=()
important_file_was_removed=0
removed_important_paths=()
for locked_path in $locked_down_paths; do
    ...
    for commit in $locked_path_commit_hashes; do
        ...
        if ! git verify-commit "$commit" 2> /dev/null; then
            ...
            unsigned_changed_paths+=("$locked_path")
        fi
    done
    ...
    if [[ ! -e "$REPO_DIR/$locked_path" ]]; then
        ...
        removed_important_paths+=("$locked_path")
    fi
done

if [[ "$unsigned_commits_exist" != 0 || "$important_file_was_removed" != 0 ]]; then
    if (( ${#unsigned_changed_paths[@]} )); then
        echo
        echo "The following files are changed but not correctly signed:"
        for unsigned in "${unsigned_changed_paths[@]}"; do
            echo "$unsigned"
        done
    fi

    if (( ${#removed_important_paths[@]} )); then
        echo
        echo "The following files were removed:"
        for important_file in "${removed_important_paths[@]}"; do
            echo "$important_file"
        done
    fi
