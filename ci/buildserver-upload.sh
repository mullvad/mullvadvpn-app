...
while true; do
  ...
  for f_checksums in *.sha256; do
    ...
    for f in $files; do
      file_upload_path="$upload_path/$version"
      if [[ ! $f =~ \.($BUILD_ARTIFACT_EXTENSIONS|asc)$ ]]; then
        file_upload_path="$file_upload_path/additional-files"
      fi

      rsync -av --rsh='ssh -p 1122' "$f" "build@releases.mullvad.net:$file_upload_path/" || continue
      ...
      if [[ $f =~ \.($BUILD_ARTIFACT_EXTENSIONS)$ ]]; then
        ...
        rsync -av --rsh='ssh -p 1122' "$f.asc" "build@releases.mullvad.net:$file_upload_path/" || continue
        rm -f "$f.asc"
      fi
    done
