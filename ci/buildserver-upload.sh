...
while true; do
  ...
  for f_checksums in *.sha256; do
    ...
    for f in $files; do
      ...
      rsync -av --rsh='ssh -p 1122' "$f" "build@releases.mullvad.net:$file_upload_dir/" || continue
      ...
      if [[ $f =~ \.($BUILD_ARTIFACT_EXTENSIONS)$ ]]; then
        ...
        rsync -av --rsh='ssh -p 1122' "$f.asc" "build@releases.mullvad.net:$file_upload_dir/" || continue
        rm -f "$f.asc"
      fi
    done
