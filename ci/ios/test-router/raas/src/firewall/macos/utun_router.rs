//! Ferries packets between the two tunnel devices, dropping what the block list forbids.
//!
//! The kernel does the actual forwarding and NAT; this router only carries each packet across
//! the gap between the source and sink utuns, in both directions, applying the block rules on
//! the way.

use std::{net::IpAddr, sync::Arc};

use crate::firewall::{Endpoints, BlockRule, rule::RuleSet};
    ...
    pub fn spawn(devices: TunnelDevices, rules: Arc<ArcSwap<RuleSet>>) {
        ...
        tokio::spawn(async move {
            ...
            while let Ok(bytes_received) = source.recv(&mut buf).await {
                let packet = &buf[..bytes_received];
                if blocked(&rules_clone.load(), packet) {
                    continue;
                }
...
fn blocked(rules: &RuleSet, packet: &[u8]) -> bool {
    if rules.is_empty() {
        return false;
    }
