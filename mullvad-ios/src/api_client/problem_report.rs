...
// Safety: key and value must point to nunll-terminated strings
#[no_mangle]
...
pub extern "C" fn swift_problem_report_meta_data_free(mut map: ProblemReportMetadata) {
    if !map.inner.is_null() {
        // Safety: `map.inner` must have been allocated with `Box::into_raw()`
        // The caller must guarantee that `map.inner` is not null and has not been
        unsafe {
