...
async fn mullvad_ios_rotate_device_key_inner(
    ...
    let future_factory =
        || api.replace_wg_key_reponse(account_number.clone(), identifier.clone(), pub_key.clone());
...
async fn mullvad_ios_create_device_inner(
    ...
    let future_factory = || api.create_reponse(account_number.clone(), pub_key.clone());
