...
impl ConnectionTracker {
    ...
    fn register_secondary_outbound(&mut self, packet: &[u8]) {
        if let Ok(ipv4) = Ipv4Packet::new_checked(packet) {
            self.register_secondary_outbound_v4(&ipv4);
        } else if let Ok(ipv6) = Ipv6Packet::new_checked(packet) {
            self.register_secondary_outbound_v6(&ipv6);
        }
    }

    fn register_secondary_outbound_v4(&mut self, ip: &Ipv4Packet<&[u8]>) {
        let src_ip = IpAddr::from(ip.src_addr());
        let dst_ip = IpAddr::from(ip.dst_addr());
        let payload = ip.payload();
        ...
        match ip.next_header() {
            ...
            IpProtocol::Icmp => {
                ...
                if icmp.msg_type() == Icmpv4Message::EchoRequest {
                    self.track_icmp(dst_ip, icmp.echo_ident());
                }
            }
            _ => {}
        }
    ...
    fn register_secondary_outbound_v6(&mut self, ip: &Ipv6Packet<&[u8]>) {
        let src_ip = IpAddr::from(ip.src_addr());
        let dst_ip = IpAddr::from(ip.dst_addr());
        let payload = ip.payload();
