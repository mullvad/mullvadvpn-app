...
use super::{get_string, TunnelObfuscatorHandle, TunnelObfuscatorRuntime};
...
pub unsafe extern "C" fn start_udp2tcp_obfuscator_proxy(
    ...
    let peer_sock_addr = match get_socket_address(peer_address, peer_address_len, peer_port) {
        Ok(value) => value,
...
fn get_socket_address(peer_address: *const u8, peer_address_len: usize, peer_port: u16) -> Result<SocketAddr, i32> {
    let peer_sock_addr =
...
fn start(proxy_handle: *mut ProxyHandle, result: Result<(SocketAddr, TunnelObfuscatorHandle), std::io::Error>) -> i32 {
    match result {
        Ok((local_endpoint, obfuscator_handle)) => {
            let boxed_handle = Box::new(obfuscator_handle);
            // SAFETY: See notes for `write`.
