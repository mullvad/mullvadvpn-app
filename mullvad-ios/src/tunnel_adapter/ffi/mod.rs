...
use super::{
    IosTunnelAdapter, ObfuscationConfig, PeerConfig, TunnelCallbackHandler, TunnelConfig,
};
...
fn key32(bytes: &[u8], what: &str) -> Result<[u8; 32], GotaTunFfiError> {
    bytes.try_into().map_err(|_| {
        GotaTunFfiError::InvalidConfig(format!(
    ...
    T::Err: std::fmt::Display,
{
    value.parse().map_err(|e| {
        GotaTunFfiError::InvalidConfig(format!("invalid {what} '{value}': {e}"))
    })
}
...
fn build_peer(peer: &GotaTunPeer, allowed_ips: Vec<IpNetwork>) -> Result<PeerConfig, GotaTunFfiError> {
    Ok(PeerConfig {
...
fn build_obfuscation(obfuscation: GotaTunObfuscation) -> Result<ObfuscationConfig, GotaTunFfiError> {
