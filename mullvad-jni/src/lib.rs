...
use mullvad_daemon::{
    logging, version, Daemon, DaemonCommandSender, EventListener, ManagementCommand,
};
...
pub enum Error {
    ...
    #[error(display = "Failed to store daemon command sender")]
    StoreDaemonCommandSender,
}
...
fn initialize(log_dir: PathBuf) -> Result<(), Error> {
    ...
    match DAEMON_INTERFACE.lock() {
        Ok(mut daemon_interface) => {
            daemon_interface.set_command_sender(daemon_command_sender);
            Ok(())
        }
        Err(_) => {
            daemon_command_sender
                .send(ManagementCommand::Shutdown)
                .unwrap();
            Err(Error::StoreDaemonCommandSender)
        }
    }
}
