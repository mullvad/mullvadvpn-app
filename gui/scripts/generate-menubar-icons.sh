...
function generate_ico() {
    ...
    for size in 16 32 48; do
        local target_size_no_padding=$((7 * size / 8))
        local notification_icon_size=$((size / 3))
        local png_tmp_path="$TMP_DIR/$size"

        rsvg-convert -o "$png_tmp_path.png" -w $target_size_no_padding -h $target_size_no_padding \
            "$svg_source_path"
        convert -background transparent "$png_tmp_path.png" -gravity center -extent ${size}x$size \
            "$png_tmp_path.png"

        # Append notification notification icon
        overlay_notification_icon "$png_tmp_path.png" "${png_tmp_path}_notification.png" \
...
function generate() {
    ...
    # MacOS colored
    generate_png "$svg_source_path" "$macos_target_base_path.png" \
        "${macos_target_base_path}_notification.png" 22 3 4
    generate_png "$svg_source_path" "$macos_target_base_path@2x.png" \
        "${macos_target_base_path}_notification@2x.png" 44 6 8

    # MacOS monochrome
    generate_png "$black_svg_source_path" "${macos_target_base_path}Template.png" \
        "${macos_target_base_path}_notificationTemplate.png" 22 3 4
    generate_png "$black_svg_source_path" "${macos_target_base_path}Template@2x.png" \
        "${macos_target_base_path}_notificationTemplate@2x.png" 44 6 8

    # Linux colored
    generate_png "$svg_source_path" "$linux_target_base_path.png" \
        "${linux_target_base_path}_notification.png" 48 8 10

    # Linux white
    generate_png "$white_svg_source_path" "${linux_target_base_path}_white.png" \
        "${linux_target_base_path}_white_notification.png" 48 8 10
