...
const config = {
  ...
  linux: {
    ...
    category: 'Network',
    executableName: 'mullvad-gui',
    extraFiles: [{ from: '../dist-assets/linux/mullvad-gui-launcher.sh', to: './mullvad-vpn' }],
    extraResources: [
      { from: distAssets('problem-report'), to: '.' },
      { from: distAssets('mullvad-daemon'), to: '.' },
...
function packLinux() {
  return builder.build({
    ...
    config: {
      ...config,
      afterPack: (context) => {
        const launcherScript = path.join(context.appOutDir, 'mullvad-vpn');
        const chromeSandbox = path.join(context.appOutDir, 'chrome-sandbox');

        return Promise.all([
          // make sure the mullvad-vpn launcher script has the correct permissions
          chmodAsync(launcherScript, 0o755),
