...
export default class SvgMap extends React.Component<IProps, IState> {
  ...
  private removeOldViewportBboxesTimout?: number;
  ...
  public componentDidUpdate(_prevProps: IProps, _prevState: IState) {
    if (this.state.viewportBboxes.length > 1) {
      clearTimeout(this.removeOldViewportBboxesTimout);
      this.removeOldViewportBboxesTimout = setTimeout(() => {
  ...
  private removeOldViewportBboxes(state: IState) {
    return { viewportBboxes: [state.viewportBboxes[state.viewportBboxes.length - 1]] };
  }
}
