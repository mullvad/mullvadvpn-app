...
export class IPv4Address extends IpAddress<IPv4Groups> {
  public constructor(groups: IPv4Groups) {
    super(groups);

    // Ensure that each group is the correct number of bits
    if (groups.some((group) => !isNumberOfBits(group, IPv4GroupSize))) {
  ...
  public static fromString(ip: string): IPv4Address {
    try {
      const groups = IPv4Address.groupsFromString(ip);
      return new IPv4Address(groups);
    } catch (e) {
      throw new Error(`Invalid ip: ${ip}`);
    }
  }

  public static groupsFromString(ip: string): IPv4Groups {
    try {
      const groups = ip.split('.');
      if (groups.every((group) => /^\d{1,3}$/.test(group))) {
        const parsedGroups = groups.map((group) => parseInt(group, 10));
        if (IPv4Address.isIPv4Groups(parsedGroups)) {
          return parsedGroups;
        }
      }
    } catch (e) {
  ...
  private static isIPv4Groups(groups: number[]): groups is IPv4Groups {
    return groups.length === 4 && groups.every((group) => !isNaN(group));
  }
}

export class IPv4Range extends IpRange<IPv4Groups> {
  public constructor(groups: IPv4Groups, prefixSize: number) {
    super(groups, prefixSize);

    // Makes sure that the prefix is within the correct range
    if (prefixSize < 0 || prefixSize > 32) {
      throw new Error(`Invalid ip: ${groups.join('.')}/${prefixSize}`);
    }
  }
  ...
  public static fromString(subnet: string): IPv4Range {
    try {
      ...
      const groups = IPv4Address.groupsFromString(parts[0]);
      const prefixSize = parseInt(parts[1]);
      return new IPv4Range(groups, prefixSize);
    } catch (e) {
  ...
  public includes(ip: IPv4Address): boolean {
    return super.includes(ip, IPv4GroupSize);
  }
}
