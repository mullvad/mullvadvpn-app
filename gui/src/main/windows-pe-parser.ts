...
export async function rvaToOffset(
  ...
  for (let i = 0; i < numberOfSections; i++) {
    const sectionHeaderAddress = firstSectionAddress + i * Value.sizeOf(IMAGE_SECTION_HEADER);
    const sectionHeader = await Value.fromFile(
      fileHandle,
      sectionHeaderAddress,
      IMAGE_SECTION_HEADER,
    );
    const sectionVirtualAddress = sectionHeader.get('VirtualAddress').value<number>();
    const sectionSize = sectionHeader.get('SizeOfRawData').value<number>();

    if (rva >= sectionVirtualAddress && rva <= sectionVirtualAddress + sectionSize) {
      const pointerToRawData = sectionHeader.get('PointerToRawData').value();
      return pointerToRawData + (rva - sectionVirtualAddress);
    }
  }

  throw new Error('Failed to calculate RVA');
}
