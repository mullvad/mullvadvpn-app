...
async function getPeSignature(fileHandle: fs.promises.FileHandle): Promise<string> {
  const { buffer: offsetBuffer } = await fileHandle.read(Buffer.alloc(4), 0, 4, 0x3c);
  const offset = offsetBuffer.readUIntLE(0, 4);

  const { buffer: signatureBuffer } = await fileHandle.read(Buffer.alloc(4), 0, 4, offset);
  return signatureBuffer.toString('ascii');
}
...
async function getNtHeader(
  ...
  const signature = await getPeSignature(fileHandle);
  if (
    signature !== 'PE\0\0' ||
    dosHeader.get<PrimitiveValue>('e_magic').value<number>() !== 0x5a4d
  ) {
