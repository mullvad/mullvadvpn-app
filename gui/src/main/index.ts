...
class ApplicationMain {
  private notificationController = new NotificationController((url: string, withAuth?: boolean) =>
    this.openLink(url, withAuth),
  );
  ...
  private onDaemonConnected = async () => {
    ...
    if (this.evaluateNotificationVisibility(notification)) {
      this.notificationController.notify(notification.getSystemNotification());
    }
  ...
  private setTunnelState(newState: TunnelState) {
    ...
    this.notificationController.notifyTunnelState(
      this.evaluateNotificationVisibility,
      newState,
  ...
  private notifyOfAccountExpiry() {
    if (this.accountData) {
      ...
      const notification = new AccountExpiryNotificationProvider({
        accountExpiry,
  ...
  private evaluateNotificationVisibility = (notificationProvider: SystemNotificationProvider) => {
    // notification.critical and notification.suppressInDevelopment might be dependent on the
    // condition in notification.visible being true and should therefore not be called if
    // notification.visible is false.
    if (notificationProvider.mayDisplay()) {
      ...
      return !suppressDueToDevelopment && !this.shouldSuppressNotifications(notification.critical);
    } else {
      return false;
    }
  };
  ...
  private shouldSuppressNotifications(isCriticalNotification: boolean): boolean {
    const isVisible = this.windowController?.isVisible() ?? false;
