...
export default class NotificationController {
  ...
  constructor(
    private openLink: (url: string, withAuth?: boolean) => Promise<void>,
    private isWindowVisible: () => boolean,
    private areSystemNotificationsEnabled: () => boolean,
  ) {
    let usePngIcon;
  ...
  private evaluateNotification(notification: SystemNotification) {
    ...
    const suppressDueToVisibleWindow = this.isWindowVisible();
    const suppressDueToPreference = !this.areSystemNotificationsEnabled() && !notification.critical;
