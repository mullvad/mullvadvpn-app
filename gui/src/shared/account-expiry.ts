...
export function formattedDate(date: DateArgument, locale: string): string {
  return moment(date).locale(locale).format('lll');
}

export function durationUntilExpiry(expiry: DateArgument, locale: string): string {
...
export function remainingTime(
  ...
  const duration = durationUntilExpiry(expiry, locale);
