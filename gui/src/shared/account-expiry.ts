import { DateComponent, DateType, formatRelativeDate, modifyDate } from './date-helper';
...
export function closeToExpiry(expiry: DateType): boolean {
  return !hasExpired(expiry) && new Date(expiry) <= modifyDate(new Date(), DateComponent.day, 3);
}
