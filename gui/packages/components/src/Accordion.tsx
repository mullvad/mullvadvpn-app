...
export default class Accordion extends Component<IProps, IState> {
  ...
  private containerRef: Animated.View | null = null;
    ...
    return (
      <Animated.View
        ...
        ref={ (ref: React.ReactNode) => {
          // TODO: Replace with React.createRef when ReactXP typings are aligned with React 16.4
          // See: https://github.com/Microsoft/reactxp/issues/781
          if (ref) {
            if (ref instanceof Animated.View) {
  ...
  private async animateHeightChanges() {
    const containerView = this.containerRef;
    if (!containerView) {
      return;
    }
  ...
  private contentLayoutDidChange = ({ height }: Types.ViewOnLayoutEvent) => (this.contentHeight = height);
}
