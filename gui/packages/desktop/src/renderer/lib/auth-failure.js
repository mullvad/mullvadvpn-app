...
export class AuthFailure {
  _reason_id: AuthFailureKind;
  _message: string;

  constructor(reason: ?string) {
    if (!reason) {
      log.error('failed to authenticate');
      this._reason_id = 'INVALID';
    ...
    this._reason_id = str_to_failure_kind(id_str);
    this._message = reason.substring(bracket_idx + 1).trim();
  }

  show(): string {
    switch (this._reason_id) {
    ...
    // unreachable but flow complains
    return 'Account authentication failed';
  }
}

export function str_to_failure_kind(id: string): AuthFailureKind {
  switch (id) {
    case 'INVALID_ACCOUNT':
    case 'EXPIRED_ACCOUNT':
    case 'TOO_MANY_CONNECTIONS':
