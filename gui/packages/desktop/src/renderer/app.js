...
export default class AppRenderer {
  ...
  constructor() {
    ...
    ipcRenderer.on('daemon-connected', () => {
      this._onDaemonConnect();
    });

    ipcRenderer.on('daemon-disconnected', (error: ?Error) => {
      this._onDaemonDisconnect(error);
    });
  ...
  _onDaemonDisconnect(error: ?Error) {
    const actions = this._reduxActions;
