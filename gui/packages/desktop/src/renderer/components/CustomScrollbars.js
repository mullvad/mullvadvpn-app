...
const SCROLLBAR_HIDE_ANIMATION_DURATION = 250;
...
export default class CustomScrollbars extends React.Component<Props, State> {
  ...
  handleMouseMove = (event: MouseEvent) => {
    ...
    if (this.state.isDragging && scrollable && thumb) {
      ...
      const thumbTop = pointInScrollContainer.y - this.state.dragStart.y - this.props.thumbInset.y;
  ...
  _startAutoHide() {
    ...
    this._autoHideTimer = setTimeout(() => {
      this.setState(
        ...
        () => {
          setTimeout(() => {
            // make sure the component is still mounted since this timer is not being cancelled on
            // unmount.
            if (this._isMounted) {
              // reset isWide after scrollbar has faded away.
              this.setState((state) => {
                if (state.showScrollIndicators) {
                  return state;
                } else {
          ...
          }, SCROLLBAR_HIDE_ANIMATION_DURATION);
        },
      );
    }, AUTOHIDE_TIMEOUT);
  }
  ...
  _computeTrackLength(scrollable: HTMLElement) {
    return scrollable.offsetHeight - this.props.thumbInset.y * 2;
  }
