...
export default class SelectLocation extends Component<Props, State> {
  ...
  componentDidUpdate(oldProps: Props) {
    ...
    if (
      !this._testLocationEqualLoose(oldLocation, newLocation) &&
      !this._testLocationEqualLoose(currentLocation, newLocation)
    ) {
  ...
  _testLocationEqualLoose(lhs: ?RelayLocation, rhs: ?RelayLocation) {
    if (lhs && rhs) {
      return this._testLocationEqual(lhs, rhs);
    } else {
      return lhs === rhs;
    }
  }
  ...
  _isCollapsed(relayLocation: RelayLocation) {
    return !this.state.expandedItems.some((location) =>
      this._testLocationEqual(location, relayLocation),
    );
  }

  _isSelected(relayLocation: RelayLocation) {
    return this._testLocationEqualLoose(this.state.selectedLocation, relayLocation);
  }

  _handleSelection = (location: RelayLocation) => {
    if (!this._testLocationEqualLoose(this.state.selectedLocation, location)) {
