...
enum ApplicationLanguage: String, CaseIterable, Identifiable {
    ...
    static var allSorted: [ApplicationLanguage] {
        ApplicationLanguage.allCases
            .sorted { $0.displayName.localizedCaseInsensitiveCompare($1.displayName) == .orderedAscending }
    }

    static func from(_ code: String) -> ApplicationLanguage {
        ApplicationLanguage(rawValue: code) ?? .english
    }
    ...
    static var currentLanguage: ApplicationLanguage {
        ...
        return ApplicationLanguage.from(langCode)
    }
}
