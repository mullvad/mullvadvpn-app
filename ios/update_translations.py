...
def merge_translations():
  ...
  for table_name in new_string_tables:
    if table_name not in existing_string_tables:
      ...
      new_table.write(dst, encoding='utf_8')
...
def merge_two_tables(base_table_path, new_table_path):
  ...
  # Iterate through newly generated table
  for new_key in new_table.strings:
    new_entry = new_table.lookup(new_key)

    if new_key in base_table.strings:
      base_entry = base_table.lookup(new_key)

      # Update comments
      if base_entry.comment != new_entry.comment:
        # base_table.strings[new_key] = new_entry
        base_entry.comment = new_entry.comment
        print("Update comment for key: {}".format(new_key))

      # Support adding default value to an existing entry.
      # Before: NSLocalizedString("key", comment: "") # key = key;
      # After: NSLocalizedString("key", value: "value", comment: "") # key = value;
      if base_entry.source == base_entry.target and new_entry.source != new_entry.target:
        base_entry.target = new_entry.target
        print("Update target for key: {}".format(new_key))
    else:
      print("New key: {}".format(new_key))
      base_table.store(new_entry)

  # Drop keys that don't exist anymore
  for old_key in list(base_table.strings.keys()):
    if old_key not in new_table.strings:
      print("Remove key: {}".format(old_key))
      del base_table.strings[old_key]
  ...
  base_table.write(base_table_path, encoding='utf_8')
