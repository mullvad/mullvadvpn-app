...
/// decode/encode happen on a single dedicated thread — safe to block, invisible to the cooperative
/// thread pool.
///
/// Coalescing: when multiple async callers call `read() async throws` concurrently while a disk
/// read is already in flight, they all suspend on the same `Task` and resume together when the
/// single I/O operation completes.
///
