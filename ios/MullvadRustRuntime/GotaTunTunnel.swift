...
public final class GotaTunTunnel: @unchecked Sendable {
    ...
    /// Retains the CallbackBox while the tunnel is running.
    /// Released in stop() after gotatun_stop_tunnel guarantees no more callbacks.
    ...
    public static func start(
        ...
        // callbackRetainer keeps the box alive; passUnretained avoids an extra
        // retain that would leak (gotatun_stop_tunnel is synchronous, so no
        // callbacks fire after stop() sets callbackRetainer = nil).
