...
extension AnyIPEndpoint {
    ...
    func withResolvedIP() -> Result<AnyIPEndpoint?, Error> {
        ...
        let endpoint = result.flatMap { (addrinfoPointer) -> AnyIPEndpoint? in
            if let ipv4Address = IPv4Address(addrInfo: addrinfoPointer.pointee) {
                return .ipv4(IPv4Endpoint(ip: ipv4Address, port: port))
            } else if let ipv6Address = IPv6Address(addrInfo: addrinfoPointer.pointee) {
                return .ipv6(IPv6Endpoint(ip: ipv6Address, port: port))
            } else {
                // The addrinfo is corrupt?
                return nil
            }
        }
