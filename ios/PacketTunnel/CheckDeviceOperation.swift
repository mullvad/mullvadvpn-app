...
final class CheckDeviceOperation: ResultOperation<DeviceCheck> {
    ...
    private func startFlow(completion: @escaping (Result<DeviceCheck, Error>) -> Void) {
        do {
            ...
            fetchData(
                ...
                self.didReceiveData(accountResult: accountResult, deviceResult: deviceResult, completion: completion)
            }
        } catch {
    ...
    private func completeKeyRotation(_ device: Device) throws {
        ...
        guard case let .loggedIn(accountData, deviceData) = deviceState else { throw InvalidDeviceStateError() }
        ...
        do {
            ...
            logger.error(error: error, message: "Failed to persist device state.")
