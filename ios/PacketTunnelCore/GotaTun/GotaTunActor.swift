...
public final class GotaTunActor: PacketTunnelActorProtocol, @unchecked Sendable {
    ...
    private func handleAdapterTimeout() async {
        ...
        case var .connected(info), var .reconnecting(info):
            info.incrementAttemptCount()
            logger.debug("Timeout — entering reconnecting (attempt \(info.connectionAttemptCount))")
            stopCurrentAdapter()
    ...
    private func handleNetworkReachability(_ pathStatus: NWPath.Status) {
        ...
        switch state {
        case var .connecting(info), var .connected(info), var .reconnecting(info):
            info.networkReachability = newReachability
            switch state {
            case .connecting: state = .connecting(info)
            case .connected: state = .connected(info)
            case .reconnecting: state = .reconnecting(info)
            default: break
            }
    ...
    private func startConnection(nextRelays: NextRelays) async {
        ...
        guard let ipv4Address = settings.interfaceAddresses.lazy.compactMap({ IPv4Address("\($0.address)")}).first
        else {
        ...
        guard let ipv6Address = settings.interfaceAddresses.lazy.compactMap({ IPv6Address("\($0.address)")}).first
        else {
