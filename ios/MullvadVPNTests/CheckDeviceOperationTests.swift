//
//  CheckDeviceOperationTests.swift
//  MullvadVPNTests
//
...
class CheckDeviceOperationTests: XCTestCase {
    ...
    func testShouldRespectCooldownWhenAttemptingToRotateImmediately() {
        ...
        startDeviceCheck(
            ...
            XCTAssertNotNil(deviceCheck)
            XCTAssertTrue(deviceCheck?.keyRotationStatus == KeyRotationStatus.none)
    ...
    func testShouldNotRotateKeyBeforeTwentyFourHoursHavePassed() {
        ...
        startDeviceCheck(
            ...
            XCTAssertNotNil(deviceCheck)
            XCTAssertEqual(deviceCheck?.keyRotationStatus, KeyRotationStatus.none)
            XCTAssertEqual(try? deviceStateAccessor.read().deviceData?.wgKeyData.privateKey, currentKey)
    ...
    private func startDeviceCheck(
        ...
        let operation = CheckDeviceOperation(
...
private class MockRemoteService: DeviceCheckRemoteServiceProtocol {
    typealias GetAccountHandler = (_ accountNumber: String) -> Result<AccountData, Error>
    typealias GetDeviceHandler = (_ accountNumber: String, _ identifier: String) -> Result<Device, Error>
    typealias RotateDeviceKeyHandler = (_ accountNumber: String, _ identifier: String, _ publicKey: PublicKey)
        -> Result<Device, Error>

    private let getAccountDataHandler: GetAccountHandler?
    private let getDeviceHandler: GetDeviceHandler?
    ...
    init(
        initialKey: PrivateKey,
        getAccountData: GetAccountHandler? = nil,
        getDevice: GetDeviceHandler? = nil,
        rotateDeviceKey: RotateDeviceKeyHandler? = nil
    ) {
        currentKey = initialKey
        getAccountDataHandler = getAccountData
        getDeviceHandler = getDevice
    ...
    func getAccountData(
        ...
        DispatchQueue.main.async { [self] in
            let result = getAccountDataHandler?(accountNumber) ?? .success(AccountData.mock())
    ...
    func getDevice(
        ...
        DispatchQueue.main.async { [self] in
            let result = getDeviceHandler?(accountNumber, identifier) ?? .success(Device.mock(privateKey: currentKey))
