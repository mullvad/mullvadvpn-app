...
final class RecentConnectionRepositoryTest {
    ...
    func run(_ scenario: Scenario) throws {
        switch scenario {
        case let .add(recentConnections):
            let repository = RecentConnectionRepository(store: InMemorySettingsStore<SettingNotFound>())
            try repository.clear()
            for recentConnection in recentConnections { try repository.add(recentConnection) }
            let all = try repository.all()
            #expect(all.count == recentConnections.count)

        case let .exceedLimit(recentConnections, max):
            let repository = RecentConnectionRepository(store: InMemorySettingsStore<SettingNotFound>(), maxLimit: max)
            try repository.clear()
            for recentConnection in recentConnections { try repository.add(recentConnection) }
        ...
        case let .addDuplicate(params):
            let repository = RecentConnectionRepository(store: InMemorySettingsStore<SettingNotFound>())
            try repository.clear()
            for recentConnection in params { try repository.add(recentConnection) }
            let all = try repository.all()
