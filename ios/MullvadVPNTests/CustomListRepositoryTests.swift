...
class CustomListRepositoryTests: XCTestCase {
    ...
    func testAddingCustomList() throws {
        ...
        var item = try XCTUnwrap(repository.create(name, locations: [
            .country("SE"),
            .city("SE", "Gothenburg"),
        ]))
    ...
    func testFetchingAllCustomList() throws {
        var streaming = try XCTUnwrap(repository.create("Netflix", locations: [
            .country("FR"),
            .city("SE", "Gothenburg"),
        ]))
        repository.update(streaming)

        var gaming = try XCTUnwrap(repository.create("PS5", locations: [
            .country("DE"),
            .city("SE", "Gothenburg"),
        ]))
        repository.update(streaming)
