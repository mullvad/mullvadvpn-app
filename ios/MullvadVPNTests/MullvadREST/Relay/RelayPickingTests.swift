...
class RelayPickingTests: XCTestCase {
    ...
    func testMultihopCannotPickAutomaticallyInvalidConfiguration() throws {
        let constraints = RelayConstraints(
        ...
        // It will already have pre-filtered relays to select a QUIC endpoint because it's the 2nd connection attempt
        obfuscation = try RelayObfuscator(
            relays: sampleRelays,
            tunnelSettings: LatestTunnelSettings(),
