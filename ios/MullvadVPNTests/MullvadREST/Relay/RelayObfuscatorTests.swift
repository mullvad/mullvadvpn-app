...
final class RelayObfuscatorTests: XCTestCase {
    ...
    func testObfuscateOffDoesNotChangeEndpoint() throws {
        ...
        let obfuscationResult = try RelayObfuscator(
            ...
            connectionAttemptCount: 0, bias: UnbiasedObfuscationBypass()
        ).obfuscate()
    ...
    func testObfuscateUpdOverTcpPortAutomaticIsRandomPort() throws {
        ...
        try (0 ... 10).filter { $0.isMultiple(of: 2) }.forEach { attempt in
            let obfuscationResult = try RelayObfuscator(
                relays: sampleRelays,
                tunnelSettings: tunnelSettings,
                connectionAttemptCount: UInt(attempt), bias: UnbiasedObfuscationBypass()
            ).obfuscate()
    ...
    func testObfuscatorBypass() throws {
        ...
        let obfuscationResult = try RelayObfuscator(
            relays: sampleRelays,
            tunnelSettings: tunnelSettings,
            connectionAttemptCount: 0, bias: ForceShadowsocksObfuscationBypassStub()
        ).obfuscate()
...
struct ForceShadowsocksObfuscationBypassStub: ObfuscationNotSupportedBypassing {
