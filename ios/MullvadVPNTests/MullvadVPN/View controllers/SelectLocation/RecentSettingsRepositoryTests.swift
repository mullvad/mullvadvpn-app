//
//  RecentSettingsRepositoryTests.swift
//  MullvadVPN
//
...
@Suite("RecentSettingsRepositoryTests")
final class RecentSettingsRepositoryTests {
    ...
    func addRecentsBeforeEnablingRecents() throws {
        ...
        #expect(throws: RecentSettingsError.recentsDisabled, performing: action)
    }

    private func makeRepository(max: Int = 50) -> RecentSettingsRepository {
        return RecentSettingsRepository(store: InMemorySettingsStore<SettingNotFound>(), maxLimit: max)
    }

    private func addLocations(
        _ repository: RecentSettingsRepository, locations: [UserSelectedRelays], to: RecentLocationType
    ) throws {
    ...
    private func cleanup(_ repository: RecentSettingsRepository) throws {
        try repository.setRecentsEnabled(false)
    }
}
