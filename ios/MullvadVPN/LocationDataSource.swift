...
final class LocationDataSource: UITableViewDiffableDataSource<Int, RelayLocation> {
    ...
    func indexPathForSelectedRelay() -> IndexPath? {
        if let location = selectedRelayLocation {
            return indexPath(for: location)
        } else {
            return nil
        }
    }
    ...
    private func toggleChildren(
        ...
        let node = nodeByLocation[relayLocation]
        node?.showsChildren = show

        updateCellFactory(with: nodeByLocation)
        updateDataSnapshot(with: locationList, animated: true) { [weak self] in
    ...
    private func item(for indexPath: IndexPath) -> LocationDataSourceItemProtocol? {
        return nodeByLocation[snapshot().itemIdentifiers[indexPath.row]]
    }

    override func indexPath(for location: RelayLocation) -> IndexPath? {
        return snapshot().itemIdentifiers.firstIndex(where: { $0 == location }).flatMap { index in
