...
class LocationDataSource: NSObject, UITableViewDataSource {
    ...
    func toggleChildren(_ relayLocation: RelayLocation, animated: Bool, completion: (() -> Void)? = nil) {
        ...
        showChildrenHelper(relayLocation, show: !node.showsChildren, showHiddenParents: false, animated: animated, completion: completion)
    }

    private func showChildrenHelper(_ relayLocation: RelayLocation, show: Bool, showHiddenParents: Bool, animated: Bool, completion: (() -> Void)? = nil) {
...
extension LocationDataSource {
    ...
    private class Node: LocationDataSourceItemProtocol {
        ...
        func computeActiveChildrenRecursive() {
            switch nodeType {
            case .root, .country, .city:
                var hasActiveChild = false
                for node in children {
                    node.computeActiveChildrenRecursive()
                    if node.isActive && !hasActiveChild {
                        hasActiveChild = true
                    }
                }
