...
class SelectLocationViewController: UIViewController, RelayCacheObserver, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let setRelayLocationOnViewDidLoad = setRelayLocationOnViewDidLoad {
            showChildrenForRelayLocation(setRelayLocationOnViewDidLoad, animated: false)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        dataSource?.setSelectedRelayLocation(item.location, animated: false, scrollPosition: .none)
    ...
    func setSelectedRelayLocation(_ relayLocation: RelayLocation?, animated: Bool, scrollPosition: UITableView.ScrollPosition) {
        ...
        if let relayLocation = relayLocation {
            showChildrenForRelayLocation(relayLocation, animated: animated) {
                setSelection()
            }
        } else {
            setSelection()
    ...
    private func showChildrenForRelayLocation(_ relayLocation: RelayLocation, animated: Bool, completion: (() -> Void)? = nil) {
        switch relayLocation {
        case .country:
            completion?()
        case .city:
            if let countryLocation = relayLocation.ascendants.first {
                self.dataSource?.showChildren(countryLocation, showHiddenParents: false, animated: animated, completion: completion)
            }
        case .hostname:
            if let cityLocation = relayLocation.ascendants.last {
                self.dataSource?.showChildren(cityLocation, showHiddenParents: true, animated: animated, completion: completion)
            }
        }
    }
}
