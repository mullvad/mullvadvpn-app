    ...
    @preconcurrency NotificationManagerDelegate
{
    ...
    private func handleNewAppVersionInAppNotification() {
        let message = [
            NSLocalizedString(
                "“Force all apps” is enabled, please disable it before updating or you will lose network connectivity. "
            ...
            NSLocalizedString(
                "If you do not wish to disable “Force all apps“, you can choose to disconnect from the "
                    + "VPN instead.",
                comment: ""
            ),
        ...
        let presentation = AlertPresentation(
            ...
            buttons: [
                AlertAction(
                    title: NSLocalizedString("Disable “Force all apps”", comment: ""),
                    style: .default,
                    handler: { [weak self] in
                        self?.tunnelManager.updateSettings([.includeAllNetworks(false)]) {
                            DispatchQueue.main.async {
