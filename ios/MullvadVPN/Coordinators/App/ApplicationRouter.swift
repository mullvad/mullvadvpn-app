...
protocol ApplicationRouterDelegate: AnyObject {
    /**
     Delegate should present the route pass corresponding `Coordinator` upon completion.
     */
...
final class ApplicationRouter {
    ...
    private func addPresentedRoute(_ presented: PresentedRoute) {
        ...
        logger.debug("Modal stack = \(modalStack)")
        logger.debug("Presented routes = \(presentedRoutes)")
    }
}
