...
class MethodSettingsViewController: UITableViewController {
    ...
    private lazy var dataSoruceConfiguration: MethodSettingsDataSourceConfiguration? = {
    ...
    private func viewModelDidChange(previousValue: AccessMethodViewModel?, newValue: AccessMethodViewModel) {
        ...
        validateInput()
        dataSoruceConfiguration?.updateDataSource(
    ...
    private func onTest() {
        ...
        guard contentValidationErrors.isEmpty else {
            dataSoruceConfiguration?.updateDataSourceWithContentValidationErrors(viewModel: subject.value)
            return
        }
