...
enum MethodSettingsItemIdentifier: Hashable {
    ...
    var isSelectable: Bool {
        switch self {
        case .name, .testingStatus, .cancelTest:
            false
        case .protocol:
            true
        case let .proxyConfiguration(itemIdentifier):
    ...
    static func fromValidationErrors(
        _ errors: AccessMethodValidationError,
        ...
        case .direct, .bridges:
            []
        case .shadowsocks:
            errors.fieldErrors.compactMap { error in
                switch error.field {
        ...
        case .socks5:
            errors.fieldErrors.map { error in
                switch error.field {
