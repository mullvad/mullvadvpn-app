...
struct IncludeAllNetworksSettingsView<ViewModel: IncludeAllNetworksSettingsViewModel>: View {
    ...
    var body: some View {
        ...
        .onChange(of: viewModel.showEnableNotificationsAlert, initial: false) { _, showAlert in
            guard showAlert else { return }
            showEnableNofiticationsAlert()
        }
        .onChange(of: viewModel.showReconsiderNotificationsAlert, initial: false) { _, showAlert in
            guard showAlert else { return }
            showReconsiderNofiticationsAlert()
        }
        ...
        return SettingsInfoViewModel(
            pages: [
                ...
                SettingsInfoViewModelPage(
                    ...
                    customView: AnyView(
                        ActionBox(
                            isOn: viewModel.consent,
                            isInteractable: !viewModel.consent,
                            label: NSLocalizedString(
                                "Given the pros and cons of using this feature, I wish to proceed",
                                comment: ""
                            ),
                            didToggle: { isOn in
                                viewModel.consent = isOn
                            }
