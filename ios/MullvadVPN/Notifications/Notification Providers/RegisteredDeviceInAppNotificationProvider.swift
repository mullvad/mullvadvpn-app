    ...
    InAppNotificationProvider
{
    ...
    private var showsInAppNotification = false {
        didSet {
            // Invalidate notification after one second for the sake of transition animation
            DispatchQueue.global().asyncAfter(deadline: .now() + .seconds(1), execute: .init(block: { [weak self] in
