    ...
    @unchecked Sendable
{
    ...
    var onMigrationStateChanged: (MigratedSettingsState) -> Void
    ...
    private var shouldShowNotification: Bool {
        hasMigratedSettings && migratedSettingsState.hasCompletedMigrationWizard == false
    }
    ...
    init(
        ...
        migratedSettingsState: MigratedSettingsState,
        onMigrationStateChanged: @escaping (MigratedSettingsState) -> Void
    ) {
        self.tunnelManager = tunnelManager
        self.relaySelector = relaySelector
        self.migratedSettingsState = migratedSettingsState
        self.onMigrationStateChanged = onMigrationStateChanged
        self.isSettingsMigrated = MigratedVersion.current.rawValue > migratedSettingsState.lastMigratedVersion
        self.isAppUpdated =
            migratedSettingsState.lastInstalledVersion.isEmpty
            ? false : migratedSettingsState.lastInstalledVersion != appVersion
        super.init()
        ...
        self.migratedSettingsState.shouldShowMigratedSettingsMenuItem =
            !isAppUpdated && migratedSettingsState.shouldShowMigratedSettingsMenuItem

        self.migratedSettingsState.lastMigratedVersion = MigratedVersion.current.rawValue
        self.migratedSettingsState.lastInstalledVersion = appVersion

        onMigrationStateChanged(self.migratedSettingsState)
        addTunnelObserver()
    }
    ...
    private func handleMigratedSettings(_ settings: LatestTunnelSettings) {
        ...
        tunnelManager.updateSettings([
            ...
            if !migrationResult.changes.isEmpty {
                self.migratedSettingsState.preMigrationSettings = preMigrationSettings
                self.migratedSettingsState.hasCompletedMigrationWizard = false
                self.migratedSettingsState.shouldShowMigratedSettingsMenuItem = true
                self.migrationOutput = migrationResult
            } else {
                self.migratedSettingsState.preMigrationSettings = nil
                self.migratedSettingsState.hasCompletedMigrationWizard = true
                self.migratedSettingsState.shouldShowMigratedSettingsMenuItem = false
            }
            self.onMigrationStateChanged(self.migratedSettingsState)
        }
    }
