...
class ConnectViewController: UIViewController, MKMapViewDelegate, RootContainment, TunnelObserver {
    ...
    private func computeCoordinateRegion(center: CLLocationCoordinate2D, offset: CGPoint) -> MKCoordinateRegion {
        ...
        return mainContentView.mapView.regionThatFits(region).rounded()
    }
...
private extension CLLocationCoordinate2D {
    func rounded() -> CLLocationCoordinate2D {
        return CLLocationCoordinate2D(
            latitude: latitude.roundedTo(numDecimalPlaces: 2),
            longitude: longitude.roundedTo(numDecimalPlaces: 2)
        )
    }

    func isApproximatelyEqualTo(_ other: CLLocationCoordinate2D) -> Bool {
        return fabs(latitude - other.latitude) <= .ulpOfOne &&
            fabs(longitude - other.longitude) <= .ulpOfOne
    }
}

private extension MKCoordinateSpan {
    func rounded() -> MKCoordinateSpan {
        return MKCoordinateSpan(
            latitudeDelta: latitudeDelta.roundedTo(numDecimalPlaces: 2),
            longitudeDelta: longitudeDelta.roundedTo(numDecimalPlaces: 2)
        )
    }
}

private extension MKCoordinateRegion {
    func rounded() -> MKCoordinateRegion {
        return MKCoordinateRegion(center: center.rounded(), span: span.rounded())
    }

    func isApproximatelyEqualTo(_ other: MKCoordinateRegion) -> Bool {
        return center.isApproximatelyEqualTo(other.center) &&
...
private extension Double {
    func roundedTo(numDecimalPlaces: Int) -> Double {
        let power = pow(10, Double(numDecimalPlaces))
