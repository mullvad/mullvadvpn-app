...
>, UITableViewDelegate {
    ...
    init(
        ...
        let settingsFactory = SettingsCellFactory()
        settingsFactory.tableView = tableView
        settingsFactory.interactor = interactor

        super.init(tableView: tableView) { tableView, indexPath, itemIdentifier in
            settingsFactory.makeCell(for: itemIdentifier, indexPath: indexPath)
        }
    ...
    private func didUpdateDeviceState(_ deviceState: DeviceState) {
           ...
           oldAccountData.expiry != newAccountData.expiry
        {
            if #available(iOS 15.0, *) {
                ...
                for case let cell as SettingsAccountCell in tableView?.visibleCells ?? [] {
                    cell.accountExpiryDate = newAccountData.expiry
                }
            }
        }
    }
