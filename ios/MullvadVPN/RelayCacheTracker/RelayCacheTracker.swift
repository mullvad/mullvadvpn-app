...
final class RelayCacheTracker: RelayCacheTrackerProtocol {
    ...
    private func hotfixRelaysThatDoNotHaveDaita() throws {
        ...
        let updatedCachedRelays = StoredRelays(etag: cachedRelays.etag, rawData: updatedRawRelayData, updatedAt: now)

        try cache.write(record: updatedCachedRelays)
        self.cachedRelays = CachedRelays(etag: cachedRelays.etag, relays: updatedRelays, updatedAt: now)
    }
