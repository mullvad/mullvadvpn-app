...
class SearchAnythingViewController: UIViewController {
    ...
    enum Destination {
        case account, selectLocation, changelog, daita, multihop, settings, vpnSettings, problemReport, faq, apiAccess, logout
    }
    ...
    init(relays: [RelayWithLocation<REST.ServerRelay>]) {
        ...
        items.append(Item(title: "Log out", destination: .logout, location: nil, cell: .setting))
