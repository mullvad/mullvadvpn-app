...
final class StoreTransactionLog {
    ...
    static var defaultFileURL: URL {
        ...
        let location = directories.first?.appendingPathComponent("best/transaction.log", isDirectory: false)
    ...
    /// - Returns: `true` if transaction log contains ssuch transaction identifier, otherwise false.
    ...
    func read() {
        stateLock.withLock {
            do {
                ...
                transactionIdentifiers = Set(contents.split { $0.isNewline }.map { String($0) })
            } catch {
                switch error {
