...
class RelayFilterViewModel {
    var relays: [REST.ServerRelay] {
        didSet { didUpdateRelays?() }
    }

    var relayFilter: RelayFilter {
        didSet {
            didUpdateFilter.forEach { callback in
                callback?(relayFilter)
            }
        }
    }

    var uniqueProviders: [String] {
        return Array(Set(relays.map { $0.provider })).caseInsensitiveSorted()
    }

    var didUpdateRelays: (() -> Void)?
