...
>, UITableViewDelegate {
    ...
    func resetToLastPortSelection() -> Bool {
        if let customPortCell = getCustomPortCell(), customPortCell.textField.isEditing {
            ...
            } else if let port = viewModel.wireGuardPort, viewModel.wireGuardPortIsCustom {
    ...
    func tableView(
        ...
        case .wireGuardCustomPort:
            if viewModel.wireGuardPortIsCustom {
                selectRow(at: indexPath)
            }
        default:
            break
    ...
    private func configureWireguardPortsHeader(_ reusableView: SettingsHeaderView) {
        ...
        reusableView.didCollapseHandler = { [weak self] headerView in
            ...
            var snapshot = self.snapshot()
            var shouldDelayUpdate = false

            if headerView.isExpanded {
                shouldDelayUpdate = resetToLastPortSelection()
            ...
            // The updated should be delayed when we're reverting an ongoing change, to give the
            // user just enough time to notice it.
            DispatchQueue.main.asyncAfter(deadline: .now() + (shouldDelayUpdate ? 0.4 : 0)) {
                headerView.isExpanded.toggle()
                self.apply(snapshot, animatingDifferences: true)
            }
        }
