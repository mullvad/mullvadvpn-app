...
class AccountDeletionContentView: UIView {
    ...
    private func configureUI() {
        addConstrainedSubviews([textsStack, buttonsStack]) {
            ...
            buttonsStack.pinEdgesToSuperviewMargins(.all().excluding(.top).excluding(.bottom))
            textsStack.bottomAnchor.constraint(lessThanOrEqualTo: buttonsStack.topAnchor)
        }
        bottomsOfButtonsConstraint = buttonsStack.bottomAnchor
            .constraint(lessThanOrEqualTo: layoutMarginsGuide.bottomAnchor).withPriority(.defaultLow)
