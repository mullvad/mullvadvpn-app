...
class AllLocationDataSource: LocationDataSourceProtocol {
    ...
    func node(by location: RelayLocation) -> LocationNode? {
        let rootNode = RootNode(children: nodes)

        return switch location {
        case let .country(countryCode):
            rootNode.nodeFor(nodeCode: countryCode)
        case let .city(_, cityCode):
            rootNode.nodeFor(nodeCode: cityCode)
        case let .hostname(_, _, hostCode):
            rootNode.nodeFor(nodeCode: hostCode)
        }
    }
    ...
    private func addLocation(
        ...
        switch location {
        case let .country(countryCode):
            let countryNode = LocationCountryNode(
        ...
        case let .city(countryCode, cityCode):
            let cityNode = LocationCityNode(nodeName: serverLocation.city, nodeCode: cityCode, locations: [location])
        ...
        case let .hostname(countryCode, cityCode, hostIdentifier):
            let hostNode = LocationHostNode(nodeName: relay.hostname, nodeCode: hostIdentifier, locations: [location])
