...
final class LocationNodeResolver {
    ...
    func setSelectedNode(selectedRelays: UserSelectedRelays) {
        resetSelection()
        selectedNode = first(by: selectedRelays)
        selectedNode?.isSelected = true
    }

    func setConnectedRelay(hostname: String?) {
        let rootNode = RootLocationNode(children: providers.flatMap(\.nodes))
        rootNode
            .flattened
            .forEachNode { node in
    ...
    private func first(by relays: UserSelectedRelays) -> LocationNode? {
