...
final class LocationViewControllerWrapper: UIViewController {
    ...
    init(
        ...
        if multihopEnabled {
            ...
            if daitaSettings.shouldDoAutomaticRouting {
                entryLocationViewController?.enableDaitaAutomaticRoutingMode()
            }
        }
    ...
    func onDaitaSettingsUpdate(_ settings: DAITASettings, relaysWithLocation: LocationRelays, filter: RelayFilter) {
        ...
        if daitaSettings.shouldDoAutomaticRouting {
            entryLocationViewController?.enableDaitaAutomaticRoutingMode()
        } else if !daitaSettings.daitaState.isEnabled || daitaSettings.shouldDoDirectOnly {
            entryLocationViewController?.disableDaitaAutomaticRoutingMode()
        }
    }
