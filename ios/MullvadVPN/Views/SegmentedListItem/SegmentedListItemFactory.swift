...
struct SegmentedListItemFactory {
    enum Leading {
        ...
        case recentLocation(node: LocationNode)
        case setting(title: String, subtitle: String? = nil, level: Int = 0, isSelected: Bool = false)
    }
    ...
    @ViewBuilder func leading(for leading: Leading) -> some View {
        ...
        case .recentLocation(let node):
            RecentItemView(node: node)
        case .setting(let title, let subtitle, let level, let isSelected):
            ListItem(
                title: title,
                subtitle: subtitle,
                level: level,
