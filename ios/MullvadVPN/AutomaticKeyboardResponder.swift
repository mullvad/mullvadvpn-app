...
extension AutomaticKeyboardResponder {
    ...
    convenience init<T: UIScrollView>(targetView: T) {
        self.init(targetView: targetView) { (scrollView, offset) in
            if targetView.canBecomeFirstResponder {
                targetView.contentInset.bottom = targetView.isFirstResponder ? offset : 0
                targetView.scrollIndicatorInsets.bottom = targetView.isFirstResponder ? offset : 0
            } else {
                targetView.contentInset.bottom = offset
                targetView.scrollIndicatorInsets.bottom = offset
            }
        }
    }
}
