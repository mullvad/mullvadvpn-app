...
actor TunnelManager {
    ...
    func rotatePrivateKey(completionHandler: @MainActor @escaping @Sendable (Error?) async -> Void) -> Cancellable {
        accountManager.rotatePrivateKey { result in
            Task { [weak self] in
                guard let self else { return }

                await updatePrivateKeyRotationTimer()

                switch result {
                case .success:
                    await notifyTunnelKeyRotated(completionHandler: completionHandler)
                case .failure(let error):
                    await handleRestError(error)
                }
            }
        }
    }

    private func notifyTunnelKeyRotated(completionHandler: @MainActor @escaping @Sendable (Error?) async -> Void) {
        guard let tunnel else { return }
        _ = tunnel.notifyKeyRotation { result in
            Task { @MainActor in
                await completionHandler(result.error)
            }
        }
    }
    ...
    func reconnectTunnel(selectNewRelay: Bool, completionHandler: (@Sendable (Error?) -> Void)? = nil) {
        ...
        let operation = AsyncBlockOperation(dispatchQueue: internalQueue) { [self] finish in
            // On internalQueue, so safe to assume isolation.
                ...
                _ = tunnel.reconnectTunnel(to: selectNewRelay ? .random : .current) { [weak self] result in
                        ...
                        let self
                    {
                        self.internalQueue.async {
                            // On internalQueue, so safe to assume isolated.
                            self.assumeIsolated { actor in
                                _ = actor.setTunnelStatus { tunnelStatus in
                ...
                didUpdateTunnelStatus: { [weak self] _, status in
                    if case .disconnected = status.state {
                        ...
                        self.internalQueue.async {
                            // On internalQueue, so safe to assume isolation.
                            self.assumeIsolated { actor in
                                if let observer = actor.observer {
    ...
    /// the observer.
    /// Nonisolation is safe as long as `ObserverList.append` keeps its locks.
    ...
    private func subscribeVPNStatusObserver(tunnel: any TunnelProtocol) {
        ...
        statusObserver = tunnel.addBlockObserver(queue: internalQueue) { [weak self] _, status in
            ...
            self.assumeIsolated { actor in
                ...
                actor._lastNEVPNStatus = status
                actor.logger.debug("VPN connection status changed to \(status).")
        ...
        let operation = AsyncBlockOperation(dispatchQueue: internalQueue) { [self, settingsManager] in
            ...
            self.assumeIsolated { actor in
                do {
                    ...
                    actor.logger.error(error: error, message: "Failed to refresh device state")
                }
            }
        }
        ...
        _ = tunnel.getTunnelStatus { [weak self] result in
            ...
            self.internalQueue.async {
                ...
                self.assumeIsolated { actor in
                    ...
                    guard actor.isTunnelAlive else {
                        actor.logger.debug("Ignoring stale IPC response, tunnel is dead.")
                        return
                    }

                    if case let .success(observedState) = result {
                        let newState = actor.mapObservedStateToTunnelState(observedState)
                        _ = actor.setTunnelStatus { tunnelStatus in
                            tunnelStatus.observedState = observedState
    ...
    private func scheduleSettingsUpdate(
        ...
        let operation = AsyncBlockOperation(dispatchQueue: internalQueue) { [self] in
            // On internalQueue, so safe to assume isolated.
            self.assumeIsolated { actor in
    ...
    private func unsetTunnelConfiguration(isRemovingProfile: Bool = true) {
        ...
        tunnel.removeFromPreferences { [weak self] error in
            ...
            self.internalQueue.async {
                ...
                self.assumeIsolated { actor in
                    ...
                    actor.setTunnel(nil, shouldRefreshTunnelState: false)
                }
            }
        }
    }
...
/// Used by operations in TunnelManager. Since those operations excute on `internalQueueu`
/// it's safe to assume isolation in the contained functions.
private struct TunnelInteractorProxy: TunnelInteractor {
    ...
    var tunnel: (any TunnelProtocol)? {
        tunnelManager.assumeIsolated { $0._tunnel }
    }
    ...
    func setTunnel(_ tunnel: (any TunnelProtocol)?, shouldRefreshTunnelState: Bool) {
        tunnelManager.assumeIsolated { $0.setTunnel(tunnel, shouldRefreshTunnelState: shouldRefreshTunnelState) }
    }

    var tunnelStatus: TunnelStatus {
        tunnelManager.assumeIsolated { $0._tunnelStatus }
    }

    func updateTunnelStatus(_ block: @Sendable (inout TunnelStatus) -> Void) -> TunnelStatus {
        tunnelManager.assumeIsolated { $0.setTunnelStatus(block) }
    }

    var isConfigurationLoaded: Bool {
        tunnelManager.assumeIsolated { $0._isConfigurationLoaded }
    }

    var settings: LatestTunnelSettings {
        tunnelManager.assumeIsolated { $0._tunnelSettings }
    }

    var deviceState: DeviceState {
        tunnelManager.assumeIsolated { $0._deviceState }
    }

    func setConfigurationLoaded() {
        tunnelManager.assumeIsolated { $0.setConfigurationLoaded() }
    }

    func setSettings(_ settings: LatestTunnelSettings, persist: Bool) {
        tunnelManager.assumeIsolated { $0.setSettings(settings, persist: persist) }
    }

    func setDeviceState(_ deviceState: DeviceState, persist: Bool) {
        tunnelManager.assumeIsolated { $0.setDeviceState(deviceState, persist: persist) }
    }

    func removeLastUsedAccount() {
        tunnelManager.assumeIsolated { $0.removeLastUsedAccount() }
    }

    func startTunnel() {
        tunnelManager.assumeIsolated { $0.startTunnel() }
    }

    func prepareForVPNConfigurationDeletion() {
        tunnelManager.assumeIsolated { $0.prepareForVPNConfigurationDeletion() }
    }

    func selectRelays() throws -> SelectedRelays {
        try tunnelManager.assumeIsolated { try $0.selectRelays(tunnelSettings: $0._tunnelSettings) }
    }

    func handleRestError(_ error: Error) {
        tunnelManager.assumeIsolated { $0.handleRestError(error) }
    }
}
...
extension TunnelManager {
    ...
    fileprivate struct AccountManagerInteractor: AccountManagerTunnelInteractor {
        ...
        var deviceState: DeviceState {
            manager?.assumeIsolated { $0._deviceState } ?? .loggedOut
        }

        func setDeviceState(_ deviceState: DeviceState, persist: Bool) {
            manager?.assumeIsolated { $0.setDeviceState(deviceState, persist: persist) }
        }
        ...
        func unsetTunnelConfiguration() {
            manager?.assumeIsolated { $0.unsetTunnelConfiguration() }
        }

        func removeLastUsedAccount() {
            manager?.assumeIsolated { $0.removeLastUsedAccount() }
        }
    }
}
