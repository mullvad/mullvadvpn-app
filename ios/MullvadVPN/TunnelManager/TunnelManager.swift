...
actor TunnelManager {
    ...
    /// the observer.
    /// Fires `tunnelManagerDidLoadConfiguration` asynchronously if configuration is already loaded,
    /// so observers added after `setConfigurationLoaded()` don't miss the one-shot notification.
    ...
    nonisolated func addObserver(_ observer: TunnelObserver) {
        ...
        Task { @MainActor in
            if await _isConfigurationLoaded {
