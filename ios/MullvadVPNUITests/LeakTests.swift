...
class LeakTests: LoggedInWithTimeUITestCase {
    /* override func setUp() {
         let ipAddress = Networking.getAlwaysReachableIPAddress()
         FirewallAPIClient().createRule(try! FirewallRule.makeBlockAllTrafficRule(toIPAddress: ipAddress))

         let trafficGenerator = TrafficGenerator(destinationHost: ipAddress, port: 80)
         trafficGenerator.startGeneratingUDPTraffic(interval: 3.0)
         Thread.sleep(forTimeInterval: 10.0)
         trafficGenerator.stopGeneratingUDPTraffic()
         // Networking.generateUDPTraffic(host: ipAddress, port: 80, duration: 30.0, dataInterval: 1.0)
         let capturedTraffic = stopPacketCapture()
         print(capturedTraffic)
     } */
