...
class PacketCaptureClient: TestRouterAPIClient {
    ...
    static func cutStreams(streams: [Stream], secondsStart: Double, secondsEnd: Double) -> [Stream] {
        ...
        return streams.filter { stream in
            return stream.dateInterval.start >= cutStartDate && stream.dateInterval.end <= cutEndDate
        }
    }
