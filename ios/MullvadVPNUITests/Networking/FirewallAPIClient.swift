...
class FirewallAPIClient {
    ...
    static func getIPAddress() -> String? {
        ...
        guard let firstInterfaceAddress = interfaceList else { return nil }

        // For each interface
        for ptr in sequence(first: firstInterfaceAddress, next: { $0.pointee.ifa_next }) {
            let flags = Int32(ptr.pointee.ifa_flags)
            let interfaceAddress = ptr.pointee.ifa_addr.pointee
            ...
            if (
                ...
                interfaceAddress.sa_family == UInt8(AF_INET) || interfaceAddress.sa_family == UInt8(AF_INET6) {
