...
class CustomListsTests: LoggedInWithTimeUITestCase {
    ...
    func testDeletingCustomListWithSingleEntryDoesNotCreateDuplicateInRecents() throws {
        ...
        /// 2 Items will be expected in the list:
        /// -  the default selection after re-enabling recents,
        /// -  the `BaseUITestCase.testsDefaultMullvadOwnedRelayName`
        XCTAssertTrue(
            app.buttons.matching(NSPredicate(format: "identifier BEGINSWITH %@", "recentListItem")).count == 2)
    }
