...
extension XCUIElement {
    ...
    func wait(
        ...
        let condition: (path: KeyPath<XCUIElement, Bool>, value: Bool) =
            switch condition {
            case .exists:
                (\.exists, true)
            case .notExists:
        ...
        let conditionMet =
            conditionIsAlreadyMet(condition)
            || wait(for: condition.path, toEqual: condition.value, timeout: timeout.rawValue)
    ...
    private func conditionIsAlreadyMet(_ condition: (path: KeyPath<XCUIElement, Bool>, value: Bool)) -> Bool {
        switch condition {
        case (\.exists, true):
            return exists
        case (\.exists, false):
            return !exists
        case (\.isHittable, true):
            return isHittable
        default:
            XCTFail("Unsupported condition")
            return false
        }
    }
}
