...
final class GotaTunActorTests: XCTestCase {
    ...
    func testKeyRotationTriggersReconnect() async throws {
        ...
        // Notify key rotation — should reconnect after propagation delay
        let reconnectedExpectation = expectation(description: "Reconnected after key rotation")
        ...
        let task = Task { @Sendable in
            ...
            for await state in states {
                ...
                if case .connected = state, sawReconnecting {
                    reconnectedExpectation.fulfill()
                    return
                }
            }
    ...
    func testCascadingErrorsWithRecovery() async throws {
        // Two consecutive errors, then success via recovery timer
        let factory = GotaTunAdapterFactoryStub(outcomes: [
            .error(.internalError("first")),
            .error(.internalError("second")),
            .connected(),
        ])
        let actor = makeActor(adapterFactory: factory)

        let connectedExpectation = expectation(description: "Eventually connected")
        ...
        XCTAssertEqual(factory.adaptersCreated.count, 3, "Should have retried through 2 errors")
    }

    // MARK: - Stop during connection

    func testStopDuringConnecting() async throws {
        // Use a slow-connect adapter so we can stop mid-connect
        let factory = GotaTunAdapterFactoryStub(outcome: .connected(after: .seconds(5)))
        let actor = makeActor(adapterFactory: factory)
        let options = launchOptions
