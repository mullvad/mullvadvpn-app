...
use h3::{client, ext::Protocol, quic::StreamId};
...
impl RunningClient {
    ...
    pub async fn until_closed(self) -> Result<()> {
        Ok(self.tasks.join_and_abort().await?)
    }
}
        ...
        select! {
            datagram = connection.read_datagram() => {
                match datagram {
                    ...
                    Err(err) => return Err(ProxyTaskError::ReadDatagram(err)),
                }
            }
