...
const FRAGMENT_BUFFER_CAP: usize = 64;
...
fn test_fragment_cap() {
    ...
    let fragment_survives_flood = |fragment_flood_count| {
        let mut fragments = Fragments::default();
        ...
        // then send a bunch of framents to fill the queue
        let mut bad_payload = Bytes::from([0u8; 2].to_vec());
        let incomplete_fragment = fragment_packet(1, &mut bad_payload, bad_packet_id)
            .unwrap()
            .next()
            .unwrap();
        for _ in fragment_buf.len()..fragment_flood_count {
            let packet = fragments
