    ...
    match winnet::get_best_default_route(addr_family) {
        Ok(Some(route)) => {
            let addr_family = match addr_family {
                WinNetAddrFamily::IPV4 => net::AddressFamily::Ipv4,
                WinNetAddrFamily::IPV6 => net::AddressFamily::Ipv6,
            };
            let luid = NET_LUID_LH {
                Value: route.interface_luid,
            };
            let interface_row = net::get_ip_interface_entry(addr_family, &luid).map_err(|e| {
