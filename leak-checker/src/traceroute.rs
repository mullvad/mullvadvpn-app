...
use platform::{AsyncIcmpSocket, AsyncIcmpSocketImpl, Impl, Traceroute};
...
pub async fn try_run_leak_test(opt: &TracerouteOpt) -> eyre::Result<LeakStatus> {
    ...
    if opt.icmp {
        timeout(SEND_TIMEOUT, send_icmp_probes(opt, &icmp_socket))
    ...
    // TODO: do concurrency again
    let recv_task = icmp_socket.recv_ttl_responses(&opt);
...
async fn send_udp_probes(opt: &TracerouteOpt, socket: &mut UdpSocket) -> eyre::Result<()> {
