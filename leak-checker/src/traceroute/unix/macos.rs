...
/// If the packet fails to parse, or is not a reply to a packet sent by us, this function returns
/// an error.
fn parse_icmp6_time_exceeded(ip_packet: &Ipv6Packet<'_>) -> anyhow::Result<IpAddr> {
    let ip_protocol = ip_packet.get_next_header();
    ensure!(ip_protocol == IpNextHeaderProtocols::Icmpv6, "Not ICMP6");
