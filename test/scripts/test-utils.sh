...
function download_app_package {
    ...
    if [[ ! -f "$package_dir/$filename" ]]; then
        ...
        echo "Build for version $version ($os) already exists at $package_dir/$filename, skipping download"
    fi
}
...
function build_current_version {
    ...
    if [ ! -f "$app_package" ]; then
        ...
        echo "Found app build for $CURRENT_VERSION ($TEST_OS)"
    fi
    ...
    if [ ! -f "$GUI_TEST_BIN" ]; then
        ...
        echo "Found GUI e2e test build for $CURRENT_VERSION ($TEST_OS)"
    fi
}