...
async fn test_clearing_blocked_state_on_failed_upgrade(
    ...
    // Assert that the blow settings are disabled. If they are not,
...
async fn test_note_clearing_blocked_state_on_failed_upgrade_with_auto_connect(
    ...
    // Make sure that auto-connectis enabled.
    // If it is not, then blocking firewall rules *will not* persist after a reboot.
    {
        mullvad_client.set_auto_connect(true).await?;
        let settings = mullvad_client.get_settings().await?;
        assert!(
            !settings.block_when_disconnected,
            "Block when disconnected should be disabled"
        );
        assert!(settings.auto_connect, "Auto connect should be enabled");
    }
    ...
    // The conn check should now succeed - the firewall filters should *not* have been removed at this point 💦❌
