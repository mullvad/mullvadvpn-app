...
async fn test_custom_access_methods_gui(
    ...
    let (host, endpoint) = (API.host(), API.address());
    let env = [
        (env::API_HOST_VAR, host),
        (env::API_ADDR_VAR, &endpoint.to_string()),
    ];

    let test_env = env
        .into_iter()
        .chain([(env::API_FORCE_DIRECT_VAR, "false")]);

    tokio::time::timeout(
        std::time::Duration::from_secs(60),
        rpc.set_daemon_environment(test_env),
    )
    .await
