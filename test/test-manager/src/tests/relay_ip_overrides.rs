...
use crate::vm;
use crate::vm::network::NON_TUN_GATEWAY;
...
use mullvad_types::{
    ...
    relay_constraints::{
        BridgeConstraints, BridgeSettings, BridgeState, BridgeType, Constraint,
        GeographicLocationConstraint, LocationConstraint, ObfuscationSettings, OpenVpnConstraints,
        RelayConstraints, RelayOverride, SelectedObfuscation, TransportPort, WireguardConstraints,
    },
...
pub async fn test_wireguard_ip_override(
    ...
    let (_proxy_abort_handle, _) = spawn_udp_proxy(
        SocketAddr::new(relay_ip.into(), TUNNEL_PORT),
        Some(TUNNEL_PORT),
    )
    .await
...
pub async fn test_openvpn_ip_override(
    ...
    let (_proxy_abort_handle, _) = spawn_tcp_proxy(
        SocketAddr::new(relay_ip.into(), TUNNEL_PORT),
        Some(TUNNEL_PORT),
    )
    .await
...
pub async fn test_bridge_ip_override(
    ...
    for shadowsocks in &relays.bridge.shadowsocks {
        ...
        proxy_abort_handles.push(spawn_udp_proxy((relay_ip, port).into(), Some(port)).await?);
        proxy_abort_handles.push(spawn_tcp_proxy((relay_ip, port).into(), Some(port)).await?);
    }
...
///
/// # Returns
/// - A handle that will stop the proxy when dropped
/// - The port that the proxy is listening on
async fn spawn_tcp_proxy(
    destination: SocketAddr,
    port: Option<u16>,
) -> anyhow::Result<(AbortOnDrop<()>, u16)> {
    let socket = TcpListener::bind((Ipv4Addr::UNSPECIFIED, port.unwrap_or(0))).await?;
    let bind_port = socket.local_addr()?.port();
    log::info!("started TCP proxy to {destination} on port {bind_port}");
...
async fn spawn_udp_proxy(
    ...
    let bind_port = socket.local_addr()?.port();
    log::info!("started UDP proxy to {destination} on port {bind_port}");
