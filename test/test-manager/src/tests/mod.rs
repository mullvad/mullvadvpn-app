...
async fn ensure_daemon_version(
    ...
    if !correct_daemon_version_is_running(rpc_provider.new_client().await).await
        ...
        && rpc.enable_mullvad_daemon().await.is_err() && rpc.start_mullvad_daemon().await.is_err()
    {
        log::debug!("Daemon has incorrect version or is not running and cannot be started");
