...
pub async fn test_bridge(
    ...
    let relay_settings = RelaySettings::Normal(RelayConstraints {
        location: helpers::into_constraint(&exit).unwrap(),
        tunnel_protocol: Constraint::Only(TunnelType::OpenVpn),
        ..Default::default()
    });
...
pub async fn test_multihop(
    ...
    let exit_constraint = helpers::into_constraint(&exit).unwrap();
    let wireguard_constraints = WireguardConstraints {
        use_multihop: true,
        entry_location: helpers::into_constraint(&entry).unwrap(),
        ..Default::default()
    };
