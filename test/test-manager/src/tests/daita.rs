...
/// Test that daita and daita_use_anywhere works by connecting
/// - to a non-DAITA relay with singlehop (should block)
/// - to a DAITA relay with singlehop
/// - to a DAITA relay with auto-multihop using use_anywhere
...
pub async fn test_daita(
    ...
    log::info!("Enabling DAITA and trying to connect without use_anywhere");
    {
        mullvad_client.set_enable_daita(true).await?;
        mullvad_client.set_daita_use_anywhere(false).await?;
    ...
    log::info!("Connecting to non-daita relay with use use_anywhere");
    {
        helpers::disconnect_and_wait(&mut mullvad_client).await?;
        mullvad_client.set_daita_use_anywhere(true).await?;
        let state = helpers::connect_and_wait(&mut mullvad_client)
            .await
            .context("Failed to connect with use_anywhere enabled")?;
        ...
        log::info!("Successfully multihopped with use use_anywhere");
    }

    log::info!("Connecting to daita relay with use_anywhere");
    {
        ...
        let state = helpers::connect_and_wait(&mut mullvad_client)
            .await
            .context("Failed to connect to daita location with use_anywhere enabled")?;
        ...
        log::info!("Successfully singlehopped with use_anywhere");
    }
