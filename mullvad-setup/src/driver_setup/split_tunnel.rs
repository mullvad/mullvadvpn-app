...
use windows_sys::Win32::{
    Foundation::{FALSE, GENERIC_READ, GENERIC_WRITE, HANDLE, INVALID_HANDLE_VALUE, TRUE},
...
impl StDevice {
    fn open() -> Result<Self, crate::Error> {
    ...
    fn state(&self) -> Result<u64, crate::Error> {
        let mut state: u64 = 0;
...
unsafe fn send_ioctl(
    ...
    let result = unsafe {
        GetOverlappedResult(device, &raw const overlapped, &raw mut bytes_returned, TRUE)
    };
    if result == FALSE {
        return Err(io::Error::last_os_error());
    }
