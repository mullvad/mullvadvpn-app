...
use talpid_types::net::{AllowedEndpoint, AllowedTunnelTraffic, TransportProtocol};
use talpid_types::tunnel::FirewallPolicyError;
use widestring::WideCString;

pub mod error;
pub mod sys;
use error::*; // TODO: Stop doing the wildcard import
use sys::*; // TODO: Stop doing the wildcard import
...
pub fn initialize_blocked(allowed_endpoint: AllowedEndpoint, allow_lan: bool) -> Result<(), Error> {
...
pub fn deinit(cleanup_policy: WinFwCleanupPolicy) -> Result<(), Error> {
...
pub fn reset() -> Result<(), FirewallPolicyError> {
...
pub fn apply_policy_connecting(
...
pub fn apply_policy_connected(
    endpoint: &AllowedEndpoint,
    winfw_settings: &WinFwSettings,
    tunnel_interface: &str,
    //dns_config: &crate::dns::ResolvedDnsConfig,
    // TODO: Rename?
    tunnel_dns_config: Vec<IpAddr>,
    non_tunnel_dns_config: Vec<IpAddr>,
) -> Result<(), FirewallPolicyError> {
    ...
    let tunnel_dns_servers: Vec<*const u16> = tunnel_dns_config
        .into_iter()
        .map(widestring_ip)
        .map(|ip| ip.as_ptr())
        .collect();
    let non_tunnel_dns_servers: Vec<*const u16> = non_tunnel_dns_config
        .into_iter()
        .map(widestring_ip)
        .map(|ip| ip.as_ptr())
        .collect();
...
impl WinFwPolicyStatus {
    pub fn into_result(self) -> Result<(), FirewallPolicyError> {
        match self {
            WinFwPolicyStatus::Success => Ok(()),
            WinFwPolicyStatus::GeneralFailure => Err(FirewallPolicyError::Generic),
            WinFwPolicyStatus::LockTimeout => {
                // TODO: Obtain application name and string from WinFw
                Err(FirewallPolicyError::Locked(None))
            }
        }
    }
}

impl From<WinFwPolicyStatus> for Result<(), FirewallPolicyError> {
