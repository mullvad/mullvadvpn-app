...
impl fmt::Debug for WireguardEndpointData {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        let private_key_field = match &self.client_private_key {
            Some(_) => &"Some(WgPrivateKey)",
            None => &"None",
        };
        f.debug_struct(&"WireguardEndpointData")
            ...
            .field("client_private_key", private_key_field)
            .finish()
    }
}
...
impl fmt::Debug for WgPrivateKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        f.debug_struct("WgPrivateKey")
            .field("private_key", &base64::encode(&self.private_key))
            .finish()
    }
}
...
impl fmt::Display for WgPrivateKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        write!(f, "{}", &base64::encode(&self.private_key))
    }
}
