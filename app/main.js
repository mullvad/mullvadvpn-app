...
const systemTempDir = getSystemTemporaryDirectory();
const rpcAddressFile = path.join(systemTempDir, '.mullvad_rpc_address');
  ...
  onReady: async () => {
    ...
    ipcMain.on('collect-logs', (event, id, toRedact) => {
      ...
      fs.readdir(appDelegate._logFileLocation, (err, files) => {
        ...
        const reportPath = path.join(systemTempDir, uuid.v4() + '.report');
