...
export default class TransitionContainer extends Component<TransitionContainerProps, State>{
  ...
  slideUpTransition(nextProps: TransitionContainerProps) {
    let currentTranslationValue = Animated.createValue(this.state.dimensions.height);
    this.setState({
  ...
  slideDownTransition(nextProps: TransitionContainerProps) {
    let previousTranslationValue = Animated.createValue(0);
    this.setState({
      previousChildren: this.props.children,
      childrenAnimation: Styles.createAnimatedViewStyle({
        pointerEvents: 'none',
  ...
  pushTransition(nextProps: TransitionContainerProps) {
    let currentTranslationValue = Animated.createValue(this.state.dimensions.width);
    let previousTranslationValue = Animated.createValue(0);
    this.setState({
      previousChildren: this.props.children,
      childrenAnimation: Styles.createAnimatedViewStyle({
  ...
  popTransition(nextProps: TransitionContainerProps) {
    let currentTranslationValue = Animated.createValue(- this.state.dimensions.width / 2 );
    let previousTranslationValue = Animated.createValue(0);
    this.setState({
      previousChildren: this.props.children,
      childrenAnimation: Styles.createAnimatedViewStyle({
