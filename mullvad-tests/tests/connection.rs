...
fn spawns_openvpn() {
    ...
    wait_for_file(&openvpn_args_file, Duration::from_secs(5));
...
fn respawns_openvpn_if_it_crashes() {
    ...
    // Stop OpenVPN by removing the command_line file
    fs::remove_file(&openvpn_args_file)
        .expect("Failed to remove the mock OpenVPN command-line file");
...
fn assert_current_state(mut rpc_client: DaemonRpcClient, expected_state: DaemonState) {
