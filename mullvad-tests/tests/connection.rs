...
const NOT_CONNECTED_STATE: DaemonState = DaemonState {
...
const EXITING_STATE: DaemonState = DaemonState {
    state: SecurityState::Secured,
    target_state: TargetState::Unsecured,
};
...
fn disconnects() {
    ...
    assert_state_event(&state_events, EXITING_STATE);
    assert_state_event(&state_events, NOT_CONNECTED_STATE);
    assert_eq!(rpc_client.get_state().unwrap(), NOT_CONNECTED_STATE);
}
