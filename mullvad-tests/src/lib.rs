...
impl PathWatcher {
    ...
    pub fn check_create_write_close_sequence(&mut self) -> bool {
        if self.next() != Some(watch_event::CREATE) {
            return false;
        }

        let mut event = self.next();
        if event != Some(watch_event::WRITE) {
            return false;
        }

        while let Some(watch_event::WRITE) = event {
            event = self.next();
        }

        event == Some(watch_event::CLOSE_WRITE)
    }
}
