...
impl ProblemReport {
    ...
    /// Tries to parse out the metadata map from a string that is supposed to be a report written by this struct.
    pub fn parse_metadata(report: &str) -> Option<HashMap<String, String>> {
        ...
        for line in lines {
            // Abort on first empty line, as this is the separator between the metadata and the first log
            if line.is_empty() {
                break;
            }
