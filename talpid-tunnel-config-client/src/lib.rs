...
pub struct DaitaSettings {
    pub client_machines: Vec<maybenot::Machine>,
...
fn parse_daita_response(daita: proto::DaitaResponseV2) -> Result<DaitaSettings, Error> {
    let machines = daita
        ...
        .collect::<Result<Vec<_>, maybenot::Error>>()
        .map_err(|error| {
            // NOTE: The parsing logic in `maybenot` always return the `Error::Machine` variant.
            let reason = match error {
                maybenot::Error::PaddingLimit | maybenot::Error::BlockingLimit => {
                    "unknown reason".to_string()
                }
                maybenot::Error::Machine(reason) => reason,
            };
