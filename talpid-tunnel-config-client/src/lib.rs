...
async fn connect_to_config_service(addr: SocketAddr) -> std::io::Result<tokio::net::TcpStream> {
    socket::TcpSocket::new()?.connect(addr).await
}
...
async fn connect_to_config_service(addr: SocketAddr) -> std::io::Result<tokio::net::TcpStream> {
    ...
    loop {
        attempts += 1;
        let error = match socket::TcpSocket::new()?.connect(addr).await {
            Ok(stream) => {
...
async fn connect_relay_config_client(ip: Ipv4Addr) -> Result<RelayConfigService, Error> {
    ...
    let connection = endpoint
        .connect_with_connector(service_fn(move |_| async move {
            let stream = connect_to_config_service(addr).await?;
