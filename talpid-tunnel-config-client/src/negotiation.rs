...
pub struct NegotiatedPeers {
    /// The private key to use with every relay.
    pub private_key: PrivateKey,
...
/// Negotiate ephemeral peers with the relays in `config`, using a single new ephemeral key.
///
...
pub async fn negotiate_ephemeral_peers<T: IngressTransport>(
    ...
    let ingress_peer = negotiate_with_ingress(config, transport, &ephemeral_key).await?;

    let exit_psk = match &config.relays {
        Relays::Singlehop(_) => None,
        Relays::Multihop { entry, exit } => {
            log::debug!("Negotiating ephemeral peer with the exit relay");
            negotiate_through_entry(
                config,
                transport,
                &ephemeral_key,
                entry,
...
async fn negotiate_with_ingress<T: IngressTransport>(
    ...
    let result =
        request_ephemeral_peer_through(&net, config, ephemeral_key, config.enable_daita).await;
    device.stop().await;
    result
}
...
///
/// The entry device uses `ephemeral_key` and `entry_psk`, which were negotiated with the entry
/// relay. The exit device uses the private key of this device.
async fn negotiate_through_entry<T: IngressTransport>(
    ...
    let Ingress {
        ...
        .connect(&ephemeral_key.public_key())
        .await
        .map_err(NegotiationError::Transport)?;
    let (net, net_recv, net_send, _net_guard) = userspace_net(config);
    ...
    let entry_device = match DeviceBuilder::new()
        ...
        .with_private_key(static_secret(ephemeral_key))
    ...
    let result = request_ephemeral_peer_through(&net, config, ephemeral_key, false).await;
