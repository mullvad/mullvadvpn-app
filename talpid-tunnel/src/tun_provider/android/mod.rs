...
impl AndroidTunProvider {
    ...
    pub fn create_blocking_tun(&mut self) -> Result<(), Error> {
        ...
        let config = VpnServiceConfig::new_without_dns(
            original_config.clone(),
            &self.allowed_lan_networks,
            self.allow_lan,
            self.excluded_packages.clone(),
    ...
    fn recreate_tun_if_open(&mut self) -> Result<(), Error> {
        ...
        let config = if blocking {
            VpnServiceConfig::new_without_dns(
                last_tun_config,
                &self.allowed_lan_networks,
                self.allow_lan,
                self.excluded_packages.clone(),
...
impl VpnServiceConfig {
    pub fn new(
        ...
        let dns_servers = Self::resolve_dns_servers(&tun_config, custom_dns);
    ...
    fn resolve_routes(
        ...
        let required_ipv6_routes = config.ipv6_gateway.map(|addr| IpNetwork::from(IpAddr::from(addr))).into_iter().collect::<Vec<IpNetwork>>();
