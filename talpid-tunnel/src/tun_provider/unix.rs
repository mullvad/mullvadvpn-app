...
impl TunnelDeviceBuilder {
    ...
    /// Disable route setup for tun device
    #[cfg(target_os = "macos")]
    pub fn disable_routing(&mut self) -> &mut Self {
        self.config.platform_config(|config| {
    ...
    fn set_ip(&mut self, ip: IpAddr) -> Result<(), Error> {
        match ip {
            ...
            IpAddr::V6(ipv6) => {
                ...
                let device = self.dev.tun_name().map_err(Error::GetDeviceName)?;
