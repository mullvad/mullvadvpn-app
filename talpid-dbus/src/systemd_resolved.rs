...
impl SystemdResolved {
    ...
    pub fn watch_dns_changes<
        F: FnMut(Vec<DnsServer>) -> bool + Send + Sync + 'static + Clone,
        S: Fn() -> bool,
    >(
        &mut self,
        callback: F,
        ...
        while should_continue() {
            ...
            {
                let mut cloned_callback = callback.clone();
                let _dns_matcher = self.dbus_connection.add_match(
                    match_rule,
                    move |mut prop_changed: PropertiesPropertiesChanged, _connection, _message| {
                        if let Some(dns_change) =
                            prop_changed.changed_properties.get_mut(DNS_SERVERS)
                        {
                            if let Some(message_data) =
                                dns_change.as_iter().and_then(|mut iter| iter.next())
                            {
                                match DnsServer::server_list_from_refarg(message_data) {
                                    Some(new_server_list) => {
                                        return cloned_callback(new_server_list);
                                    }
                                    None => {
                                        log::error!(
