...
impl Settings {
    ...
    async fn build_client_config(&self) -> Result<ClientConfig> {
    ...
    pub async fn connect_client(&self) -> Result<mullvad_masque_proxy::client::Client> {
        let config = self.build_client_config().await?;
        Ok(mullvad_masque_proxy::client::Client::connect(config).await?)
    }
