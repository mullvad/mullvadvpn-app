...
//!
//! The expected calling order on send is [`pad`] (append the returned padding, if any), then
//! [`obfuscate`]. On receive, [`deobfuscate`] validates the packet and returns a [`Verdict`]
//! telling the caller whether to forward, truncate, or drop it.
...
const MARKER_MASK: u8 = 0b11000000;
const MARKER: u8 = 0b1000000;
...
/// Whether an outgoing WireGuard packet has random padding appended before it is obfuscated.
...
/// A copy of `packet` with random padding appended, or `None` if this packet takes no padding.
///
/// Only handshakes are padded.
pub fn pad(packet: &[u8]) -> Option<Vec<u8>> {
    // Check message type before constructing rng
    if !takes_padding(packet) {
        return None;
    }

    let mut rng = rand::rng();
    let padding = rng.random_range(1..=MAX_PADDING);
    let mut padded = vec![0u8; packet.len() + padding];
    padded[..packet.len()].copy_from_slice(packet);
    rng.fill_bytes(&mut padded[packet.len()..]);
    Some(padded)
}

/// Obfuscate a plaintext WireGuard packet in place. `key` is the server's public key.
///
/// Any padding must already have been appended (use [`pad`]).
