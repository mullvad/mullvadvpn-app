//! Benchmark: the path a WireGuard packet takes to the remote, with and without the multiplexer.
//!
//! Both scenarios send one WireGuard data packet and wait for it to arrive at the relay:
//!
//! 1. **single_lwo** - one LWO obfuscator, reached over its loopback socket
//! 2. **multiplexer_lwo** - the same obfuscator behind the multiplexer, in connected mode
//!
//! The difference is what the multiplexer costs on top of the obfuscator itself. It is meant to
//! be run on two revisions and compared with criterion's `--save-baseline`/`--baseline`.
...
/// Control: plain UDP send and receive, with nothing in between. Identical on every revision, so
/// the other benchmarks can be read as a multiple of this one instead of in absolute time.
...
/// One LWO obfuscator on its own, which is how every obfuscation method is reached outside the
/// multiplexer.
...
/// The same obfuscator behind the multiplexer, once the multiplexer has committed to it.
