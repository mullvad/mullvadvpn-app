...
impl Relay {
    ...
    fn set_custom(&self, matches: &clap::ArgMatches) -> Result<()> {
        let custom_endpoint = match matches.subcommand() {
            ...
            ("wireguard", Some(wg_matches)) => Self::read_custom_wg_relay(wg_matches),
            (unknown_tunnel, _) => {
                // this maybe is unreachable ?
                eprintln!("Unknown tunnel type: {}", unknown_tunnel);
                ::std::process::exit(1);
            }
        };
    ...
    fn read_custom_wg_relay(matches: &clap::ArgMatches) -> CustomTunnelEndpoint {
        let host = value_t!(matches.value_of("host"), String).unwrap_or_else(|e| e.exit());
        let port = value_t!(matches.value_of("port"), u16).unwrap_or_else(|e| e.exit());
        ...
        let private_key = Self::validate_wg_key(&private_key_str).into();
        let peer_public_key = Self::validate_wg_key(&peer_key_str).into();
    ...
    fn validate_wg_key(key_str: &str) -> [u8; 32] {
