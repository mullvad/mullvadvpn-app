...
/// Metadata struct used for enhancing an instance of
/// [`GeographicLocationConstraint`] with rich information about the
/// constraint's country and city.
...
impl<'a> std::fmt::Display for GeographicLocationConstraintFormatter<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        let unwrap_country = |city: Option<String>, constraint: &str| {
            city.unwrap_or(format!("{constraint} <invalid country>"))
        };
