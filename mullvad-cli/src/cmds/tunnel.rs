...
impl Tunnel {
    ...
    async fn handle_wireguard(
        ...
        if let Some(allowed_ips_str) = allowed_ips {
            let ips: Vec<String> = allowed_ips_str
                .split(',')
                .map(|s| s.trim().to_string())
                .collect();
            match rpc.set_wireguard_allowed_ips(ips).await {
                Ok(_) => println!("WireGuard allowed IPs have been updated"),
                Err(mullvad_management_interface::Error::Rpc(status))
                    if status.code() == mullvad_management_interface::Code::InvalidArgument =>
                {
                    eprintln!("{}", status.message())
                }
                Err(e) => {
                    eprintln!("Failed to update WireGuard allowed IPs: {}", e);
                    return Err(e.into());
                }
            }
        }
