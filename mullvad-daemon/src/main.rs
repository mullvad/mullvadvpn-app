...
use tunnel_state_machine::{TunnelCommand, TunnelParameters, TunnelStateInfo};
...
impl Daemon {
    ...
    fn handle_tunnel_state_change(&mut self, tunnel_state_info: TunnelStateInfo) -> Result<()> {
        ...
        debug!("New tunnel state: {:?}", tunnel_state_info);
    ...
    fn reconnect_tunnel(&mut self) -> Result<()> {
        ...
        self.tunnel_command_tx
            .send(TunnelCommand::Reconnect(parameters))
            .expect("Tunnel state machine has stopped");
