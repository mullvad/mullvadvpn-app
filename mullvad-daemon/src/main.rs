...
impl DaemonExecutionState {
    ...
    pub fn disconnected(&mut self) {
        ...
        match self {
            Running => {
                mem::replace(self, RunningAndDisconnected);
            }
            Exiting => {
                mem::replace(self, Finished);
            }
            RunningAndDisconnected | Finished => {}
        };
    }

    pub fn starting_to_connect(&mut self) {
        use self::DaemonExecutionState::*;

        match self {
            RunningAndDisconnected => {
                mem::replace(self, Running);
            }
            Running | Exiting | Finished => {}
        };
    }

    pub fn allows_new_connection(&mut self) -> bool {
        use self::DaemonExecutionState::*;

        match self {
            RunningAndDisconnected | Running => true,
...
impl Daemon {
    pub fn new(
        ...
        let settings = settings::Settings::load().chain_err(|| "Unable to read settings")?;
        let allow_lan = settings.get_allow_lan();

        let firewall = FirewallProxy::new(&cache_dir).chain_err(|| ErrorKind::FirewallError)?;

        let (tx, rx) = mpsc::channel();
        let tunnel_requests =
            tunnel_state_machine::spawn(allow_lan, firewall, IntoSender::from(tx.clone()));
        ...
        Ok(Daemon {
            ...
            state: DaemonExecutionState::RunningAndDisconnected,
            rx,
            tx,
    ...
    fn handle_tunnel_state_change(&mut self, tunnel_state_info: TunnelStateInfo) -> Result<()> {
        ...
        if tunnel_state_info == NotConnected {
            self.state.disconnected();
        }

        self.security_state = match tunnel_state_info {
            NotConnected | Connecting => SecurityState::Unsecured,
    ...
    fn on_set_target_state(&mut self, new_target_state: TargetState) -> Result<()> {
        if self.state.allows_new_connection() {
            ...
            Ok(settings_changed) => {
                if settings_changed {
                    self.tunnel_requests
                        .send(TunnelRequest::AllowLan(allow_lan))
    ...
    fn start_tunnel(&mut self) -> Result<()> {
        ...
        self.state.starting_to_connect();
        self.tunnel_requests
