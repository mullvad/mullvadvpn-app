...
impl DaemonExecutionState {
    pub fn shutdown(&mut self, tunnel_state: TunnelStateInfo) {
        use self::DaemonExecutionState::*;

        match self {
            Running => {
                match tunnel_state {
                    TunnelStateInfo::Disconnected => mem::replace(self, Finished),
...
impl Daemon {
    pub fn new(
        ...
        Ok(Daemon {
            ...
            tunnel_state: TunnelStateInfo::Disconnected,
    ...
    fn handle_tunnel_state_change(&mut self, tunnel_state_info: TunnelStateInfo) -> Result<()> {
        ...
        if tunnel_state_info == Disconnected {
            self.state.disconnected();
        }

        self.tunnel_state = tunnel_state_info;
        self.security_state = match tunnel_state_info {
            Disconnected | Connecting => SecurityState::Unsecured,
            Connected | Reconnecting | Disconnecting => SecurityState::Secured,
        };
    ...
    fn start_tunnel(&mut self) -> Result<()> {
        ...
        self.tunnel_requests
            .send(TunnelRequest::Connect(parameters))
            .expect("Tunnel state machine has stopped");
    ...
    fn restart_tunnel(&mut self) -> Result<()> {
        ...
        self.tunnel_requests
            .send(TunnelRequest::Reconnect(parameters))
            .expect("Tunnel state machine has stopped");
