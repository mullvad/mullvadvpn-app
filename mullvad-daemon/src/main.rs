...
pub enum DaemonEvent {
    ...
    TunnelStateChange(TunnelStateTransition),
...
impl From<TunnelStateTransition> for DaemonEvent {
    fn from(tunnel_state_transition: TunnelStateTransition) -> Self {
        DaemonEvent::TunnelStateChange(tunnel_state_transition)
    }
}
...
impl Daemon {
    ...
    fn handle_event(&mut self, event: DaemonEvent) -> Result<()> {
        use DaemonEvent::*;
        match event {
            TunnelStateChange(transition) => self.handle_tunnel_state_change(transition),
    ...
    fn handle_tunnel_state_change(&mut self, tunnel_state: TunnelStateTransition) -> Result<()> {
