...
impl AccountHistory {
    pub fn new(cache_dir: &Path) -> Result<AccountHistory> {
        ...
        let mut reader = options
            ...
            .read(true)
            .create(true)
            .open(cache_dir.join(ACCOUNT_HISTORY_FILE))
            .map(io::BufReader::new)
            .chain_err(|| ErrorKind::ReadError)?;

        let accounts: Vec<AccountEntry> = match serde_json::from_reader(&mut reader) {
            Err(e) => {
    ...
    fn try_old_format(reader: &mut io::BufReader<fs::File>) -> Vec<AccountToken> {
    ...
    pub fn get(&mut self, account_id: &AccountToken) -> Result<Option<AccountEntry>> {
        let (idx, entry) = match self
            .accounts
            .iter()
            .enumerate()
            .filter(|(_idx, entry)| &entry.account == account_id)
            .next()
        {
    ...
    pub fn insert(&mut self, new_entry: AccountEntry) -> Result<()> {
        ...
        self.accounts = ::std::iter::once(&new_entry)
            .chain(self.accounts.iter().take(ACCOUNT_HISTORY_LIMIT - 1))
            .cloned()
            .collect();
        self.save_to_disk()
    }
    ...
    pub fn remove_account(&mut self, account_id: &str) -> Result<()> {
        self.accounts.retain(|entry| entry.account != account_id);
        self.save_to_disk()
    }
