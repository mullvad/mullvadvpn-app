...
fn version_check_inner(
    ...
    async move {
        let v2_result = v2_endpoint.await.map_err(Error::Download)?;

        Ok(VersionCache {
            current_version_supported: v2_result.current_version_supported,
            version_info: v2_result.version_info,
            #[cfg(in_app_upgrade)]
            metadata_version: v2_result.metadata_version,
        })
    }
}
