...
extern "system" fn find_address_module(address: *mut c_void) -> Option<ModuleInfo> {
    let snap =
    ...
    for module in snap.modules() {
        unsafe {
            if (address as *const BYTE) >= module.base_address
                && (address as *const BYTE) < module.base_address.offset(module.size as isize)
            {
                return Some(module);
            }
        }
...
impl Iterator for ProcessSnapshotModules<'_> {
    ...
    fn next(&mut self) -> Option<ModuleInfo> {
        ...
        Some(ModuleInfo {
            name: String::from(cstr.to_str().unwrap()),
