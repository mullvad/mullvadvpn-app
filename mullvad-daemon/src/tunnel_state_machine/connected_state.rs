...
use super::{
    AfterDisconnect, CloseHandle, ConnectingState, DisconnectingState, EventConsequence, Result,
...
impl ConnectedState {
    ...
    fn handle_commands(
        ...
        match try_handle_event!(self, commands.poll()) {
            ...
            Ok(TunnelCommand::Reconnect(parameters)) => NewState(DisconnectingState::enter(
                shared_values,
                (
                    self.close_handle.close(),
                    AfterDisconnect::Reconnect(parameters),
            ...
            Ok(TunnelCommand::Disconnect) | Err(_) => NewState(DisconnectingState::enter(
                shared_values,
                (self.close_handle.close(), AfterDisconnect::Nothing),
            )),
        ...
        match Self::set_security_policy(shared_values, endpoint, metadata, allow_lan) {
            ...
            Err(error) => Err((
                ...
                DisconnectingState::enter(
                    shared_values,
                    (bootstrap.close_handle.close(), AfterDisconnect::Nothing),
