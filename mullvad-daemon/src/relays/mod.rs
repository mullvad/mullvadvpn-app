...
impl ParsedRelays {
    ...
    fn filter_invalid_relays(relay: &mut Relay) {
        ...
        let filtered_openvpn_endpoints = &mut relay.tunnels.openvpn;
        filtered_openvpn_endpoints
            .retain(|data| data.port != 0)
;

        if filtered_openvpn_endpoints.len() < total_openvpn_endpoints {
            log::error!(
                "Relay {} contained {} invalid OpenVPN endpoints out of {} endpoints",
                relay.hostname,
                total_openvpn_endpoints - filtered_openvpn_endpoints.len(),
                total_openvpn_endpoints
            );
        }

        let total_wireguard_endpoints = relay.tunnels.wireguard.len();
        let filtered_wireguard_endpoints= &mut relay.tunnels.wireguard;
        ...
        if filtered_wireguard_endpoints.len() < total_wireguard_endpoints {
            log::error!(
                "Relay {} contained {} invalid WireGuard endpoints out of {} endpoints",
                relay.hostname,
                total_wireguard_endpoints - filtered_wireguard_endpoints.len(),
