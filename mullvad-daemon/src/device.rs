...
impl AccountManagerHandle {
    pub async fn login(&self, token: AccountToken) -> Result<(), Error> {
        ...
        self.send_command(AccountManagerCommand::Login(token, tx), rx)
            .await
    }

    pub async fn logout(&self) -> Result<(), Error> {
        let (tx, rx) = oneshot::channel();
        self.send_command(AccountManagerCommand::Logout(tx), rx)
            .await
    }

    pub async fn set(&self, data: DeviceData) -> Result<(), Error> {
        let (tx, rx) = oneshot::channel();
        self.send_command(AccountManagerCommand::SetData(data, tx), rx)
            .await
    }

    pub async fn data(&self) -> Result<Option<DeviceData>, Error> {
        let (tx, rx) = oneshot::channel();
        self.send_command(AccountManagerCommand::GetData(tx), rx)
            .await
    }

    pub async fn rotate_key(&self) -> Result<(), Error> {
        let (tx, rx) = oneshot::channel();
        self.send_command(AccountManagerCommand::RotateKey(tx), rx)
            .await
    }

    pub async fn set_rotation_interval(&self, interval: RotationInterval) -> Result<(), Error> {
        let (tx, rx) = oneshot::channel();
        self.send_command(AccountManagerCommand::SetRotationInterval(interval, tx), rx)
            .await
    }

    pub async fn rotation_interval(&self) -> Result<RotationInterval, Error> {
        let (tx, rx) = oneshot::channel();
        self.send_command(AccountManagerCommand::GetRotationInterval(tx), rx)
            .await
    }

    pub async fn validate_device(&self) -> Result<ValidationResult, Error> {
        let (tx, rx) = oneshot::channel();
        self.send_command(AccountManagerCommand::ValidateDevice(tx), rx)
            .await
    }
...
impl AccountManager {
    pub async fn spawn(
        ...
        tokio::spawn(manager.into_future(cmd_rx));
    ...
    async fn into_future(mut self, mut cmd_rx: mpsc::UnboundedReceiver<AccountManagerCommand>) {
