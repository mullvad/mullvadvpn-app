...
impl DeviceService {
    ...
    pub fn generate_for_account(
        ...
        let token_copy = account_number.clone();
        async move {
    ...
    pub async fn list_devices(&self, token: AccountNumber) -> Result<Vec<Device>, Error> {
        let proxy = self.proxy.clone();
        let api_handle = self.api_availability.clone();
        retry_future(
    ...
    pub async fn get(&self, token: AccountNumber, device: DeviceId) -> Result<Device, Error> {
        let proxy = self.proxy.clone();
        let api_handle = self.api_availability.clone();
        retry_future(
...
impl AccountService {
    ...
    pub async fn get_data(&self, token: AccountNumber) -> Result<AccountData, rest::Error> {
        let proxy = self.proxy.clone();
        let api_handle = self.api_availability.clone();
    ...
    pub async fn get_data_2(&self, token: AccountNumber) -> Result<AccountData, Error> {
