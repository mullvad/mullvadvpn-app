...
lazy_static::lazy_static! {
    ...
    static ref APP_VERSION: AppVersion = AppVersion::from_str(PRODUCT_VERSION).unwrap();
}
...
impl VersionUpdater {
    ...
    fn suggested_upgrade(
        ...
        let stable_version = response
            .latest_stable
            .as_ref()
            .and_then(|stable| Some((stable, AppVersion::from_str(stable)?)));

        let beta_version = AppVersion::from_str(&response.latest_beta);

        let stable_newer_than_beta = stable_version
            .and_then(|(_, stable_version)| Some(beta_version.clone()? < stable_version))
            .unwrap_or(false);

        if !show_beta || stable_newer_than_beta {
            return stable_version.and_then(|(stable_str, latest_stable)| {
                if latest_stable > *current_version {
                    Some(stable_str.to_owned())
                } else {
                    None
                }
            });
        ...
        if beta_version? > *current_version {
            Some(response.latest_beta.to_owned())
        } else {
            None
        }
    }
    ...
    fn test_version_parsing() {
        let tests = vec![
            ...
            (
                ...
                Some(AppVersion::Beta(2020, 15, 1)),
            ),
            ("", None),
        ];
