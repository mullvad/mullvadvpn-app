...
impl LogFileWriter {
    ...
    fn new(log_location: Option<LogLocation>) -> Result<Self, Error> {
        ...
        // using a task.
        // On Android, the runtime will be restarted multiple times during the application's lifecycle,
        // so we call `mem::forget` to never drop the guard. To flush on Android, one should call
        // `LogFileWriter::get_flusher`, to receive channel for manually flushing
