    ...
    for ManagementInterface<T>
{
    ...
    fn get_relay_locations(&self, _meta: Self::Metadata) -> BoxFuture<RelayList, Error> {
    ...
    fn get_account(&self, _meta: Self::Metadata) -> BoxFuture<Option<AccountToken>, Error> {
        trace!("get_account");
        let (tx, rx) = sync::oneshot::channel();
        let future = self
    ...
    fn get_relay_settings(&self, _meta: Self::Metadata) -> BoxFuture<RelaySettings, Error> {
    ...
    fn set_allow_lan(&self, _meta: Self::Metadata, allow_lan: bool) -> BoxFuture<(), Error> {
        trace!("set_allow_lan");
        let (tx, rx) = sync::oneshot::channel();
        let future = self
    ...
    fn get_allow_lan(&self, _meta: Self::Metadata) -> BoxFuture<bool, Error> {
        trace!("get_allow_lan");
        let (tx, rx) = sync::oneshot::channel();
        let future = self
    ...
    fn set_auto_connect(&self, _meta: Self::Metadata, auto_connect: bool) -> BoxFuture<(), Error> {
    ...
    fn get_auto_connect(&self, _meta: Self::Metadata) -> BoxFuture<bool, Error> {
    ...
    fn connect(&self, _meta: Self::Metadata) -> BoxFuture<(), Error> {
        trace!("connect");
        self.send_command_to_daemon(ManagementCommand::SetTargetState(TargetState::Secured))
    }

    fn disconnect(&self, _meta: Self::Metadata) -> BoxFuture<(), Error> {
        trace!("disconnect");
        self.send_command_to_daemon(ManagementCommand::SetTargetState(TargetState::Unsecured))
    }

    fn get_state(&self, _meta: Self::Metadata) -> BoxFuture<DaemonState, Error> {
        trace!("get_state");
    ...
    fn get_current_location(&self, _meta: Self::Metadata) -> BoxFuture<GeoIpLocation, Error> {
    ...
    fn shutdown(&self, _meta: Self::Metadata) -> BoxFuture<(), Error> {
        trace!("shutdown");
        self.send_command_to_daemon(ManagementCommand::Shutdown)
    }

    fn get_account_history(&self, _meta: Self::Metadata) -> BoxFuture<Vec<AccountToken>, Error> {
    ...
    fn get_tunnel_options(&self, _meta: Self::Metadata) -> BoxFuture<TunnelOptions, Error> {
    ...
    fn get_current_version(&self, _meta: Self::Metadata) -> BoxFuture<String, Error> {
        let (tx, rx) = sync::oneshot::channel();
        let future = self
    ...
    fn get_version_info(&self, _meta: Self::Metadata) -> BoxFuture<version::AppVersionInfo, Error> {
        let (tx, rx) = sync::oneshot::channel();
        let future = self
