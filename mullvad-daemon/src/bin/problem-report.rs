...
fn add_logs_from_log_directory(
    ...
    let mut logs_to_ignore: HashSet<PathBuf> =
        logs_to_ignore.iter().map(|&log| log.to_owned()).collect();
    ...
    for dir_entry in dir_entries {
        ...
        if file_path.extension() == log_extension && !logs_to_ignore.contains(&file_path) {
            ...
            logs_to_ignore.insert(file_path);
        }
    }
...
fn add_extra_logs(problem_report: &mut ProblemReport, extra_logs: &[&Path]) {
