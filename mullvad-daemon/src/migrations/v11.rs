...
fn get_api_access_method_names(api_access_methods: &[serde_json::Value]) -> Vec<String> {
    api_access_methods
        .iter()
        .map(get_api_access_method_name)
        .collect()
}
...
fn generate_access_method_name_initial_suffix(
    ...
    let access_method_name_count = access_method_names
        .iter()
        .filter(|name| *name == access_method_name)
        .count();
...
fn migrate_duplicated_api_access_method_names(settings: &mut serde_json::Value) -> Result<()> {
        ...
        .and_then(|custom_api_access_methods| custom_api_access_methods.as_array_mut())
    {
        for index in 0..custom_api_access_methods.len() {
            let access_method_names = get_api_access_method_names(custom_api_access_methods);
            let access_method_name = &access_method_names[index];
            ...
            if should_rename_api_access_method {
                ...
                custom_api_access_methods[index]["name"] =
                    encode_string_to_json(generated_access_method_name);
            }
        }
    }
