//! Given a top-level settings blob and [`Settings`], update the subset of the settings blob
//! represented by [`Settings`].

use crate::migrations::Error;
use crate::migrations::multihop::scenario::{self, Scenario};
...
///
/// See [scenario].
pub(crate) fn detect(settings: &v17::Settings) -> Scenario {
    use Scenario::*;
...
///
/// See [`v18::RelaySettingsInner::migrate`] for details.
fn relay_settings_migration(
    ...
    let Some(relay_settings) = v17::Settings::relay_settings(settings) else {
    ...
    let new = v18::RelaySettingsInner::migrate(
        serde_json::from_value(relay_settings.clone())
            .expect("It should be safe to cast settings to v17::RelaySettingsInner"),
        value,
        filters,
        automatic_entry,
    );
...
/// Update the daita value of an existing settings blob to the new [`v18::WireguardSettings`] format.
///
/// See [`v18::WireguardSettings::migrate`] for details.
fn daita_migration(settings: &mut Value) {
    let wg_settings = v17::Settings::wireguard_settings(settings);
    let wg: v17::WireguardSettings = serde_json::from_value(wg_settings.clone())
        .expect("It should be safe to cast settings to v17::WireguardSettings");
    *wg_settings = json!(v18::WireguardSettings::migrate(wg));
}
