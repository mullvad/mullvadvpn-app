    ...
    L: EventListener + Clone + Send + 'static,
{
    ...
    ///
    /// This function implements the method used to test if some
    /// [`AccessMethod`] is able to reach the API. Its parameters are as
    /// low-level as possible to promot re-use between different kinds of
    /// testing contexts, such as testing [`AccessMethodSetting`]s or on the fly
    /// testing of [`talpid_types::net::proxy::CustomProxy`]s.
    ...
    /// Create the [`ApiProxy`] which will test against this endpoint. It is
    /// limited to using `testee` to reach the Mullvad API.
