...
impl RelaySelector {
    ...
    pub fn new(rpc_handle: HttpHandle, resource_dir: &Path, cache_dir: &Path) -> Self {
        ...
        let unsynchronized_parsed_relays = match Self::read_cached_relays(&cache_path, resource_dir)
        {
            Ok(value) => value,
            Err(error) => {
    ...
    fn read_cached_relays(cache_path: &Path, resource_dir: &Path) -> Result<ParsedRelays> {
        match ParsedRelays::read_from(cache_path) {
            Ok(value) => Ok(value),
            Err(read_cache_error) => match ParsedRelays::read_from(
