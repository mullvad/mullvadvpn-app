...
const EXPONENTIAL_BACKOFF_DELAY: u64 = 2;
const EXPONENTIAL_BACKOFF_FACTOR: u64 = 1000;
...
impl RelayListUpdater {
    ...
    fn download_relay_list(
        ...
        let exponential_backoff = ExponentialBackoff::from_millis(EXPONENTIAL_BACKOFF_DELAY)
            .factor(EXPONENTIAL_BACKOFF_FACTOR)
            .max_delay(UPDATE_INTERVAL * 3);

        let download_future = retry_future_with_backoff(
            download_futures,
            |result| match result {
                Ok(_) => false,
                Err(RestError::DeserializeError(_)) => false,
                Err(_) => true,
            },
