...
///
/// Setting the `MULLVAD_LOCKDOWN_ON_CORRUPT_SETTINGS` environment
...
static LOCKDOWN_ON_CORRUPT_SETTINGS: std::sync::LazyLock<bool> =
    std::sync::LazyLock::new(
        || match std::env::var("MULLVAD_LOCKDOWN_ON_CORRUPT_SETTINGS") {
        ...
        R: Future<Output = Result<Settings, Error>>,
    {
        let mut result = match load_settings().await {
            ...
            Err(error) => {
                ...
                let settings = if *LOCKDOWN_ON_CORRUPT_SETTINGS {
