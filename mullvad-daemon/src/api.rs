...
impl ConnectionModesIterator {
    pub fn new(access_methods: Vec<AccessMethodSetting>) -> Result<ConnectionModesIterator, Error> {
        ...
        Ok(Self {
            next: None,
            current: iterator.next().ok_or(Error::NoSettings)?,
    ...
    ///
    /// If the `access_methods` argument is an empty vector, a singleton
    /// iterator providing the [`Direct`](access_method::Settings::direct)
    /// access method is created.
    fn new_iterator(
        access_methods: Vec<AccessMethodSetting>,
    ) -> Result<Box<dyn Iterator<Item = AccessMethodSetting> + Send>, Error> {
        if access_methods.is_empty() {
            Err(Error::NoSettings)
        } else {
