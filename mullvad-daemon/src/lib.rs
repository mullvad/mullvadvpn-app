    ...
    async fn handle_new_excluded_paths(
        ...
        let save_result = match update {
            ExcludedPathsUpdate::SetState(state) => {
                ...
                // split tunneling. We have observed users getting into the error state until they
                // reconnect in these scenarios. Since FDA is an implementation detail of split
                // tunneling, we don't actually have a way of getting this information at this
                // point, so we fallback to always reconnecting on macOS. This code can be removed
                // if we ever remove our dependency on FDA.
