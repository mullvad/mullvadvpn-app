...
impl<B> Daemon<B>
where
    B: EventBroadcaster + Clone + Send + 'static,
{
    pub fn start_with_event_broadcaster(
        event_broadcaster: B,
        log_dir: Option<PathBuf>,
        resource_dir: PathBuf,
        cache_dir: PathBuf,
    ...
    fn start_internal(
        ...
        let relay_list_broadcaster = event_broadcaster.clone();
    ...
    fn handle_tunnel_state_transition(&mut self, tunnel_state: TunnelStateTransition) {
        ...
        self.event_broadcaster.notify_new_state(tunnel_state);
    }
