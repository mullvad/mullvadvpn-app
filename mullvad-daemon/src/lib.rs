    ...
    L: EventListener,
{
    ...
    fn handle_feature_indicator_event(&self) {
        ...
        if let Some(current_indicators) = self.tunnel_state.get_feature_indicators() {
            let new_indicators = self.get_feature_indicators();
            if new_indicators != *current_indicators {
                let new_tunnel_state = self
                    .tunnel_state
                    .clone()
                    .set_feature_indicators(new_indicators);
                self.event_listener.notify_new_state(new_tunnel_state);
            }
        }
    }
