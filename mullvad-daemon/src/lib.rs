...
const TARGET_START_STATE_FILE: &str = "target-start-state.txt";
    ...
    L: EventListener + Clone + Send + 'static,
{
    pub fn start(
        ...
        let initial_target_state = if cached_target_state.is_none() {
            if settings.get_auto_connect() && settings.get_account_token().is_some() {
    ...
    fn on_prepare_restart(&mut self) {
        ...
        match File::create(&cache_file) {
            Ok(handle) => {
                let _ = serde_json::to_writer(io::BufWriter::new(handle), &self.target_state)
                    .map_err(|e| {
