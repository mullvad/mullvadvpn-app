...
impl DaemonRunner {
    ...
    fn request_clean_shutdown(&mut self) -> bool {
    ...
    #[cfg(unix)]
    fn send_terminate_signal(process: &duct::Handle) -> bool {
    ...
    #[cfg(not(unix))]
    fn send_terminate_signal(_process: &duct::Handle) -> bool {
        false
    }
}
...
impl Drop for DaemonRunner {
    fn drop(&mut self) {
        if let Some(process) = self.process.take() {
            if self.request_clean_shutdown() || Self::send_terminate_signal(&process) {
                let process: Arc<duct::Handle> = Arc::new(process);
