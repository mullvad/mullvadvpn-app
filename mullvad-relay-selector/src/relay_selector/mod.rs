...
/// [`RelaySettings::Custom`]. For this variant, the  an endpoint where the client should connect to
/// is already specified inside of the variant, so in practice relay selector's function is
/// superflous. Also, there exists no mapping to [`RelayQueryBuilder`].
#[derive(Debug, Clone)]
...
impl RelaySelector {
    ...
    /// Chooses a suitable relay from a set of parsed relays based on specified constraints and
    /// configuration.
    ///
    /// This function filters the available relays according to the given [query][`RelayQuery`] and
    /// [config][`NormalSelectorConfig`], then selects one relay at random from the filtered
    /// list.
    ///
    /// # Returns
