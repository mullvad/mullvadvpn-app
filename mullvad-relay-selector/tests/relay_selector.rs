...
use mullvad_relay_selector::{
    Error, GetRelay, OPENVPN_RETRY_ORDER, RelaySelector, SelectedObfuscator, SelectorConfig,
    WIREGUARD_RETRY_ORDER, WireguardConfig,
    query::{BridgeQuery, ObfuscationQuery, OpenVpnRelayQuery, builder::RelayQueryBuilder},
};
use mullvad_types::{
        ...
        match relay {
            GetRelay::OpenVpn {
                ...
                if BridgeQuery::should_use_bridge(&query.openvpn_constraints().bridge_settings) {
                    assert!(
                        bridge.is_some(),
                        "Relay selector should have selected a bridge for query {query:?}, but bridge was `None`"
                    );
                };
                assert!(query
                    .openvpn_constraints()
                ...
                relays: vec![
                    Relay {
                        ...
                        endpoint_data: RelayEndpointData::Wireguard(WireguardRelayEndpointData {
                            ...
                            daita: true,
                            quic: Some(Quic::new(
                                vec1!["185.195.233.172".parse().unwrap()],
                                "d18a23fc-7d5e-4fd2-8372-5e0e73de741a".to_string(),
                                "se-sto-wg-009.blockerad.eu".to_string(),
                            )),
                    ...
                    Relay {
                        ...
                        endpoint_data: RelayEndpointData::Wireguard(WireguardRelayEndpointData {
                            ...
                            quic: Some(Quic::new(
                                vec1![
                                    "89.37.63.195".parse().unwrap(),
                                    "2a02:6ea0:1508:4::c101".parse().unwrap()
                                ],
                                "d18a23fc-7d5e-4fd2-8372-5e0e73de741a".to_string(),
                                "se-sto-wg-204.blockerad.eu".to_string(),
                            )),
                            shadowsocks_extra_addr_in: Default::default(),
                        }),
                    },
