...
for ARCHITECTURE in $ARCHITECTURES; do
    ...
    STRIP_TOOL="${ANDROID_NDK_HOME}/toolchains/llvm/prebuilt/linux-x86_64/bin/${LLVM_TRIPLE}-strip"
    DESTINATION_PATH="$SCRIPT_DIR/android/build/extraJni/$ABI/libmullvad_jni.so"

    cp "$SCRIPT_DIR/target/$TARGET/$BUILD_TYPE/libmullvad_jni.so" "$DESTINATION_PATH"
    $STRIP_TOOL --strip-debug --strip-unneeded "$DESTINATION_PATH"
done
