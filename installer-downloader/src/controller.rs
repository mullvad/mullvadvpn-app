...
pub fn initialize_controller<T: AppDelegate + 'static>(delegate: &mut T, environment: Environment) {
    ...
    AppController::initialize::<_, Downloader<T>, _, DirProvider>(delegate, version_provider, environment)
}
...
impl AppController {
    ...
    pub fn initialize<D, A, V, DirProvider>(delegate: &mut D, version_provider: V, environment: Environment)
    where
