...
error_chain! {
    errors {
        AuthDenied {
            description("Authentication credentials were denied by Talpid Core IPC server")
        }
        AuthError {
            description("Failed to authenticate with Talpid Core IPC server")
        }
        IpcSendingError {
...
impl EventProcessor {
    pub fn new(arguments: &Arguments) -> Result<EventProcessor> {
        ...
        match ipc_client.call("authenticate", &[&arguments.credentials]) {
            ...
            Err(error) => bail!(Error::with_chain(error, ErrorKind::AuthError)),
        }
